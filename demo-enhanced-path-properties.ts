...
async function runInteractiveDemo() {
	...
	console.log(
		colorize("새로운 PathInfo 기능들을 실제로 체험해보세요!\n", "yellow"),
	);
	...
	try {
		...
		console.log("\n📊 PathInfo 상세 정보:");
		console.log(
			`   ${colorize("입력 경로:", "magenta")} ${singleResult.pathInfo.input}`,
		);
		console.log(
			`   ${colorize("절대 경로:", "magenta")} ${singleResult.pathInfo.absolute}`,
		);
		console.log(
			`   ${colorize("상대 경로:", "magenta")} ${singleResult.pathInfo.relative}`,
		);
		console.log(
			`   ${colorize("파일명:", "magenta")} ${singleResult.pathInfo.fileName}`,
		);
		console.log(
			`   ${colorize("기본명:", "magenta")} ${singleResult.pathInfo.baseName}`,
		);
		console.log(
			`   ${colorize("확장자:", "magenta")} ${singleResult.pathInfo.extension}`,
		);
		console.log(
			`   ${colorize("디렉토리:", "magenta")} ${singleResult.pathInfo.relativeDirectory}`,
		);
		console.log(
			`   ${colorize("프로젝트 내부:", "magenta")} ${singleResult.pathInfo.isWithinProject ? colorize("✅ Yes", "green") : colorize("❌ No", "red")}`,
		);
		console.log(
			`   ${colorize("깊이:", "magenta")} ${singleResult.pathInfo.depth}`,
		);

		subsection("1.2 기존 방식과 비교");
		console.log(
			`${colorize("기존 filePath:", "yellow")} ${singleResult.filePath}`,
		);
		console.log(
			`${colorize("새로운 absolute:", "yellow")} ${singleResult.pathInfo.absolute}`,
		);
		console.log(
			`${colorize("일치 여부:", "yellow")} ${singleResult.filePath === singleResult.pathInfo.absolute ? colorize("✅ 일치", "green") : colorize("❌ 불일치", "red")}`,
		);
		...
		const batchFiles = ["./README.md", "./docs/quickstart.md", "./CLAUDE.md"];
		...
		sortedResults.forEach((result, index) => {
			const status =
				result.errors.length > 0
					...
					: colorize("✅", "green");
			console.log(
				`   ${index + 1}. ${status} ${colorize(result.pathInfo.relative, "cyan")} (깊이: ${result.pathInfo.depth})`,
			);
		});
		...
		markdownFiles.forEach((info) => {
			console.log(
				`   - ${colorize(info.relative, "cyan")} (깊이: ${info.depth})`,
			);
		});
		...
		for (const test of validationTests) {
			...
			if (!validation.isValid && validation.validationError) {
				console.log(
					`      ${colorize(`→ ${validation.validationError}`, "red")}`,
				);
			}
		}
		...
		console.log("\n📊 프로젝트 구조 분석 결과:");
		console.log(
			`   ${colorize("총 파일 수:", "magenta")} ${structureAnalysis.totalFiles}`,
		);
		console.log(
			`   ${colorize("평균 깊이:", "magenta")} ${structureAnalysis.averageDepth.toFixed(1)}`,
		);
		console.log(
			`   ${colorize("최대 깊이:", "magenta")} ${structureAnalysis.maxDepth}`,
		);
		...
		Object.entries(structureAnalysis.depthDistribution)
			...
			.forEach(([depth, count]) => {
				const bar = "█".repeat(
					Math.ceil((count / structureAnalysis.totalFiles) * 20),
				);
		...
		Object.entries(structureAnalysis.extensionDistribution)
			...
			.forEach(([ext, count]) => {
				const percentage = (
					(count / structureAnalysis.totalFiles) *
					100
				).toFixed(1);
...
function analyzeProjectStructure(pathInfos: PathInfo[]) {
	...
	const depthDistribution = depths.reduce(
		(acc, depth) => {
	...
	const extensionDistribution = extensions.reduce(
		(acc, ext) => {
			acc[ext] = (acc[ext] || 0) + 1;
			return acc;
		},
	...
	const averageDepth =
		depths.reduce((sum, depth) => sum + depth, 0) / totalFiles;
