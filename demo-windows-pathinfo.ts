...
function windowsBanner(): void {
	...
	console.log(colorize('│                🪟 Windows PathInfo Demo                   │', 'blue'));
	console.log(colorize('│        Windows 환경 특화된 경로 분석 시스템                │', 'blue'));
	console.log(colorize('└─────────────────────────────────────────────────────────────┘', 'blue'));
}
...
function analyzeWindowsDrives(pathInfos: PathInfo[]) {
	...
	pathInfos.forEach(pathInfo => {
		...
		if (match) {
			...
			else if (['.ts', '.js', '.json'].includes(pathInfo.extension)) info.types.add('개발파일');
			else if (['.md', '.txt', '.doc'].includes(pathInfo.extension)) info.types.add('문서파일');
...
async function runWindowsDemo() {
	...
	console.log(`   컴퓨터명: ${colorize(process.env.COMPUTERNAME || 'Unknown', 'green')}`);
	console.log(`   사용자: ${colorize(process.env.USERNAME || 'Unknown', 'green')}`);
	console.log(`   사용자 프로필: ${colorize(process.env.USERPROFILE || homedir(), 'green')}`);
	...
	try {
		...
		// =================================================================
		console.log(colorize('\n╭─────────────────────────────────────────────────────────────╮', 'blue'));
		console.log(colorize('│              Demo 1: Windows 특화 경로 처리                │', 'blue'));
		console.log(colorize('╰─────────────────────────────────────────────────────────────╯', 'blue'));
		...
		windowsPaths.forEach((originalPath, i) => {
			...
			const wasExpanded = originalPath !== expandedPath ? colorize('🔄', 'cyan') : '  ';

			console.log(`   ${i + 1}. ${wasExpanded} ${originalPath}`);
			if (originalPath !== expandedPath) {
				console.log(`      → ${colorize(expandedPath, 'cyan')}`);
			}
			console.log(`      PathInfo: ${pathInfo.relative} (깊이: ${pathInfo.depth}, 구분자: ${separator}, ${status})`);
			...
			if (pathInfo.absolute.startsWith('\\\\')) {
				...
				if (uncMatch) {
					...
					console.log(`      UNC: 서버=${colorize(server, 'magenta')}, 공유=${colorize(share, 'magenta')}`);
				}
			}
		});
		...
		// =================================================================
		console.log(colorize('\n╭─────────────────────────────────────────────────────────────╮', 'blue'));
		console.log(colorize('│          Demo 2: Windows 환경 변수 및 시스템 경로          │', 'blue'));
		console.log(colorize('╰─────────────────────────────────────────────────────────────╯', 'blue'));
		...
		systemPaths.forEach(envPath => {
			...
			const pathInfo = createPathInfo(expandedPath);
			const exists = existsSync(expandedPath) ? colorize('✅', 'green') : colorize('❌', 'red');

			console.log(`   ${envPath}`);
			console.log(`   → ${pathInfo.absolute} ${exists}`);
			console.log(`     깊이: ${pathInfo.depth}, 절대경로: ${pathInfo.wasAbsolute ? '✅' : '❌'}`);
		});
		...
		// =================================================================
		console.log(colorize('\n╭─────────────────────────────────────────────────────────────╮', 'blue'));
		console.log(colorize('│             Demo 3: Windows 개발 도구 감지                 │', 'blue'));
		console.log(colorize('╰─────────────────────────────────────────────────────────────╯', 'blue'));
		...
		const devToolPaths = [
			...
			{ name: 'PowerShell', path: '%WINDIR%\\System32\\WindowsPowerShell\\v1.0\\powershell.exe' },
		...
		devToolPaths.forEach(tool => {
			...
			const status = exists ? colorize('✅ 설치됨', 'green') : colorize('❌ 없음', 'red');
			const icon = tool.name === 'Node.js' ? '📗' :
						tool.name === 'npm' ? '📦' :
						tool.name === 'Git' ? '🔄' :
						tool.name === 'VS Code' ? '💻' :
		...
		// =================================================================
		console.log(colorize('\n╭─────────────────────────────────────────────────────────────╮', 'blue'));
		console.log(colorize('│           Demo 4: Windows 프로젝트 구조 분석               │', 'blue'));
		console.log(colorize('╰─────────────────────────────────────────────────────────────╯', 'blue'));
		...
		if (grouped.has('.')) {
			...
			console.log(`   📁 ${colorize('(프로젝트 루트)', 'bright')} (${rootFiles.length}개)`);
			...
			if (rootFiles.length > 10) {
				console.log(`   └── ${colorize(`... 그리고 ${rootFiles.length - 10}개 더`, 'gray')}`);
			}
		}
		...
		if (driveAnalysis.size > 0) {
			...
			for (const [drive, info] of driveAnalysis) {
				...
				console.log(`   ${drive}: ${colorize(info.count.toString(), 'cyan')}개 파일 (평균 깊이: ${avgDepth})`);
				console.log(`      확장자: ${info.extensions.size}종류, 타입: ${Array.from(info.types).join(', ')}`);
			}
		}
		...
		// =================================================================
		console.log(colorize('\n╭─────────────────────────────────────────────────────────────╮', 'blue'));
		console.log(colorize('│            Demo 5: Windows 성능 및 최적화                  │', 'blue'));
		console.log(colorize('╰─────────────────────────────────────────────────────────────╯', 'blue'));
		...
		console.log(colorize(`\n⚡ Windows ${benchmarkPaths.length}개 경로 성능 테스트:`, 'yellow'));
		...
		console.log(`   환경 변수 확장: ${colorize(expandTime.toString(), 'cyan')}ms`);
		console.log(`   개별 PathInfo 생성: ${colorize(singleTime.toString(), 'cyan')}ms`);
		console.log(`   배치 PathInfo 생성: ${colorize(batchTime.toString(), 'cyan')}ms`);
		console.log(`   드라이브 분석: ${colorize(driveTime.toString(), 'cyan')}ms`);
		...
		// =================================================================
		console.log(colorize('\n╭─────────────────────────────────────────────────────────────╮', 'blue'));
		console.log(colorize('│             Demo 6: Windows 로그 및 진단                   │', 'blue'));
		console.log(colorize('╰─────────────────────────────────────────────────────────────╯', 'blue'));
		...
		console.log(`   분석된 드라이브: ${diagnosticInfo.analysis.drives.join(', ')}`);
		...
		// =================================================================
		console.log(colorize('\n╭─────────────────────────────────────────────────────────────╮', 'green'));
		console.log(colorize('│                🎉 Windows Demo 완료!                      │', 'green'));
		console.log(colorize('╰─────────────────────────────────────────────────────────────╯', 'green'));

		console.log(colorize('\n✨ Windows 특화 기능 검증 완료:', 'bright'));
		console.log(colorize('   ✅ 드라이브 문자 완전 지원 (C:, D:, E:)', 'green'));
