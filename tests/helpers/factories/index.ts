...
export class TestDataFactory {
  ...
  static createOptimizationOpportunity(overrides: Partial<OptimizationOpportunityData> = {}): OptimizationOpportunityData {
    return {
      id: `opp-${Date.now()}`,
      type: 'optimize',
  ...
  static createPerformanceBaseline(overrides: Partial<PerformanceBaselineData> = {}): PerformanceBaselineData {
    return {
  ...
  static createComplexTestSuite(): TestSuiteData {
    ...
    suite.tests = [
      this.createTestCase({ type: 'unit', category: 'critical', executionTime: 25, name: 'Critical Unit Test' }),
      this.createTestCase({ type: 'integration', category: 'optimize', executionTime: 150, name: 'Integration Test' }),
      this.createTestCase({ type: 'contract', category: 'critical', executionTime: 75, name: 'Contract Test' }),
      this.createTestCase({ type: 'unit', category: 'remove', executionTime: 200, flaky: true, name: 'Flaky Test' })
    ];

    suite.executionTime = suite.tests.reduce((sum, test) => sum + test.executionTime, 0);
    return suite;
  }
