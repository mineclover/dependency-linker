...
export class TempResourceManager {
	 ...
	 */
	public async createTestFiles(
		files: Record<string, string>,
	): Promise<Record<string, TempResource>> {
...
export async function withResourceLimits<T>(
	...
	const { maxMemoryMB = 500, maxFileHandles = 100, timeoutMs = 30000 } = limits;

	// Set timeout
	const timeoutPromise = new Promise<never>((_, reject) => {
		setTimeout(
			() => reject(new Error(`Test exceeded timeout: ${timeoutMs}ms`)),
			timeoutMs,
		);
	});
	...
	try {
		...
		if (after.fileHandles > maxFileHandles) {
			throw new Error(
				`File handle limit exceeded: ${after.fileHandles} > ${maxFileHandles}`,
			);
		}
