...
describe("CircularDependencyDetector", () => {
    describe("Simple Circular Dependencies", () => {
        test("should detect basic circular dependency (A -> B -> A)", async () => {
            ...
            const detector = (0, CircularDependencyDetector_1.createCircularDependencyDetector)({
                maxDepth: 5,
                maxCycles: 10,
                timeout: 1000,
                edgeTypes: ["imports"],
            });
            const result = await detector.detect(async (nodeId) => graph.get(nodeId) || [], async () => [
                { id: "A", type: "file" },
                { id: "B", type: "file" },
            ]);
        ...
        test("should detect self-referencing circular dependency (A -> A)", async () => {
            ...
            const result = await detector.detect(async (nodeId) => graph.get(nodeId) || [], async () => [{ id: "A", type: "file" }]);
            expect(result.cycles.length).toBe(1);
    ...
    describe("Performance and Limits", () => {
        test("should respect depth limits", async () => {
            ...
            const result = await detector.detect(async (nodeId) => graph.get(nodeId) || [], async () => Array.from({ length: chainLength }, (_, i) => ({
                id: `node${i}`,
                type: "file",
            })));
        ...
        test("should respect timeout limits", async () => {
            ...
            const result = await detector.detect(async (nodeId) => {
                await new Promise((resolve) => setTimeout(resolve, 1));
                return graph.get(nodeId) || [];
            }, async () => Array.from({ length: nodeCount }, (_, i) => ({
                id: `node${i}`,
                type: "file",
            })));
    ...
    describe("Edge Type Filtering", () => {
        test("should only detect cycles for specified edge types", async () => {
            ...
            const importDetector = (0, CircularDependencyDetector_1.createCircularDependencyDetector)({
                maxDepth: 10,
                maxCycles: 10,
                timeout: 1000,
                edgeTypes: ["imports"],
            });
            const importResult = await importDetector.detect(async (nodeId) => (graph.get(nodeId) || []).filter((edge) => edge.type === "imports"), async () => [
                { id: "A", type: "file" },
                { id: "B", type: "file" },
                { id: "C", type: "file" },
            ]);
            expect(importResult.cycles.length).toBe(1);
            const callDetector = (0, CircularDependencyDetector_1.createCircularDependencyDetector)({
                maxDepth: 10,
                maxCycles: 10,
                timeout: 1000,
                edgeTypes: ["calls"],
            });
            const callResult = await callDetector.detect(async (nodeId) => (graph.get(nodeId) || []).filter((edge) => edge.type === "calls"), async () => [
                { id: "A", type: "file" },
                { id: "B", type: "file" },
                { id: "C", type: "file" },
            ]);
    ...
    describe("Node-Specific Detection", () => {
        test("should detect cycles from specific starting node", async () => {
            ...
            const result = await detector.detectFromNode("A", async (nodeId) => graph.get(nodeId) || []);
            expect(result.cycles.length).toBe(1);
        ...
        test("should find circular path between two specific nodes", async () => {
            ...
            const circularPath = await detector.findCircularPath("A", "C", async (nodeId) => graph.get(nodeId) || []);
    ...
    describe("Edge Cases", () => {
        test("should handle empty graph", async () => {
            const detector = (0, CircularDependencyDetector_1.createCircularDependencyDetector)();
            const result = await detector.detect(async () => [], async () => []);
            expect(result.cycles.length).toBe(0);
        ...
        test("should handle single node with no edges", async () => {
            const detector = (0, CircularDependencyDetector_1.createCircularDependencyDetector)();
            const result = await detector.detect(async () => [], async () => [{ id: "A", type: "file" }]);
            expect(result.cycles.length).toBe(0);
