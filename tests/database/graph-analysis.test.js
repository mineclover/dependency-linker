...
describe("Graph Analysis System - Comprehensive Test Scenarios", () => {
    ...
    beforeAll(async () => {
        testDir = await (0, promises_1.mkdtemp)((0, node_path_1.join)((0, node_os_1.tmpdir)(), "graph-test-"));
    });
    beforeEach(async () => {
        ...
        analyzer = (0, NodeCentricAnalyzer_1.createNodeCentricAnalyzer)(db, queryEngine, nodeIdentifier);
    });
    afterEach(async () => {
        await db.close();
    });
    ...
    describe("Scenario 1: React Application Analysis", () => {
        ...
        test("should analyze hook dependencies", async () => {
            ...
            if (useUserDataId) {
                ...
                const apiCalls = hookDependencies.filter((dep) => dep.identifier.includes("api") || dep.type === "function");
        ...
        test("should analyze service layer coupling", async () => {
            ...
            if (userServiceId) {
                ...
                if (analysis.risks.highCoupling) {
                    expect(analysis.dependencies.direct.length +
                        analysis.dependents.direct.length).toBeGreaterThan(10);
                }
            }
        });
    ...
    describe("Scenario 3: Library and Package Analysis", () => {
        ...
        test("should analyze library usage patterns", async () => {
            ...
            if (reactLibId) {
                ...
                const componentFiles = reactDependents.filter((dep) => dep.sourceFile.includes("components/") ||
        ...
        test("should identify unused libraries", async () => {
            ...
            const usedLibraries = await Promise.all(allLibraries.map(async (lib) => {
                const dependents = await db.findNodeDependents(lib.id, ["imports"]);
    ...
    describe("Scenario 4: Node-Centric Deep Analysis", () => {
        ...
        test("should analyze node neighborhood", async () => {
            ...
            expect(neighborhood.immediate.incoming.length +
                neighborhood.immediate.outgoing.length).toBeGreaterThan(0);
        });
