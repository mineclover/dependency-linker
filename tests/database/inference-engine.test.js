...
const InferenceEngine_1 = require("../../src/database/inference/InferenceEngine");
describe("InferenceEngine", () => {
    let db;
    let engine;
    let tempDir;
    beforeEach(async () => {
        tempDir = (0, node_fs_1.mkdtempSync)((0, node_path_1.join)((0, node_os_1.tmpdir)(), "inference-test-"));
        const dbPath = (0, node_path_1.join)(tempDir, "test.db");
        db = (0, GraphDatabase_1.createGraphDatabase)(dbPath);
        await db.initialize();
        engine = new InferenceEngine_1.InferenceEngine(db, {
            enableCache: true,
    ...
    describe("Cache Synchronization", () => {
        ...
        test("should respect cache disabled config", async () => {
            const noCacheEngine = new InferenceEngine_1.InferenceEngine(db, {
                enableCache: false,
            });
    ...
    describe("Inheritable Inference", () => {
        test("should infer inheritable relationships", async () => {
            ...
            const inferences = await engine.queryInheritable(fileNode, "contains", "extends");
            expect(inferences.length).toBeGreaterThan(0);
    ...
    describe("Inference Validation", () => {
        ...
        test("should report cycle details in warnings", async () => {
            ...
            const validation = await engine.validate();
            if (!validation.valid) {
                expect(validation.warnings.some((w) => w.includes("Cycle:"))).toBe(true);
            }
        });
    });
...
//# sourceMappingURL=inference-engine.test.js.map