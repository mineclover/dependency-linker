...
describe('ITestAnalyzer Contract Tests', () => {
  ...
  describe('Error Handling Contract', () => {
    ...
    test('categorizeTests should reject invalid input', async () => {
      ...
      await expect(analyzer.categorizeTests(undefined as any)).rejects.toThrow();
    });

    test('identifyOptimizations should reject invalid input', async () => {
      await expect(analyzer.identifyOptimizations(null as any)).rejects.toThrow();
      await expect(analyzer.identifyOptimizations(undefined as any)).rejects.toThrow();
    });
  });
  ...
  describe('Performance Contract', () => {
    test('analyzeTestSuite should complete within reasonable time', async () => {
      ...
      expect(duration).toBeLessThan(CONTRACT_SCENARIOS.testAnalysis.expectedMaxExecutionTime);
    });

    test('should handle expected test count range', async () => {
      const result = await analyzer.analyzeTestSuite('tests/');
      expect(result.totalTests).toBeGreaterThanOrEqual(CONTRACT_SCENARIOS.testAnalysis.expectedMinTests);
    });
  });
