...
class MockTestBenchmark implements ITestBenchmark {
	...
	compareBenchmarks(
		...
		if (!baseline || !current) {
			throw new Error(
				"Both baseline and current benchmark results are required",
			);
		}
		...
		const standardError =
			pooledStdDev *
			Math.sqrt(1 / baseline.iterations + 1 / current.iterations);
...
describe("ITestBenchmark Contract Tests", () => {
	...
	describe("Error Handling Contract", () => {
		test("benchmarkExecution should reject invalid input", async () => {
			await expect(benchmark.benchmarkExecution(null as any)).rejects.toThrow(
				/function is required/,
			);
			await expect(
				benchmark.benchmarkExecution(() => Promise.resolve(), 0),
			).rejects.toThrow(/positive number/);
			await expect(
				benchmark.benchmarkExecution(() => Promise.resolve(), -1),
			).rejects.toThrow(/positive number/);
		});

		test("benchmarkMemory should reject invalid input", async () => {
			await expect(benchmark.benchmarkMemory(null as any)).rejects.toThrow(
				/function is required/,
			);
			await expect(benchmark.benchmarkMemory(undefined as any)).rejects.toThrow(
				/function is required/,
			);
		});

		test("compareBenchmarks should reject invalid input", async () => {
			const validResult = await benchmark.benchmarkExecution(
				() => Promise.resolve(),
				3,
			);

			expect(() =>
				benchmark.compareBenchmarks(null as any, validResult),
			).toThrow(/required/);
			expect(() =>
				benchmark.compareBenchmarks(validResult, null as any),
			).toThrow(/required/);
			expect(() =>
				benchmark.compareBenchmarks(undefined as any, validResult),
			).toThrow(/required/);
		});
	});
	...
	describe("Memory Benchmark Contract", () => {
		test("should measure memory usage", async () => {
			...
			expect(result.peakMemory.heapUsed).toBeGreaterThanOrEqual(
				result.initialMemory.heapUsed,
			);
			expect(result.finalMemory.heapUsed).toBeGreaterThanOrEqual(
				result.initialMemory.heapUsed,
			);
		});
	...
	describe("Edge Cases Contract", () => {
		test("should handle functions that throw errors", async () => {
			...
			await expect(benchmark.benchmarkExecution(errorFn, 3)).rejects.toThrow(
				"Test error",
			);
			await expect(benchmark.benchmarkMemory(errorFn)).rejects.toThrow(
				"Test error",
			);
		});
