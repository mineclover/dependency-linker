...
`;
			const result = (0,
			MarkdownDependencyExtractor_1.extractMarkdownDependencies)(
				"docs/index.md",
				content,
			);
...
`;
			...
			(0, globals_1.expect)(localImage?.metadata?.title).toBe(
...
`;
			...
			(0, globals_1.expect)(anchorLinks[0].metadata?.anchor).toBe(
				"introduction",
			);
		});
	});
...
`;
			...
			(0, globals_1.expect)(heading1?.text).toBe(
				"API Design #architecture #design",
			);
			...
			(0, globals_1.expect)(heading2?.text).toBe(
				"User Authentication #security #api",
			);
...
`;
			...
			const graphResult = await (0, MarkdownToGraph_1.markdownResultToGraph)(
				db,
				result,
				{
			...
			(0, globals_1.expect)(
				graphResult.relationshipsCreated,
			...
			(0, globals_1.expect)(archNode?.semanticTags).toEqual([
				"architecture",
				"system-design",
			]);
			...
			(0, globals_1.expect)(authNode?.semanticTags).toEqual([
				"security",
				"implementation",
			]);
		});
	...
	(0, globals_1.describe)("GraphDB Integration", () => {
		...
		(0, globals_1.it)(
			"should convert markdown dependencies to graph",
			async () => {
				const content = `
# Test Document
...
`;
		...
		(0, globals_1.it)(
			"should query markdown dependencies from GraphDB",
			async () => {
				const content = `
# Query Test
...
`;
				...
				const deps = await (0, MarkdownToGraph_1.queryMarkdownDependencies)(
					db,
					"query-test.md",
				);
				(0, globals_1.expect)(deps.length).toBeGreaterThanOrEqual(2);
				(0, globals_1.expect)(deps.some((d) => d.type === "md-link")).toBe(
					true,
				);
			},
		);
...
`;
			...
			(0, globals_1.expect)(
				archHeadings.every((h) => h.allTypes.includes("architecture")),
			).toBe(true);
			...
			const authHeading = securityHeadings.find((h) =>
...
`;
			...
			(0, globals_1.expect)(headings[0].semanticTypes).toEqual([
				"architecture",
			]);
			(0, globals_1.expect)(headings[1].semanticTypes).toEqual([
				"implementation",
				"backend",
			]);
...
`;
			...
			(0, globals_1.expect)(types.get("architecture")).toBeGreaterThanOrEqual(
				2,
			);
		...
		(0, globals_1.it)(
			"should only accept English semantic tags in headings",
			() => {
				const content = `
...
`;
				...
				const heading1 = result.headings?.[0];
				(0, globals_1.expect)(heading1?.tags).toEqual([
					"architecture",
					"design",
				]);
				...
				const heading3 = result.headings?.[2];
				(0, globals_1.expect)(heading3?.tags).toEqual([
					"api",
					"implementation",
				]);
		...
		(0, globals_1.it)(
			"should support complete workflow: extract → store → query",
			async () => {
				const content = `
...
`;
				...
				(0, globals_1.expect)(graphResult.nodesCreated).toBeGreaterThan(0);
				(0, globals_1.expect)(graphResult.relationshipsCreated).toBeGreaterThan(
					0,
				);
				const {
					queryHeadingsBySemanticType,
				...
				(0, globals_1.expect)(securityHeadings.length).toBeGreaterThanOrEqual(
					1,
				);
				const fileHeadings = await queryFileHeadings(db, "project.md");
				(0, globals_1.expect)(fileHeadings.length).toBe(5);
				const archHeading = fileHeadings.find((h) =>
	...
	(0, globals_1.describe)("Complex Document Test", () => {
		(0, globals_1.it)(
			"should extract all dependencies from comprehensive markdown",
			() => {
				const content = `---
...
`;
				...
				(0, globals_1.expect)(
					result.dependencies.some((d) => d.type === "link"),
				).toBe(true);
				(0, globals_1.expect)(
					result.dependencies.some((d) => d.type === "image"),
				).toBe(true);
				(0, globals_1.expect)(
					result.dependencies.some((d) => d.type === "wikilink"),
				).toBe(true);
				(0, globals_1.expect)(
				...
				(0, globals_1.expect)(
					result.dependencies.some((d) => d.type === "include"),
				).toBe(true);
				(0, globals_1.expect)(
					result.dependencies.some((d) => d.type === "anchor"),
				).toBe(true);
				(0, globals_1.expect)(
					result.dependencies.some((d) => d.type === "hashtag"),
				).toBe(true);
				(0, globals_1.expect)(result.frontMatter?.title).toBe(
					"Comprehensive Test",
				);
