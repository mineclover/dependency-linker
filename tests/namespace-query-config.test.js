...
(0, globals_1.describe)("Namespace Query Configuration", () => {
	...
	(0, globals_1.beforeEach)(() => {
		...
		manager = new analysis_namespace_js_1.AnalysisNamespaceManager(
			testConfigPath,
		);
	});
	...
	(0, globals_1.describe)("Query Category Mapping", () => {
		(0, globals_1.it)("should have all required query categories", () => {
			...
			expectedCategories.forEach((category) => {
				(0, globals_1.expect)(
					analysis_namespace_js_1.QUERY_CATEGORY_MAPPING,
				).toHaveProperty(category);
				(0, globals_1.expect)(
					Array.isArray(
						analysis_namespace_js_1.QUERY_CATEGORY_MAPPING[category],
					),
				).toBe(true);
				(0, globals_1.expect)(
					analysis_namespace_js_1.QUERY_CATEGORY_MAPPING[category].length,
				).toBeGreaterThan(0);
			});
		});
		(0, globals_1.it)(
			"should have correct query counts for each category",
			() => {
				(0, globals_1.expect)(
				...
				(0, globals_1.expect)(
					analysis_namespace_js_1.QUERY_CATEGORY_MAPPING["dependency-tracking"],
				).toHaveLength(6);
				(0, globals_1.expect)(
					analysis_namespace_js_1.QUERY_CATEGORY_MAPPING["advanced-analysis"],
				).toHaveLength(3);
			},
		);
		(0, globals_1.it)("should have unique query IDs across categories", () => {
			const allQueries = Object.values(
				analysis_namespace_js_1.QUERY_CATEGORY_MAPPING,
			).flat();
	...
	(0, globals_1.describe)("Query Category Information", () => {
		...
		(0, globals_1.it)(
			"should have correct query counts in category info",
			() => {
				const categories = manager.getQueryCategories();
				(0, globals_1.expect)(categories["basic-analysis"].queryCount).toBe(3);
				(0, globals_1.expect)(categories["symbol-definitions"].queryCount).toBe(
					9,
				);
				(0, globals_1.expect)(
					categories["dependency-tracking"].queryCount,
				).toBe(6);
				(0, globals_1.expect)(categories["advanced-analysis"].queryCount).toBe(
					3,
				);
			},
		);
	...
	(0, globals_1.describe)("Query Selection for Categories", () => {
		(0, globals_1.it)(
			"should return correct queries for single category",
			() => {
				const basicQueries = manager.getQueriesForCategories([
					"basic-analysis",
				]);
				(0, globals_1.expect)(basicQueries).toEqual(
					analysis_namespace_js_1.QUERY_CATEGORY_MAPPING["basic-analysis"],
				);
			},
		);
		(0, globals_1.it)(
			"should return correct queries for multiple categories",
			() => {
				const categories = ["basic-analysis", "symbol-definitions"];
		...
		(0, globals_1.it)(
			"should remove duplicates when combining categories",
			() => {
				const categories = [
					"basic-analysis",
					"symbol-definitions",
	...
	(0, globals_1.describe)("Namespace Query Configuration", () => {
		(0, globals_1.it)(
			"should create namespace with default query configuration",
			async () => {
				...
				(0, globals_1.expect)(sourceNamespace.queries.categories).toContain(
					"basic-analysis",
				);
				(0, globals_1.expect)(sourceNamespace.queries.categories).toContain(
					"symbol-definitions",
		...
		(0, globals_1.it)(
			"should handle namespace without queries configuration",
			async () => {
				const customConfig = {
				...
				const customManager =
					new analysis_namespace_js_1.AnalysisNamespaceManager(testConfigPath);
				const activeQueries =
	...
	(0, globals_1.describe)("Custom Query Configuration", () => {
		(0, globals_1.it)(
			"should include custom queries when enabled",
			async () => {
				const customConfig = {
					projectName: "test-project",
					rootPath: process.cwd(),
				...
				(0, globals_1.expect)(activeQueries).toContain("ts-import-sources");
				(0, globals_1.expect)(activeQueries).toContain(
		...
		(0, globals_1.it)(
			"should exclude custom queries when disabled",
			async () => {
				const customConfig = {
					projectName: "test-project",
					rootPath: process.cwd(),
	...
	(0, globals_1.describe)("Query Performance Options", () => {
		(0, globals_1.it)(
			"should have correct default performance options",
			async () => {
				const config = await manager.loadConfig();
				const sourceNamespace = config.namespaces.source;
				(0, globals_1.expect)(
					sourceNamespace.queries.options.enableParallelExecution,
				).toBe(true);
				(0, globals_1.expect)(
					sourceNamespace.queries.options.enableCaching,
				).toBe(true);
				(0, globals_1.expect)(
					sourceNamespace.queries.options.maxConcurrency,
				).toBe(4);
			},
		);
		(0, globals_1.it)("should allow custom performance options", async () => {
			...
			(0, globals_1.expect)(
				performanceNamespace.queries.options.enableParallelExecution,
			).toBe(false);
			(0, globals_1.expect)(
				performanceNamespace.queries.options.enableCaching,
			).toBe(false);
			(0, globals_1.expect)(
				performanceNamespace.queries.options.maxConcurrency,
			).toBe(1);
		});
	});
});
