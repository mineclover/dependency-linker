...
(0, globals_1.describe)("Query System Integration", () => {
	...
	(0, globals_1.beforeEach)(() => {
		...
		testConfigPath = path_1.default.join(
			testDir,
...
`;
			const result = await (0, analysis_js_1.analyzeFile)(
				testCode,
				"typescript",
				"test-component.tsx",
			);
			...
			(0, globals_1.expect)(
				Object.keys(result.queryResults).length,
			).toBeGreaterThan(0);
		});
...
`;
			...
			const sources = importSources.map((item) => item.source).filter(Boolean);
...
`;
			...
			const importNames = namedImports.map((item) => item.name).filter(Boolean);
...
`;
			...
			const typeNames = typeImports.map((item) => item.name).filter(Boolean);
	...
	(0, globals_1.describe)("Namespace-based Query Execution", () => {
		(0, globals_1.it)(
			"should execute queries based on namespace configuration",
			async () => {
				const config = {
...
`,
		...
		(0, globals_1.it)(
			"should respect query category configuration",
			async () => {
				const config = {
					projectName: "test-project",
					rootPath: testDir,
...
`,
				...
				(0, globals_1.expect)(activeQueries).not.toContain(
...
`;
			...
			const result1 = await (0, analysis_js_1.analyzeFile)(
				testCode,
				"typescript",
				"cache-test.ts",
			);
			...
			const result2 = await (0, analysis_js_1.analyzeFile)(
				testCode,
				"typescript",
				"cache-test.ts",
			);
			...
			(0, globals_1.expect)(endTime2 - startTime2).toBeLessThan(
				endTime1 - startTime1,
			);
		});
	});
