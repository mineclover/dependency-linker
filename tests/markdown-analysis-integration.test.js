...
(0, globals_1.describe)("마크다운 분석 통합 테스트", () => {
    ...
    (0, globals_1.beforeAll)(() => {
        ...
        headingExtractor = new MarkdownHeadingExtractor_1.MarkdownHeadingExtractor();
        tagCollector = new MarkdownTagCollector_1.MarkdownTagCollector();
        tagHeadingMapper = new MarkdownTagHeadingMapper_1.MarkdownTagHeadingMapper();
        tagConventionManager = new MarkdownTagConventionManager_1.MarkdownTagConventionManager();
        tagDocumentGenerator = new MarkdownTagDocumentGenerator_1.MarkdownTagDocumentGenerator();
        tagTypeValidator = new MarkdownTagTypeValidator_1.MarkdownTagTypeValidator();
        tagTypeDocumentGenerator = new MarkdownTagTypeDocumentationGenerator_1.MarkdownTagTypeDocumentationGenerator();
    });
    ...
    (0, globals_1.describe)("실제 프로젝트 문서 분석", () => {
        (0, globals_1.it)("README.md 분석", async () => {
            ...
            console.log(`- 링크: ${links.internal.length + links.external.length + links.anchor.length}개`);
...
## 에러 유형 #에러`;
            ...
            for (const tag of tags) {
                const validationResult = await tagTypeValidator.validateSingleTag(tag, markdown);
...
## 요구사항 #요구사항`;
            ...
            const document = await tagTypeDocumentGenerator.generateMarkdownDocumentation(validationResults);
