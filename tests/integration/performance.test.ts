...
describe("Performance Integration Tests", () => {
	...
	describe("Single File Performance", () => {
		test("should parse small files within target time", async () => {
			...
			expect(result.performanceMetrics.parseTime).toBeLessThan(
				PERFORMANCE_TARGETS.PARSE_TIME,
			);
			expect(result.performanceMetrics.totalTime).toBeLessThan(
				PERFORMANCE_TARGETS.PARSE_TIME,
			);
		});
		...
		test("should handle medium-sized files efficiently", async () => {
			...
			expect(result.performanceMetrics.parseTime).toBeLessThan(
				PERFORMANCE_TARGETS.PARSE_TIME * 2,
			);
		});
		...
		test("should maintain memory efficiency", async () => {
			...
			expect(metrics.memoryDelta).toBeLessThan(
				PERFORMANCE_TARGETS.MEMORY_GROWTH_LIMIT,
			);
		});
		...
		test("should provide accurate performance metrics", async () => {
			...
			expect(result.performanceMetrics.extractionTime).toBeGreaterThanOrEqual(
				0,
			);
			expect(
	...
	describe("Batch Processing Performance", () => {
		test("should handle batch analysis efficiently", async () => {
			...
			expect(metrics.duration).toBeLessThan(
				PERFORMANCE_TARGETS.PARSE_TIME * files.length,
			);
		...
		test("should scale linearly with file count", async () => {
			...
			const expectedMultipleTime =
				singleMetrics.duration * multipleFiles.length;
	...
	describe("Cache Performance", () => {
		...
		test("should significantly improve performance with caching", async () => {
			...
			expect(cacheMetrics.duration).toBeLessThanOrEqual(
				noCacheMetrics.duration * 1.2,
			);
		...
		test("should handle cache warming efficiently", async () => {
			...
			expect(warmupResult.averageTimePerFile).toBeLessThan(
				PERFORMANCE_TARGETS.PARSE_TIME,
			);
			expect(metrics.duration).toBeLessThan(
				PERFORMANCE_TARGETS.PARSE_TIME * files.length * 1.2,
			);
		});
	});
	...
	describe("Concurrent Analysis Performance", () => {
		test("should handle concurrent analyses efficiently", async () => {
			...
			const expectedSequentialTime =
				PERFORMANCE_TARGETS.PARSE_TIME * concurrentCount;
	...
	describe("Performance Regression Detection", () => {
		test("should detect performance degradation", async () => {
			...
			expect(variance).toBeLessThan(
				averageTime * PERFORMANCE_TARGETS.VARIANCE_THRESHOLD,
		...
		test("should handle performance under stress", async () => {
			const stressTestFiles = Array.from(
				{ length: 50 },
				(_, i) => "tests/fixtures/sample-typescript.ts",
			);
			...
			expect(engineMetrics.averageAnalysisTime).toBeLessThan(
				PERFORMANCE_TARGETS.PARSE_TIME,
			);
			// Use more lenient memory check - focus on memory growth rather than absolute values
			expect(metrics.memoryDelta).toBeLessThan(
	...
	describe("Memory Leak Detection", () => {
		test("should not leak memory over multiple analyses", async () => {
			...
			expect(memoryGrowth).toBeLessThan(
				PERFORMANCE_TARGETS.CLEANUP_MEMORY_LIMIT,
			);
		});
