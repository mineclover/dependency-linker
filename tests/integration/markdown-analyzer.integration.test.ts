...
async function testMarkdownAnalyzer() {
	...
	for (const file of keyFiles) {
		try {
			...
			console.log(
				`   📊 Lines: ${parseResult.metadata.lineCount}, Chars: ${parseResult.metadata.characterCount}`,
			);
			...
			const images = dependencies.filter(
				(d) => d.type === "image" || d.type === "image_reference",
			);
			...
			console.log(
				`      - Link density: ${analysis.summary.linkDensity.toFixed(3)}`,
			);
			...
			if (analysis.issues.length > 0) {
				...
				if (errorIssues.length > 0) {
					...
					errorIssues.slice(0, 3).forEach((issue) => {
						console.log(
							`         - ${issue.message} (line ${issue.dependency.line})`,
						);
					});
				}
	...
	for (const file of additionalFiles) {
		try {
			...
			console.log(
				`📄 ${file}: ${analysis.summary.totalLinks} links, ${analysis.summary.brokenLinks} broken, ${analysis.issues.length} issues`,
			);
			...
			console.log(
				`❌ ${file}: Failed - ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
	...
	console.log(`\n📈 Overall Statistics:`);
	console.log(
		`   Files analyzed: ${successfulAnalyses.length}/${results.length}`,
	);
	console.log(`   Total links found: ${totalLinks}`);
	console.log(`   Broken links: ${totalBrokenLinks}`);
	console.log(`   Total issues: ${totalIssues}`);
	console.log(
		`   Success rate: ${((successfulAnalyses.length / results.length) * 100).toFixed(1)}%`,
	);
	console.log(
		`   Link health: ${totalLinks > 0 ? (((totalLinks - totalBrokenLinks) / totalLinks) * 100).toFixed(1) : 100}%`,
	);

	if (successfulAnalyses.length > 0) {
		const avgParseTime =
			successfulAnalyses.reduce((sum, r) => sum + (r.parseTime || 0), 0) /
	...
	keyFiles.forEach((file) => {
		...
		if (result && result.success) {
			const status =
				result.brokenLinks === 0 && result.issues === 0
					? "✅"
					: result.brokenLinks > 0
						? "🔴"
						: "🟡";
			console.log(
				`   ${status} ${file}: ${result.totalLinks} links, ${result.issues} issues`,
			);
		}
	});
