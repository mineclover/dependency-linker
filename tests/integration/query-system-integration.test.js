...
(0, globals_1.describe)("Query System Integration", () => {
	...
	(0, globals_1.describe)("Handler Factory Integration", () => {
		(0, globals_1.it)(
			"should initialize query handler through factory",
			async () => {
				(0, globals_1.expect)(queryHandler).toBeDefined();
				(0, globals_1.expect)(queryHandler).toBeInstanceOf(
					query_handler_1.QueryHandler,
				);
			},
		);
		(0, globals_1.it)(
			"should provide singleton query handler instances",
			async () => {
	...
	(0, globals_1.describe)("Query Execution", () => {
		(0, globals_1.it)("should execute SQL query", async () => {
			...
			try {
				...
				(0, globals_1.expect)(consoleSpy).toHaveBeenCalledWith(
					globals_1.expect.stringContaining("SQL 쿼리 실행"),
				);
				(0, globals_1.expect)(consoleSpy).toHaveBeenCalledWith(
					globals_1.expect.stringContaining("SQL 쿼리 완료"),
				);
			} finally {
		...
		(0, globals_1.it)("should execute GraphQL query", async () => {
			...
			try {
				...
				(0, globals_1.expect)(consoleSpy).toHaveBeenCalledWith(
					globals_1.expect.stringContaining("GraphQL 쿼리 실행"),
				);
				(0, globals_1.expect)(consoleSpy).toHaveBeenCalledWith(
					globals_1.expect.stringContaining("GraphQL 쿼리 완료"),
				);
			} finally {
				consoleSpy.mockRestore();
			}
		...
		(0, globals_1.it)("should execute natural language query", async () => {
			...
			try {
				...
				(0, globals_1.expect)(consoleSpy).toHaveBeenCalledWith(
					globals_1.expect.stringContaining("자연어 쿼리 실행"),
				);
				(0, globals_1.expect)(consoleSpy).toHaveBeenCalledWith(
					globals_1.expect.stringContaining("자연어 쿼리 완료"),
				);
			} finally {
				consoleSpy.mockRestore();
			}
		...
		(0, globals_1.it)("should execute auto-detected query", async () => {
			...
			try {
				...
				(0, globals_1.expect)(consoleSpy).toHaveBeenCalledWith(
					globals_1.expect.stringContaining("쿼리 실행 (자동 감지)"),
				);
				(0, globals_1.expect)(consoleSpy).toHaveBeenCalledWith(
					globals_1.expect.stringContaining("쿼리 완료"),
				);
			} finally {
				consoleSpy.mockRestore();
			}
	...
	(0, globals_1.describe)("Realtime Query System", () => {
		(0, globals_1.it)("should register realtime query", async () => {
			...
			try {
				...
				(0, globals_1.expect)(consoleSpy).toHaveBeenCalledWith(
					globals_1.expect.stringContaining("실시간 쿼리 등록"),
				);
				(0, globals_1.expect)(consoleSpy).toHaveBeenCalledWith(
					globals_1.expect.stringContaining("실시간 쿼리 등록 완료"),
				);
			} finally {
				consoleSpy.mockRestore();
			}
		...
		(0, globals_1.it)("should subscribe to realtime query", async () => {
			...
			try {
				...
				(0, globals_1.expect)(consoleSpy).toHaveBeenCalledWith(
					globals_1.expect.stringContaining("실시간 쿼리 구독"),
				);
				(0, globals_1.expect)(consoleSpy).toHaveBeenCalledWith(
					globals_1.expect.stringContaining("실시간 쿼리 구독 완료"),
				);
			} finally {
				consoleSpy.mockRestore();
			}
	...
	(0, globals_1.describe)("Query Statistics and Cache Management", () => {
		(0, globals_1.it)("should get query statistics", async () => {
			...
			try {
				...
				(0, globals_1.expect)(consoleSpy).toHaveBeenCalledWith(
					globals_1.expect.stringContaining("쿼리 성능 통계"),
				);
			} finally {
				consoleSpy.mockRestore();
			}
		...
		(0, globals_1.it)("should manage cache", async () => {
			...
			try {
				...
				(0, globals_1.expect)(consoleSpy).toHaveBeenCalledWith(
					globals_1.expect.stringContaining("쿼리 캐시 통계"),
				);
				await queryHandler.manageCache("clear");
				(0, globals_1.expect)(consoleSpy).toHaveBeenCalledWith(
					globals_1.expect.stringContaining("쿼리 캐시 초기화 완료"),
				);
				await queryHandler.manageCache("optimize");
				(0, globals_1.expect)(consoleSpy).toHaveBeenCalledWith(
					globals_1.expect.stringContaining("쿼리 캐시 최적화 완료"),
				);
			} finally {
				consoleSpy.mockRestore();
			}
	...
	(0, globals_1.describe)("Error Handling", () => {
		(0, globals_1.it)(
			...
			async () => {
				...
				try {
					...
					(0, globals_1.expect)(consoleSpy).toHaveBeenCalledWith(
						globals_1.expect.stringContaining("SQL 쿼리 실행 실패"),
					);
				} finally {
					consoleSpy.mockRestore();
				}
		...
		(0, globals_1.it)(
			"should handle realtime query errors gracefully",
			async () => {
				const consoleSpy = jest.spyOn(console, "error").mockImplementation();
				try {
					...
					(0, globals_1.expect)(consoleSpy).toHaveBeenCalledWith(
						globals_1.expect.stringContaining("실시간 쿼리 구독 실패"),
					);
				} finally {
					consoleSpy.mockRestore();
				}
	...
	(0, globals_1.describe)("Performance and Scalability", () => {
		...
		(0, globals_1.it)("should handle concurrent realtime queries", async () => {
			...
			(0, globals_1.expect)(
				queryIds.every((id) => id && typeof id === "string"),
			).toBe(true);
		});
	});
});
