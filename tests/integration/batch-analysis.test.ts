...
const LARGE_BATCH_FILES = Array.from(
	{ length: 20 },
	(_, i) => `tests/fixtures/sample-typescript.ts`, // Reusing same file for consistent testing
);
...
describe("Batch Analysis Integration", () => {
	...
	describe("Batch Analysis Reporting", () => {
		...
		test("should aggregate batch results effectively", async () => {
			...
			const allDependencies = results.flatMap(
				(r) => r.extractedData.dependency?.dependencies || [],
			);

			const allIdentifiers = results.flatMap(
				(r) => r.extractedData.identifier?.identifiers || [],
			);
			...
			const totalAnalysisTime = results.reduce(
				(sum, r) => sum + r.performanceMetrics.totalTime,
				0,
			);
	...
	describe("Advanced Batch Scenarios", () => {
		test("should handle mixed file types in batch", async () => {
			...
			// Group by language
			const byLanguage = results.reduce(
				(groups, result) => {
			...
			for (const result of results) {
				...
				expect(result.performanceMetrics.totalTime).toBeGreaterThanOrEqual(
					result.performanceMetrics.parseTime +
						...
						1, // Allow for 1ms rounding error
				);
