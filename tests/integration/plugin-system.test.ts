...
describe("Plugin System Integration", () => {
	...
	describe("Plugin Execution", () => {
		...
		test("should execute custom extractor during analysis", async () => {
			...
			expect(result.extractedData["custom-test"].customData).toContain(
				"tests/fixtures/sample-typescript.ts",
			);
			expect(typeof result.extractedData["custom-test"].nodeCount).toBe(
				"number",
			);
		});
		...
		test("should execute custom interpreter during analysis", async () => {
			...
			expect(
				result.interpretedData["custom-test"].contextSummary,
			).toBeDefined();
			expect(
				result.interpretedData["custom-test"].contextSummary.filePath,
			).toBe("tests/fixtures/sample-typescript.ts");
			expect(
				result.interpretedData["custom-test"].contextSummary.language,
			).toBe("typescript");
		});
		...
		test("should execute both built-in and custom plugins together", async () => {
			...
			expect(
				result.interpretedData["custom-test"].analysis.hasBuiltinData,
			).toBe(true);
			expect(result.interpretedData["custom-test"].analysis.hasCustomData).toBe(
				true,
			);
		});
	});
	...
	describe("Plugin Interaction", () => {
		...
		test("should handle cross-plugin data dependencies", async () => {
			...
			expect(
				typeof result.interpretedData.dependent.combinedAnalysis
					.dependencyCount,
			).toBe("number");
			expect(
				typeof result.interpretedData.dependent.combinedAnalysis
	...
	describe("Plugin Lifecycle Management", () => {
		test("should unregister plugins successfully", () => {
			...
			expect(engine.getRegisteredInterpreters().has("temp-interpreter")).toBe(
				true,
			);

			const extractorUnregistered =
				engine.unregisterExtractor("temp-extractor");
			const interpreterUnregistered =
				engine.unregisterInterpreter("temp-interpreter");

			expect(extractorUnregistered).toBe(true);
			expect(interpreterUnregistered).toBe(true);
			expect(engine.getRegisteredExtractors().has("temp-extractor")).toBe(
				false,
			);
			expect(engine.getRegisteredInterpreters().has("temp-interpreter")).toBe(
				false,
		...
		test("should maintain plugin state after cache operations", async () => {
			...
			expect(engine.getRegisteredExtractors().has("persistent-extractor")).toBe(
				true,
			);
