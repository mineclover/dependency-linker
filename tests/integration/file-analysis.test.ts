...
`,
			...
			const externalDeps = result.dependencies.filter(
				(d: DependencyInfo) => d.type === "external",
			);
			const relativeDeps = result.dependencies.filter(
			...
			const typeImport = result.imports.find(
				(i: ImportInfo) => i.source === "react" && i.isTypeOnly === true,
			);
			expect(typeImport?.specifiers).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						imported: "ComponentProps",
						type: "named",
					}),
				]),
			);
