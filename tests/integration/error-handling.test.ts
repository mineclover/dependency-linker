...
describe('Error Handling Integration', () => {
  ...
  afterAll(async () => {
    ...
    const tempFiles = [
      'invalid.txt', 'large-test.ts', 'syntax-errors.ts', 'completely-invalid.ts',
      'unicode.ts', 'timeout-test.ts', 'potential-loop.ts', 'memory-test.ts',
      'bad-config.ts', 'partial-parse.ts'
    ];
  ...
  describe('File system errors', () => {
    ...
    test('should handle permission denied', async () => {
      ...
      await fs.promises.writeFile(restrictedFile, 'export const test = "restricted";');
      ...
      try {
        ...
        if (canStillRead) {
          ...
          expect(result.success).toBe(false);
          expect(result.error?.code).toMatch(/FILE_NOT_FOUND|PERMISSION_DENIED/);
        }
      } finally {
  ...
  describe('Parse errors', () => {
    test('should handle syntax errors gracefully', async () => {
      ...
      await fs.promises.writeFile(syntaxErrorFile, `
...
`);
    ...
    test('should handle completely invalid TypeScript', async () => {
      ...
      await fs.promises.writeFile(invalidTsFile, `
...
`);
    ...
    test('should handle Unicode and special characters', async () => {
      ...
      await fs.promises.writeFile(unicodeFile, `
...
`, 'utf-8');
  ...
  describe('Timeout handling', () => {
    ...
    test('should handle infinite loop prevention', async () => {
      ...
      await fs.promises.writeFile(loopFile, `
