...
import { PathResolverInterpreter, type PathResolutionResult } from "../../src/interpreters/PathResolverInterpreter";
...
describe("PathResolver Multi-Format Integration Tests", () => {
	...
	const createDependencyData = (dependencies: Array<{ source: string }>): DependencyExtractionResult => ({
		dependencies: dependencies.map((dep, index) => ({
			source: dep.source,
			specifiers: [],
			type: "import",
			isTypeOnly: false,
			location: { line: index + 1, column: 0, endLine: index + 1, endColumn: dep.source.length + 10 }
		})),
	...
	describe("TypeScript (.ts) files", () => {
		it("should resolve paths in TypeScript files", () => {
			...
			// Relative path
			expect(result.resolvedDependencies[0].originalSource).toBe("./types/User");
			expect(result.resolvedDependencies[0].resolutionType).toBe("relative");
			expect(result.resolvedDependencies[0].resolvedPath).toBe("/Users/test/project/src/services/types/User.ts");

			// Parent relative path
			expect(result.resolvedDependencies[1].originalSource).toBe("../utils/api");
			expect(result.resolvedDependencies[1].resolutionType).toBe("relative");
			expect(result.resolvedDependencies[1].resolvedPath).toBe("/Users/test/project/src/utils/api.ts");

			// Alias path
			expect(result.resolvedDependencies[2].originalSource).toBe("@/components/UserCard");
			expect(result.resolvedDependencies[2].resolutionType).toBe("alias");
			expect(result.resolvedDependencies[2].resolvedPath).toBe("/Users/test/project/src/components/UserCard.ts");

			// Node module
			expect(result.resolvedDependencies[3].originalSource).toBe("axios");
			expect(result.resolvedDependencies[3].resolutionType).toBe("node_modules");
		});
	});
	...
	describe("TypeScript React (.tsx) files", () => {
		it("should resolve paths in TSX files", () => {
			...
			// TSX component
			expect(result.resolvedDependencies[0].resolvedPath).toBe("/Users/test/project/src/components/UserCard.tsx");
			expect(result.resolvedDependencies[0].extension).toBe(".tsx");

			// Hook file
			expect(result.resolvedDependencies[1].resolvedPath).toBe("/Users/test/project/src/hooks/useUser.ts");
			expect(result.resolvedDependencies[1].extension).toBe(".ts");

			// Type alias
			expect(result.resolvedDependencies[2].resolutionType).toBe("alias");
			expect(result.resolvedDependencies[2].resolvedPath).toBe("/Users/test/project/src/types/User.ts");
		});
	});
	...
	describe("JavaScript (.js) files", () => {
		it("should resolve paths in JavaScript files", () => {
			...
			// JavaScript files
			expect(result.resolvedDependencies[0].resolvedPath).toBe("/Users/test/project/src/utils/validation.js");
			expect(result.resolvedDependencies[1].resolvedPath).toBe("/Users/test/project/src/config/constants.js");

			// Node module
			expect(result.resolvedDependencies[2].resolutionType).toBe("node_modules");
	...
	describe("JavaScript React (.jsx) files", () => {
		it("should resolve paths in JSX files", () => {
			...
			// JSX components
			expect(result.resolvedDependencies[0].resolvedPath).toBe("/Users/test/project/src/components/Header.jsx");
			expect(result.resolvedDependencies[0].extension).toBe(".jsx");

			expect(result.resolvedDependencies[1].resolvedPath).toBe("/Users/test/project/src/components/Footer.jsx");
			expect(result.resolvedDependencies[2].resolvedPath).toBe("/Users/test/project/src/pages/Home.jsx");
		});
	});
	...
	describe("Markdown (.md) files", () => {
		it("should handle imports in Markdown files (MDX-style)", () => {
			...
			// Markdown file
			expect(result.resolvedDependencies[0].resolvedPath).toBe("/Users/test/project/docs/api/UserAPI.md");
			expect(result.resolvedDependencies[0].extension).toBe(".md");

			// JavaScript example
			expect(result.resolvedDependencies[1].resolvedPath).toBe("/Users/test/project/docs/examples/user-example.js");
	...
	describe("Mixed file extensions", () => {
		it("should resolve mixed extensions with proper priority", () => {
			...
			expect(result.resolvedDependencies[0].resolvedPath).toBe("/Users/test/project/src/components/Button.tsx");
			expect(result.resolvedDependencies[0].extension).toBe(".tsx");

			// Should find .ts
			expect(result.resolvedDependencies[1].resolvedPath).toBe("/Users/test/project/src/utils/helpers.ts");
			expect(result.resolvedDependencies[1].extension).toBe(".ts");
	...
	describe("Real-world complex scenario", () => {
		it("should handle a complex project structure with all file types", () => {
			const sourceFile = "/Users/test/project/src/features/user/UserDashboard.tsx";
			const context = createContext(sourceFile, "typescript");
			...
			expect(Object.keys(result.pathMappings)).toEqual(["@", "@components", "@hooks"]);
		});
	});
});