...
describe("Optimization Execution Integration", () => {
	...
	describe("End-to-End Optimization Workflow", () => {
		it("should execute complete optimization workflow", async () => {
			...
			const optimizationResult = await testOptimizer.optimizeTestSuite(
				testSuite,
				{
			...
			const totalOriginalDuration = testSuite.testCases.reduce(
				(sum, tc) => sum + tc.estimatedDuration,
				0,
			);
			const totalOptimizedDuration =
				optimizationResult.optimizedSuite.testCases.reduce(
					(sum, tc) => sum + tc.estimatedDuration,
					0,
				);
		...
		it("should handle optimization with constraints", async () => {
			...
			expect(
				optimizationResult.optimizedSuite.testCases[0].constraints,
			).toContain("no-parallel");
			expect(
				optimizationResult.improvements.some(
					(i) =>
		...
		it("should generate comprehensive optimization report", async () => {
			...
			const optimizationResult =
				await testOptimizer.optimizeTestSuite(testSuite);
			const report =
				await testOptimizer.generateOptimizationReport(optimizationResult);
			...
			expect(report.improvements).toHaveLength(
		...
		it("should handle optimization failures gracefully", async () => {
			...
			await expect(
				testOptimizer.optimizeTestSuite(invalidTestSuite),
	...
	describe("Real-world Optimization Scenarios", () => {
		it("should optimize typical Jest test suite structure", async () => {
			...
			expect(
				result.improvements.some((i) => i.type === "setup-optimization"),
			).toBe(true);
			expect(result.optimizedSuite.testCases.length).toBeLessThanOrEqual(
				jestSuite.testCases.length,
			);

			const totalOriginal = jestSuite.testCases.reduce(
				(sum, tc) => sum + tc.estimatedDuration,
				0,
			);
			const totalOptimized = result.optimizedSuite.testCases.reduce(
				(sum, tc) => sum + tc.estimatedDuration,
				0,
			);
