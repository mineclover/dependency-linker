...
describe('Dependency Analysis Compatibility', () => {
  ...
  describe('Legacy API Compatibility', () => {
    test('should maintain compatibility with existing TypeScriptAnalyzer API', async () => {
      ...
      const dependencies = legacyResult.extractedData?.dependency || legacyResult.interpretedData?.['dependency-analysis'];
    ...
    test('should provide backward-compatible dependency format', async () => {
      ...
      const legacyDependencies = legacyResult.extractedData?.dependency || legacyResult.interpretedData?.['dependency-analysis'];
      ...
      if (newDependencies && legacyDependencies) {
        const legacyDepsArray = Array.isArray(legacyDependencies) ? legacyDependencies : legacyDependencies.imports || [];
        const newDepsArray = Array.isArray(newDependencies) ? newDependencies : newDependencies.imports || [];
        ...
        expect(newDep.source || newDep.from || newDep.specifier).toBeDefined();
    ...
    test('should handle batch processing compatibility', async () => {
      ...
      const legacyResults = await Promise.all(files.map(f => legacyAnalyzer.analyzeFile(f)));
  ...
  describe('Migration Path Validation', () => {
    ...
    test('should maintain consistent dependency detection across versions', async () => {
      ...
      const legacyDeps = legacyResult.extractedData?.dependency || legacyResult.interpretedData?.['dependency-analysis'];
      const legacyDepCount = Array.isArray(legacyDeps) ? legacyDeps.length : (legacyDeps?.imports?.length || 0);
      const newDeps = newResult.extractedData.dependency;
      const newDepCount = Array.isArray(newDeps) ? newDeps.length : (newDeps?.imports?.length || 0);
  ...
  describe('Error Handling Compatibility', () => {
    test('should handle invalid files consistently with legacy behavior', async () => {
      ...
      const legacyHasErrors = legacyResult.errors && legacyResult.errors.length > 0;
    ...
    test('should support legacy configuration options', async () => {
      ...
      const newConfig: AnalysisConfig = {
        ...
        extractorOptions: {
          dependency: {
            options: {
              includeExternalDependencies: legacyConfig.includeExternalDependencies,
  ...
  describe('Extension Compatibility', () => {
    test('should support existing dependency analysis extensions', async () => {
      ...
      class LegacyCompatExtractor implements IDataExtractor<any> {
        ...
        getName(): string { return 'legacy-compat'; }
