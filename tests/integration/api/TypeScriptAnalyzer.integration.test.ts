...
export class TestComponent extends Component {
  async fetchData() {
    ...
    it('should handle error cases gracefully', async () => {
      // Non-existent file
      await expect(analyzer.analyzeFile('/nonexistent/file.ts'))
        .rejects.toThrow(FileNotFoundError);
      
      // Invalid file type
      const jsFile = path.join(tempDir, 'test.js');
      fs.writeFileSync(jsFile, 'const x = 42;');
      await expect(analyzer.analyzeFile(jsFile))
  ...
  describe('Configuration and Options Integration', () => {
    ...
    it('should handle different log levels', async () => {
      ...
      const result = await verboseAnalyzer.analyzeFile(path.join(tempDir, 'simple.ts'));
      expect(result.success).toBe(true);
    ...
    it('should support custom dependency injection', async () => {
      ...
      const result = await customAnalyzer.analyzeFile(path.join(tempDir, 'simple.ts'));
      expect(result.success).toBe(true);
  ...
  describe('Event System Integration', () => {
    it('should emit events during analysis operations', async () => {
      ...
      analyzer.on('analysisStart' as any, (data) => events.push({ type: 'start', data }));
      analyzer.on('analysisComplete' as any, (data) => events.push({ type: 'complete', data }));
    ...
    it('should emit batch events during batch operations', async () => {
      ...
      analyzer.on('batchStart' as any, (data) => events.push({ type: 'batchStart', data }));
      analyzer.on('batchProgress' as any, (data) => events.push({ type: 'batchProgress', data }));
      analyzer.on('batchComplete' as any, (data) => events.push({ type: 'batchComplete', data }));
...
export default App;
      ...
      // Header component
      fs.writeFileSync(path.join(componentsDir, 'Header.tsx'), `
import React from 'react';
...
export const Header: React.FC = () => {
  return <h1>{formatTitle('My App')}</h1>;
      ...
      // Utils
      fs.writeFileSync(path.join(utilsDir, 'text.ts'), `
...
export const slugify = (text: string): string => {
  return text.toLowerCase().replace(/\\s+/g, '-');
      ...
      expect(appResult?.dependencies.some(d => d.source === 'react')).toBe(true);
      expect(appResult?.dependencies.some(d => d.source.includes('./components/Header'))).toBe(true);
      
      const headerResult = results.find(r => r.filePath.includes('Header.tsx'));
      expect(headerResult?.success).toBe(true);
      expect(headerResult?.dependencies.some(d => d.source.includes('../utils/text'))).toBe(true);
    });
...
export class UserService {
  async getUser(id: string): Promise<User> {
    return { id, name: 'Test User' };
      ...
      fs.writeFileSync(path.join(uiDir, 'UserDisplay.tsx'), `
import React from 'react';
...
export const UserDisplay: React.FC<Props> = ({ userId }) => {
  ...
  return user ? <div>{user.name}</div> : <div>Loading...</div>;
      ...
      expect(uiResult?.dependencies.some(d => d.source === 'react')).toBe(true);
      expect(uiResult?.dependencies.some(d => d.source === '@myapp/core')).toBe(true);
    });
  });
});