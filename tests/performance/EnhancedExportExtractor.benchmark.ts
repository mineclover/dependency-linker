...
class ExportExtractorBenchmark {
  ...
  generateTestCases(): Record<string, string> {
    return {
      ...
      'EnhancedExportExtractor.ts (1,272 lines)': this.loadRealPackageFile('src/extractors/EnhancedExportExtractor.ts'),
      'IDataExtractor.ts (822 lines)': this.loadRealPackageFile('src/extractors/IDataExtractor.ts'),
      'EnhancedDependencyExtractor.ts (503 lines)': this.loadRealPackageFile('src/extractors/EnhancedDependencyExtractor.ts')
    };
  }
...
export class Class${i} {
  ...
  public static staticMethod${i}(): void {}
    ...
    const interfaces = Array.from({ length: 10 }, (_, i) =>
      `export interface Interface${i} { prop${i}: string; }`
    );
  ...
  private generateLargeFile(): string {
    const functions = Array.from({ length: 80 }, (_, i) =>
      `export function func${i}(param${i}: number): number { return param${i} * ${i}; }`
    );

    const classes = Array.from({ length: 40 }, (_, i) => `
export class Class${i} {
  private value${i} = ${i};
  ...
  protected protectedProp${i} = '${i}';
    ...
    const interfaces = Array.from({ length: 40 }, (_, i) =>
      `export interface Interface${i} { prop${i}: string; method${i}(): void; }`
    );

    const variables = Array.from({ length: 40 }, (_, i) =>
      `export const CONST_${i} = { value: ${i}, name: 'const${i}' };`
    );
...
export default ApiClient;
  ...
  async runBenchmarkSuite(): Promise<Record<string, PerformanceMetrics>> {
    ...
    for (const [name, sourceCode] of Object.entries(testCases)) {
      ...
      try {
        ...
        console.log(`   ✅ 완료: ${metrics.executionTime.toFixed(2)}ms, ${metrics.exportCount} exports`);
        console.log(`       메모리: ${(metrics.memoryUsage / 1024).toFixed(2)}KB`);
      } catch (error) {
  ...
  generateReport(results: Record<string, PerformanceMetrics>): string {
    ...
    report.push(`## 📊 테스트 결과\n`);
    report.push('| 테스트 케이스 | 실행 시간 | 메모리 사용량 | Export 수 | 처리율 (exports/ms) |');
    report.push('|--------------|-----------|---------------|-----------|-------------------|');

    for (const [name, metrics] of Object.entries(results)) {
      const throughput = (metrics.exportCount / metrics.executionTime).toFixed(2);
      report.push(
    ...
    const avgTime = Object.values(results)
      .reduce((sum, m) => sum + m.executionTime, 0) / Object.keys(results).length;
    const avgMemory = Object.values(results)
      .reduce((sum, m) => sum + m.memoryUsage, 0) / Object.keys(results).length;
    ...
    report.push(`- **가장 빠른 케이스**: ${fastest[0]} (${fastest[1].executionTime.toFixed(2)}ms)`);
    report.push(`- **가장 느린 케이스**: ${slowest[0]} (${slowest[1].executionTime.toFixed(2)}ms)`);

    report.push(`\n## 📅 테스트 환경`);
    report.push(`- **Node.js 버전**: ${process.version}`);
    report.push(`- **플랫폼**: ${process.platform} ${process.arch}`);
    report.push(`- **메모리**: ${Math.round(process.memoryUsage().rss / 1024 / 1024)}MB`);
...
if (require.main === module) {
  ...
  benchmark.runBenchmarkSuite().then(results => {
    ...
    const reportPath = path.join(__dirname, '../../.performance-data/export-extractor-benchmark.md');
