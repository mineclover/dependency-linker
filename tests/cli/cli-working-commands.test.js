...
`;
    ...
    (0, globals_1.describe)("CLI 도움말 명령어", () => {
        (0, globals_1.it)("메인 도움말이 정상적으로 작동해야 함", async () => {
            const { stdout, stderr } = await execAsync(`npm run cli -- --help`);
            (0, globals_1.expect)(stdout).toContain("Advanced dependency analysis tool");
            (0, globals_1.expect)(stdout).toContain("Commands:");
        }, 10000);
        (0, globals_1.it)("analyze 도움말이 정상적으로 작동해야 함", async () => {
            const { stdout, stderr } = await execAsync(`npm run cli -- analyze --help`);
            (0, globals_1.expect)(stdout).toContain("Analyze files for dependencies");
            (0, globals_1.expect)(stdout).toContain("Options:");
        }, 10000);
        (0, globals_1.it)("rdf 도움말이 정상적으로 작동해야 함", async () => {
            const { stdout, stderr } = await execAsync(`npm run cli -- rdf --help`);
            (0, globals_1.expect)(stdout).toContain("RDF operations");
            (0, globals_1.expect)(stdout).toContain("Options:");
        }, 10000);
        (0, globals_1.it)("rdf-file 도움말이 정상적으로 작동해야 함", async () => {
            const { stdout, stderr } = await execAsync(`npm run cli -- rdf-file --help`);
            (0, globals_1.expect)(stdout).toContain("RDF 주소 기반 파일 위치 반환 및 파일 열기");
            (0, globals_1.expect)(stdout).toContain("Options:");
        }, 10000);
    });
    (0, globals_1.describe)("CLI 버전 명령어", () => {
        (0, globals_1.it)("버전 정보가 정상적으로 작동해야 함", async () => {
