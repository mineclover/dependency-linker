...
export class TestDatabaseManager {
  ...
  async getStatus(): Promise<{
    ...
    if (exists && this.testDb) {
      try {
        const nodes = await this.testDb.executeQuery('SELECT COUNT(*) as count FROM nodes');
        nodeCount = nodes[0]?.count || 0;

        const relationships = await this.testDb.executeQuery('SELECT COUNT(*) as count FROM relationships');
        relationshipCount = relationships[0]?.count || 0;

        const files = await this.testDb.executeQuery('SELECT COUNT(*) as count FROM files');
        fileCount = files[0]?.count || 0;

        const projects = await this.testDb.executeQuery('SELECT COUNT(*) as count FROM projects');
  ...
  async getStatistics(): Promise<{
    ...
    try {
      // 기본 통계
      const nodeCount = await this.testDb.executeQuery('SELECT COUNT(*) as count FROM nodes');
      stats.totalNodes = nodeCount[0]?.count || 0;

      const relationshipCount = await this.testDb.executeQuery('SELECT COUNT(*) as count FROM relationships');
      stats.totalRelationships = relationshipCount[0]?.count || 0;

      const fileCount = await this.testDb.executeQuery('SELECT COUNT(*) as count FROM files');
      stats.totalFiles = fileCount[0]?.count || 0;

      const projectCount = await this.testDb.executeQuery('SELECT COUNT(*) as count FROM projects');
      stats.totalProjects = projectCount[0]?.count || 0;

      // 노드 타입별 통계
      const nodeTypes = await this.testDb.executeQuery('SELECT type, COUNT(*) as count FROM nodes GROUP BY type');
      for (const row of nodeTypes) {
        stats.nodeTypes[row.type] = row.count;
      }

      // 관계 타입별 통계
      const relationshipTypes = await this.testDb.executeQuery('SELECT type, COUNT(*) as count FROM relationships GROUP BY type');
      for (const row of relationshipTypes) {
        stats.relationshipTypes[row.type] = row.count;
      }

      // 언어별 통계
      const languages = await this.testDb.executeQuery('SELECT language, COUNT(*) as count FROM nodes GROUP BY language');
