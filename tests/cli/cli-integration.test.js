...
`;
		...
		(0, globals_1.it)(
			"JavaScript 파일 분석이 정상적으로 작동해야 함",
			async () => {
...
`;
	...
	(0, globals_1.describe)("unknown 명령어", () => {
		(0, globals_1.it)(
			"Unknown Symbol 등록이 정상적으로 작동해야 함",
			async () => {
				const testFile = path.join(TEST_PROJECT_ROOT, "UserService.ts");
				const testContent = `export class UserService {}`;
				fs.writeFileSync(testFile, testContent);
		...
		(0, globals_1.it)(
			"Unknown Symbol 검색이 정상적으로 작동해야 함",
			async () => {
				const { stdout, stderr } = await execAsync(
	...
	(0, globals_1.describe)("cross-namespace 명령어", () => {
		(0, globals_1.it)(
			...
			async () => {
				...
				(0, globals_1.expect)(stderr).toBe("");
				(0, globals_1.expect)(stdout).toContain(
					"Cross-namespace analysis completed",
				);
			},
		);
	...
	(0, globals_1.describe)("inference 명령어", () => {
		(0, globals_1.it)("계층적 추론이 정상적으로 작동해야 함", async () => {
			...
			(0, globals_1.expect)(stderr).toBe("");
			(0, globals_1.expect)(stdout).toContain(
				"Hierarchical inference completed",
			);
		});
	...
	(0, globals_1.describe)("context-documents 명령어", () => {
		...
		(0, globals_1.it)(
			...
			async () => {
				...
				(0, globals_1.expect)(stderr).toBe("");
				(0, globals_1.expect)(stdout).toContain(
					"Symbol context document generated",
				);
			},
		);
		(0, globals_1.it)(
			"프로젝트 컨텍스트 문서 생성이 정상적으로 작동해야 함",
			async () => {
				const { stdout, stderr } = await execAsync(
				...
				(0, globals_1.expect)(stderr).toBe("");
				(0, globals_1.expect)(stdout).toContain(
					"Project context document generated",
				);
			},
		);
	});
	...
	(0, globals_1.describe)("typescript 명령어", () => {
		...
		(0, globals_1.it)(
			"TypeScript 프로젝트 분석이 정상적으로 작동해야 함",
			async () => {
				const { stdout, stderr } = await execAsync(
