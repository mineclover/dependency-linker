...
describe("FileAnalyzer", () => {
	...
	beforeEach(() => {
		...
		(DependencyAnalyzer as jest.Mock).mockImplementation(
			() => mockDependencyAnalyzer,
		);
	...
	describe("analyzeFile", () => {
		...
		test("should successfully analyze a valid TypeScript file", async () => {
			...
			mockDependencyAnalyzer.classifyDependencies.mockResolvedValue(
				classifiedDeps as any,
			);
		...
		test("should handle parser failure", async () => {
			...
			expect(
				mockDependencyAnalyzer.classifyDependencies,
	...
	describe("analyzeFiles", () => {
		...
		test("should analyze files with custom concurrency", async () => {
			const requests: FileAnalysisRequest[] = Array.from(
				{ length: 10 },
				(_, i) => ({
	...
	describe("validateFile", () => {
		...
		test("should reject non-TypeScript files", async () => {
			...
			expect(validation.errors).toContain(
				"File must be a TypeScript file (.ts or .tsx)",
			);
		});
	...
	describe("generateDependencyReport", () => {
		test("should generate report for successful analysis", () => {
			...
			expect(mockDependencyAnalyzer.generateReport).toHaveBeenCalledWith(
				result.dependencies,
			);
		});
