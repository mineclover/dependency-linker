...
describe("TypeScriptParser", () => {
	...
	beforeEach(() => {
		...
		(TypeScriptParserEnhanced as jest.Mock).mockImplementation(
			() => mockEnhancedParser,
		);
	...
	describe("parseFile", () => {
		...
		test("should handle partial parsing success with errors", async () => {
			...
			const result = await parser.parseFile(
				"/test/file.ts",
				'import lodash from "lodash"; invalid syntax',
			);
			...
			const mockParseResult = {
				...
				imports: [
					...
					},
					{
						source: "./types",
						specifiers: [
							{ imported: "User", local: "User", type: "named" as const },
						],
						isTypeOnly: true,
						location: { line: 2, column: 0, offset: 0 },
					},
				...
				exports: [
					{
						...
						type: "named" as const,
						isTypeOnly: false,
						location: { line: 10, column: 0, offset: 0 },
					},
					{
						name: "default",
						type: "default" as const,
						isTypeOnly: false,
						location: { line: 15, column: 0, offset: 0 },
					},
				],
				hasParseErrors: false,
			};
