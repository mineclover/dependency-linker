...
describe('CacheManager', () => {
  ...
  describe('AST Caching', () => {
        ...
        body: [
          {
            ...
            declarations: [{ type: 'VariableDeclarator', id: { name: 'x' }, init: { value: 1 } }]
          }
        ]
      };
    ...
    test('should check if AST exists in cache', async () => {
      ...
      expect(await cacheManager.has(`ast:${filePath}:${contentHash}`)).toBe(false);

      await cacheManager.setAST(filePath, ast, contentHash);
      expect(await cacheManager.has(`ast:${filePath}:${contentHash}`)).toBe(true);
    });
  });
  ...
  describe('Configuration', () => {
    test('should respect maxSize configuration', async () => {
      ...
      const hasAnyKey = await limitedCache.has('key4') ||
  ...
  describe('Error Handling', () => {
    test('should handle invalid cache operations gracefully', async () => {
      ...
      await expect(cacheManager.set('null-test', null as any)).resolves.not.toThrow();

      // Test empty key
      await expect(cacheManager.set('', { data: 'test' })).resolves.not.toThrow();
