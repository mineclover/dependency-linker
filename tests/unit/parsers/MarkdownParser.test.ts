...
> blockquote
			...
			expect(parser.detectLanguage("unknown.txt", markdownContent)).toBe(
				"markdown",
			);
		});
...
And here is a [relative link](./docs/api.md).
			...
			const relativeLink = result.metadata.links.find(
				(l) => l.url === "./docs/api.md",
			);
...
![External image](https://example.com/image.jpg "Title")
			...
			const localImage = result.metadata.links.find(
				(l) => l.url === "./images/logo.png",
			);
			expect(localImage).toBeDefined();
			expect(localImage?.type).toBe(LinkType.IMAGE);
			expect(localImage?.alt).toBe("Local image");

			const externalImage = result.metadata.links.find(
				(l) => l.url === "https://example.com/image.jpg",
			);
