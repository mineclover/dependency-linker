...
import { LinkDependencyInterpreter, DependencyCategory, LinkStatus, IssueType, IssueSeverity } from '../../../src/interpreters/LinkDependencyInterpreter';
...
describe('LinkDependencyInterpreter', () => {
	...
	const createMockDependency = (overrides: Partial<MarkdownLinkDependency> = {}): MarkdownLinkDependency => ({
	...
	describe('interpret', () => {
		...
		it('should categorize different dependency types correctly', async () => {
			...
			expect(result.dependencies[3].category).toBe(DependencyCategory.DOCUMENTATION);
		});
		...
		it('should detect broken links and missing files', async () => {
			...
			expect(result.issues.some(i => i.type === IssueType.BROKEN_LINK)).toBe(true);
			expect(result.issues.some(i => i.type === IssueType.MISSING_FILE)).toBe(true);
		...
		it('should perform security checks', async () => {
			...
			expect(result.issues.some(i => i.type === IssueType.SECURITY_RISK)).toBe(true);
		...
		it('should check file sizes for performance warnings', async () => {
			...
			expect(result.issues.some(i => i.type === IssueType.PERFORMANCE_ISSUE)).toBe(true);
		...
		it('should check accessibility for images', async () => {
			...
			const interpreter = new LinkDependencyInterpreter({ accessibilityChecks: true });
			const result = await interpreter.interpret(dependencies);

			const accessibilityIssues = result.issues.filter(i => i.type === IssueType.ACCESSIBILITY_ISSUE);
