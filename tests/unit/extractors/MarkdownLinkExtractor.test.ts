...
import type { MarkdownAST, MarkdownNode } from '../../../src/parsers/MarkdownParser';
...
describe('MarkdownLinkExtractor', () => {
	...
	const createMockNode = (type: string, overrides: Partial<MarkdownNode> = {}): MarkdownNode => ({
		type,
		position: {
	...
	describe('extract', () => {
		...
		it('should extract reference links when enabled', async () => {
			...
			const extractor = new MarkdownLinkExtractor({ followReferenceLinks: true });
			const dependencies = await extractor.extract(ast, '/path/to/file.md');
		...
		it('should skip reference links when disabled', async () => {
			...
			const extractor = new MarkdownLinkExtractor({ followReferenceLinks: false });
			const dependencies = await extractor.extract(ast, '/path/to/file.md');
		...
		it('should filter internal links when disabled', async () => {
			...
			const extractor = new MarkdownLinkExtractor({ includeInternalLinks: false });
			const dependencies = await extractor.extract(ast, '/path/to/file.md');
