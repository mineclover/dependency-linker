...
function createUniversalBanner(): void {
	...
	const emoji =
		platformEmojis[currentPlatform as keyof typeof platformEmojis] ||
		platformEmojis.default;
	...
	console.log(
		colorize(
			`│            ${emoji} Multi-OS PathInfo Demo 🌍                │`,
			"cyan",
		),
	);
	console.log(
		colorize(
			"│          크로스 플랫폼 경로 분석 통합 시스템                │",
			"cyan",
		),
	);
	console.log(
...
async function runMultiOSDemo() {
	...
	console.log(
		`   플랫폼: ${colorize(systemInfo.platformName, "green")} (${systemInfo.platform})`,
	);
	console.log(`   아키텍처: ${colorize(systemInfo.archName, "green")}`);
	console.log(`   Node.js: ${colorize(systemInfo.nodeVersion, "green")}`);
	console.log(
		`   CPU 코어: ${colorize(systemInfo.cpuCount.toString(), "green")}개`,
	);
	...
	try {
		...
		const allTestPaths = [
			...multiPlatformPaths.common,
			...multiPlatformPaths.specific,
		];

		console.log(
			colorize(
				`\n🌍 ${allTestPaths.length}개 다중 플랫폼 경로 테스트:`,
				"yellow",
			),
		);
		console.log(
			colorize("공통 경로:", "cyan"),
			multiPlatformPaths.common.length + "개",
		);
		console.log(
			colorize("플랫폼별 경로:", "cyan"),
			multiPlatformPaths.specific.length + "개",
		);
		...
		multiPathInfos.forEach((pathInfo) => {
			...
			platformResults.maxDepth = Math.max(
				platformResults.maxDepth,
				pathInfo.depth,
			);
			platformResults.totalDepth += pathInfo.depth;

			if (pathInfo.extension)
				platformResults.extensions.add(pathInfo.extension);
			platformResults.separatorTypes.add(pathInfo.separator);
		});

		console.log(colorize("\n📊 플랫폼 호환성 분석 결과:", "yellow"));
		console.log(
			`   프로젝트 내부: ${colorize(platformResults.withinProject.toString(), "green")}개`,
		);
		console.log(
			`   프로젝트 외부: ${colorize(platformResults.outsideProject.toString(), "yellow")}개`,
		);
		console.log(
			`   평균 깊이: ${colorize((platformResults.totalDepth / multiPathInfos.length).toFixed(1), "cyan")}`,
		);
		console.log(
			`   최대 깊이: ${colorize(platformResults.maxDepth.toString(), "cyan")}`,
		);
		console.log(
			`   확장자 종류: ${colorize(platformResults.extensions.size.toString(), "cyan")}개`,
		);
		console.log(
			`   구분자 종류: ${colorize(Array.from(platformResults.separatorTypes).join(", "), "cyan")}`,
		);
		...
		samplePaths.forEach((pathInfo, i) => {
			...
			if (extendedAnalysis.windows) {
				...
				console.log(
					`      Windows: 드라이브 ${winInfo.drive.drive || "N/A"}, 타입: ${winInfo.drive.driveType}`,
				);
				if (winInfo.unc) {
					console.log(
						`      UNC: 서버 ${winInfo.unc.server}, 공유 ${winInfo.unc.share}`,
					);
				}
			}
			...
			if (extendedAnalysis.macos) {
				...
				console.log(
					`      macOS: ${macInfo.systemPath.description} (${macInfo.systemPath.pathType})`,
				);
				if (macInfo.appBundle.isAppBundle) {
					console.log(
						`      앱 번들: ${macInfo.appBundle.appName} (${macInfo.appBundle.contentType})`,
					);
				}
			}
			...
			if (extendedAnalysis.linux) {
				...
				console.log(
					`      Linux: ${linuxInfo.fhs.description} (${linuxInfo.fhs.category})`,
				);
				console.log(
					`      보안: ${linuxInfo.permissions.securityLevel} 레벨, 소유자: ${linuxInfo.permissions.likelyOwner}`,
				);
			}
		});
		...
		for (const size of benchmarkSizes) {
			...
			console.log(
				`     향상: ${colorize(improvement.toFixed(1) + "%", "green")}`,
			);
		}
		...
		const avgImprovement =
			performanceResults.reduce((sum, r) => sum + r.improvement, 0) /
		...
		console.log(
			`   평균 성능 향상: ${colorize(avgImprovement.toFixed(1) + "%", "green")}`,
		);
		console.log(
			`   최적 배치 크기: ${colorize(bestSize.size.toString(), "green")}개 (${bestSize.improvement.toFixed(1)}% 향상)`,
		);
		...
		console.log(
			colorize(`✅ 프로젝트 분석 완료! (${projectAnalysisTime}ms)`, "green"),
		);
		console.log(
			`   총 파일: ${colorize(projectResults.length.toString(), "cyan")}개`,
		);
		console.log(
			`   유효한 PathInfo: ${colorize(validProjectPathInfos.length.toString(), "cyan")}개`,
		);
		...
		topDirectories.forEach(([dir, files], i) => {
			...
			console.log(
				`   ${i + 1}. ${icon} ${dir === "." ? "(루트)" : dir}: ${colorize(files.length.toString(), "cyan")}개`,
			);
		});
		...
		validProjectPathInfos.forEach((pathInfo) => {
			if (pathInfo.extension) {
				extensionStats.set(
					pathInfo.extension,
					(extensionStats.get(pathInfo.extension) || 0) + 1,
				);
			}
		});
		...
		Array.from(extensionStats.entries())
			...
			.forEach(([ext, count]) => {
				...
				console.log(
					`   ${icon} ${ext}: ${colorize(count.toString(), "cyan")}개`,
				);
			});
		...
		const memoryDiff = {
			...
			heapUsed:
				(finalMemUsage.heapUsed - systemInfo.memory.heapUsed) / 1024 / 1024,
			heapTotal:
				(finalMemUsage.heapTotal - systemInfo.memory.heapTotal) / 1024 / 1024,
			external:
				(finalMemUsage.external - systemInfo.memory.external) / 1024 / 1024,
		};

		console.log(colorize("\n💾 메모리 사용 현황:", "yellow"));
		console.log(
			`   RSS: ${colorize((finalMemUsage.rss / 1024 / 1024).toFixed(1), "cyan")} MB (${memoryDiff.rss > 0 ? "+" : ""}${memoryDiff.rss.toFixed(1)} MB)`,
		);
		console.log(
			`   Heap Used: ${colorize((finalMemUsage.heapUsed / 1024 / 1024).toFixed(1), "cyan")} MB (${memoryDiff.heapUsed > 0 ? "+" : ""}${memoryDiff.heapUsed.toFixed(1)} MB)`,
		);
		console.log(
			`   Heap Total: ${colorize((finalMemUsage.heapTotal / 1024 / 1024).toFixed(1), "cyan")} MB (${memoryDiff.heapTotal > 0 ? "+" : ""}${memoryDiff.heapTotal.toFixed(1)} MB)`,
		);
		console.log(
			`   External: ${colorize((finalMemUsage.external / 1024 / 1024).toFixed(1), "cyan")} MB (${memoryDiff.external > 0 ? "+" : ""}${memoryDiff.external.toFixed(1)} MB)`,
		);
		...
		console.log(`\n⚡ CPU 사용 시간:`);
		console.log(
			`   User: ${colorize((cpuUsage.user / 1000).toFixed(2), "cyan")}ms`,
		);
		console.log(
			`   System: ${colorize((cpuUsage.system / 1000).toFixed(2), "cyan")}ms`,
		);
		...
		const comprehensiveReport = {
			...
			demo: {
				...
				project: {
					...
					topDirectories: topDirectories.map(([dir, files]) => ({
						directory: dir,
						fileCount: files.length,
					})),
					extensions: Array.from(extensionStats.entries()).map(
						([ext, count]) => ({ extension: ext, count }),
					),
				},
				memory: {
		...
		console.log(
			`   💾 효율적 메모리 사용: ${(memoryDiff.heapUsed).toFixed(1)} MB 증가`,
		);
		console.log(
			`   ⚡ 고속 분석: ${projectResults.length}개 파일을 ${projectAnalysisTime}ms에 처리`,
		);

		console.log(
			colorize(
				"\n🌟 Multi-OS PathInfo 시스템 - 모든 플랫폼에서 최적의 성능!",
				"bright",
			),
		);
	} catch (error) {
