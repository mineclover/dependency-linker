...
import type { TreeSitterTree, TreeSitterNode } from "../types/TreeSitterTypes";
import type {
  IDataExtractor,
...
export class EnhancedExportExtractor implements IDataExtractor<EnhancedExportExtractionResult> {
  ...
  public readonly description = "Extracts detailed export information including functions, classes, variables, and class members";
  ...
  private supplementVariableExports(
    ...
    while ((match = variableExportPattern.exec(sourceCode)) !== null) {
      ...
      const exists = exportMethods.some(exp => exp.name === varName && exp.exportType === 'variable');
      if (!exists) {
        exportMethods.push({
          name: varName,
          exportType: 'variable',
    ...
    while ((match = namedExportPattern.exec(sourceCode)) !== null) {
      ...
      for (const exp of exports) {
        ...
        const exists = exportMethods.some(method => method.name === exportName);
        if (!exists && exportName && /^[A-Za-z_$][A-Za-z0-9_$]*$/.test(exportName)) {
          exportMethods.push({
            name: exportName,
            exportType: 'variable',
            declarationType: 'named_export',
  ...
  private processClassDeclaration(
    ...
    this.extractClassMembers(classNode, methods, properties, exportMethods, className, isExported);
  ...
  private processDefaultExport(
    ...
    if (exportType === 'class' && 
        (defaultExportNode.type === 'class_declaration' || defaultExportNode.type === 'abstract_class_declaration')) {
      // Process class declaration for detailed class info
      this.processClassDeclaration(defaultExportNode, exportMethods, classes, false);
  ...
  private processReExport(
    ...
    const source = this.getReExportSource(node);
  ...
  private getExportStatementType(node: Parser.SyntaxNode): string {
    // Look for specific child node types to determine export type
    let hasDefaultKeyword = false;
    let hasFromKeyword = false;
    ...
    for (let i = 0; i < node.childCount; i++) {
      ...
      if (child.text === 'from' || (child.type === 'string' && node.text.includes('from'))) {
        hasFromKeyword = true;
      }
      ...
      switch (child.type) {
        case 'function_declaration':
          return hasDefaultKeyword ? 'default' : 'function';
        case 'class_declaration':
        case 'abstract_class_declaration':
          return hasDefaultKeyword ? 'default' : 'class';
        case 'lexical_declaration':
        case 'variable_statement':
          return hasDefaultKeyword ? 'default' : 'variable';
        case 'interface_declaration':
        case 'type_alias_declaration':
        case 'enum_declaration':
          return hasDefaultKeyword ? 'default' : 'type';
      }
    }
    ...
    // If we have export clause without 'from', it's likely variable exports
    // like: export { internal as publicInternal }
    if (hasExportClause) {
      return 'variable';
    }
  ...
  private extractFunctionParameters(functionNode: Parser.SyntaxNode): ParameterInfo[] {
  ...
  private extractReturnType(functionNode: Parser.SyntaxNode): string | undefined {
  ...
  private getImplementedInterfaces(classNode: Parser.SyntaxNode): string[] | undefined {
  ...
  private findVariableNodes(node: Parser.SyntaxNode): Parser.SyntaxNode[] {
    ...
    for (let i = 0; i < node.childCount; i++) {
      ...
      if (child.type === 'lexical_declaration' || child.type === 'variable_statement') {
        ...
        if (declaratorList) {
          for (let j = 0; j < declaratorList.childCount; j++) {
            ...
            if (declarator && (declarator.type === 'variable_declarator' || declarator.type.includes('declarator'))) {
   ...
   */
  private findExportSpecifiersRecursive(node: Parser.SyntaxNode, variables: Parser.SyntaxNode[]): void {
  ...
  private findDefaultExportNode(node: Parser.SyntaxNode): Parser.SyntaxNode | null {
  ...
  private getNodeExportType(node: Parser.SyntaxNode): string {
    ...
    if (node.type === 'lexical_declaration' || node.type === 'variable_statement') return 'variable';
    return 'default';
  }
  ...
  private hasExportModifier(node: Parser.SyntaxNode): boolean {
    ...
    while (current) {
      ...
      if (current && (current.type === 'export_statement' || current.text.startsWith('export'))) {
        return true;
      }
  ...
  private processExportedNode(
    node: Parser.SyntaxNode,
    exportMethods: ExportMethodInfo[],
    classes: ClassExportInfo[]
  ): void {
    // Handle exported nodes that aren't in export statements
  }
  ...
  private isProperty(node: Parser.SyntaxNode): boolean {
    return node.type === 'property_definition' ||
           ...
           (node.type === 'lexical_declaration' && node.parent?.type === 'class_body');
  }

  private extractMethodInfo(node: Parser.SyntaxNode, className: string): ClassMethodInfo | null {
  ...
  private extractPropertyInfo(node: Parser.SyntaxNode, className: string): ClassPropertyInfo | null {
