...
/**
 * Efficient text pattern matching utilities for export detection
 */
export class TextMatcher {
	/**
	 * Pre-compiled regex patterns for better performance
	 */
	private static readonly compiledPatterns = {
	 ...
	 */
	static findAllExports(sourceCode: string): ExportMatch[] {
		const matches: ExportMatch[] = [];

		// Single pass through all patterns
		Object.entries(TextMatcher.compiledPatterns).forEach(
			([patternName, pattern]) => {
				...
				while ((match = pattern.exec(sourceCode)) !== null) {
					const exportType = TextMatcher.getExportTypeFromPattern(patternName);
					matches.push({
						match: match[0],
						type: exportType,
	 ...
	 */
	static findExportsByType(
		sourceCode: string,
		exportType: keyof typeof TextMatcher.compiledPatterns,
	): ExportMatch[] {
		const pattern = TextMatcher.compiledPatterns[exportType];
		if (!pattern) {
			return [];
		}
		...
		while ((match = pattern.exec(sourceCode)) !== null) {
			matches.push({
				match: match[0],
				type: TextMatcher.getExportTypeFromPattern(exportType),
				startIndex: match.index,
				endIndex: match.index + match[0].length,
				groups: Array.from(match).slice(1),
			});
	 ...
	 */
	static hasExports(sourceCode: string): boolean {
	 ...
	 */
	static parseNamedExports(namedExportContent: string): Array<{
		name: string;
		alias?: string;
	}> {
	 ...
	 */
	private static getExportTypeFromPattern(
		patternName: string,
	): ExportMatch["type"] {
		switch (patternName) {
			case "variableExport":
	 ...
	 */
	static cleanExportText(text: string): string {
		return text
