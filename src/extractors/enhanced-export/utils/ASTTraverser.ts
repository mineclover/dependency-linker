...
/**
 * Optimized AST traversal utilities
 */
export class ASTTraverser {
	 ...
	 */
	static traverse(root: Parser.SyntaxNode, visitor: NodeVisitor): void {
	...
	static findNodes(
		...
		ASTTraverser.traverse(root, (node) => {
			if (predicate(node)) {
				results.push(node);
			}
	...
	static findNodesByType(
		...
		return ASTTraverser.findNodes(root, (node) => node.type === nodeType);
	}
	 ...
	 */
	static findNodesByTypes(
		root: Parser.SyntaxNode,
		nodeTypes: Set<string>,
	): Parser.SyntaxNode[] {
		return ASTTraverser.findNodes(root, (node) => nodeTypes.has(node.type));
	}
	 ...
	 */
	static getChildren(node: Parser.SyntaxNode): Parser.SyntaxNode[] {
	 ...
	 */
	static getChildrenByType(
		node: Parser.SyntaxNode,
		nodeType: string,
	): Parser.SyntaxNode[] {
		return ASTTraverser.getChildren(node).filter(
