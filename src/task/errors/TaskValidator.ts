...
export function validateTask(task: Task): ValidationResult[] {
	...
	results.push(...validateBasicFields(task));

	// Business logic validation
	results.push(...validateBusinessRules(task));

	// Dependency validation
	results.push(...validateDependencies(task));

	// Metrics validation
	results.push(...validateMetrics(task));

	// Implementation tasks validation
	results.push(...validateImplementationTasks(task));
 ...
 * Validate task batch
 */
export function validateTaskBatch(batch: TaskBatch): ValidationResult[] {
	const results: ValidationResult[] = [];
...
/**
 * Private validation helper - validate basic fields
 */
function validateBasicFields(task: Task): ValidationResult[] {
	const results: ValidationResult[] = [];
...
function validateBusinessRules(task: Task): ValidationResult[] {
	const results: ValidationResult[] = [];
...
function validateDependencies(task: Task): ValidationResult[] {
	const results: ValidationResult[] = [];
...
function validateMetrics(task: Task): ValidationResult[] {
	const results: ValidationResult[] = [];
...
function validateImplementationTasks(task: Task): ValidationResult[] {
	const results: ValidationResult[] = [];
...
/**
 * Utility function to check if all validation results passed
 */
 ...
 */
export function getFailedValidations(
