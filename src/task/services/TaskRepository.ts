...
export class TaskRepository implements ITaskRepository {
	...
	private onTaskUpdatedCallbacks: Array<
	...
	async getCriticalPath(): Promise<Task[]> {
		...
		return tasks
			...
			.sort(
				(a, b) => a.metrics.estimatedDuration - b.metrics.estimatedDuration,
			);
	}

	async getParallelizableTasks(): Promise<Task[]> {
		return Array.from(this.tasks.values()).filter(
			(task) => task.isParallelizable,
		);
	}
	...
	async getStatistics(): Promise<TaskStatistics> {
		...
		const byPriority = {
			[TaskPriority.CRITICAL]: tasks.filter(
		...
		const byComplexity = {
			[TaskComplexity.SIMPLE]: tasks.filter(
				(t) => t.complexity === TaskComplexity.SIMPLE,
			).length,
			[TaskComplexity.MODERATE]: tasks.filter(
				(t) => t.complexity === TaskComplexity.MODERATE,
			).length,
			[TaskComplexity.COMPLEX]: tasks.filter(
				(t) => t.complexity === TaskComplexity.COMPLEX,
			).length,
		};
		...
		const byPhase = {
			[TaskPhase.FOUNDATION]: tasks.filter(
				(t) => t.phase === TaskPhase.FOUNDATION,
			).length,
			[TaskPhase.CORE_LAYER]: tasks.filter(
				(t) => t.phase === TaskPhase.CORE_LAYER,
			).length,
			[TaskPhase.API_LAYER]: tasks.filter(
				(t) => t.phase === TaskPhase.API_LAYER,
			).length,
			[TaskPhase.INTEGRATION_TESTING]: tasks.filter(
				(t) => t.phase === TaskPhase.INTEGRATION_TESTING,
			).length,
			[TaskPhase.PACKAGE_DISTRIBUTION]: tasks.filter(
				(t) => t.phase === TaskPhase.PACKAGE_DISTRIBUTION,
			).length,
			[TaskPhase.FINAL_VALIDATION]: tasks.filter(
				(t) => t.phase === TaskPhase.FINAL_VALIDATION,
			).length,
		};

		const estimatedTotalHours = tasks.reduce(
			(sum, task) => sum + task.metrics.estimatedDuration,
			0,
		);
		const actualTotalHours = tasks.reduce(
			(sum, task) => sum + (task.metrics.actualDuration || 0),
			0,
		);
		const completedTasksWithActual = tasks.filter(
			(t) => t.status === TaskStatus.COMPLETED && t.metrics.actualDuration,
		);
		const averageTaskDuration =
			completedTasksWithActual.length > 0
				? completedTasksWithActual.reduce(
						(sum, task) => sum + (task.metrics.actualDuration || 0),
						0,
					) / completedTasksWithActual.length
				: 0;

		const parallelizableTasksCount = tasks.filter(
	...
	async getAverageDuration(): Promise<number> {
		const completedTasks = Array.from(this.tasks.values()).filter(
			(task) =>
				task.status === TaskStatus.COMPLETED && task.metrics.actualDuration,
		);
	...
	onTaskUpdated(
		callback: (taskId: string, updates: Partial<Task>) => void,
	): void {
	...
	private applyFilters(tasks: Task[], filter: any): Task[] {
		return tasks.filter((task) => {
			...
			if (filter.priority && !filter.priority.includes(task.priority))
				return false;
			if (filter.complexity && !filter.complexity.includes(task.complexity))
				return false;
			...
			if (
				filter.tags &&
				(!task.tags ||
					!filter.tags.some((tag: string) => task.tags?.includes(tag)))
			)
				return false;
			if (filter.assignee && task.assignee !== filter.assignee) return false;
			if (
				filter.isParallelizable !== undefined &&
				task.isParallelizable !== filter.isParallelizable
			)
				return false;
		...
		return tasks.sort((a, b) => {
			...
			switch (sort.field) {
				...
				case "estimatedDuration":
					comparison =
						a.metrics.estimatedDuration - b.metrics.estimatedDuration;
					break;
				case "createdAt":
