...
import { ITaskExecutor, ExecutionStrategy } from '../interfaces/ITaskExecutor';
...
export class TaskManager implements ITaskManager {
  ...
  // Event callbacks
  private statusChangeCallbacks: Array<(taskId: string, oldStatus: TaskStatus, newStatus: TaskStatus) => void> = [];
  private progressCallbacks: Array<(taskId: string, progress: TaskProgress) => void> = [];
  private completedCallbacks: Array<(taskId: string, result: TaskExecutionResult) => void> = [];
  ...
  async deleteTask(taskId: string): Promise<boolean> {
    ...
    if (dependents.length > 0) {
      throw new Error(`Cannot delete task ${taskId}: it has ${dependents.length} dependent tasks`);
    }
  ...
  async startTask(taskId: string, context?: TaskExecutionContext): Promise<TaskExecutionResult> {
    ...
    if (!canStart) {
      throw new Error(`Task ${taskId} cannot be started: dependencies not satisfied`);
    }
    ...
    try {
      ...
      const finalStatus = result.success ? TaskStatus.COMPLETED : TaskStatus.CANCELLED;
      await this.updateTask(taskId, {
        status: finalStatus,
        metrics: {
          ...task.metrics,
          ...
          completionPercentage: result.success ? 100 : task.metrics.completionPercentage
        }
      });
      ...
      this.failedCallbacks.forEach(callback => {
        try {
          callback(taskId, error instanceof Error ? error : new Error(String(error)));
        } catch (callbackError) {
          // Ignore callback errors
        }
      });
  ...
  async completeTask(taskId: string, validationResults?: ValidationResult[]): Promise<TaskExecutionResult> {
    const task = await this.repository.findById(taskId);
    if (!task) {
      throw new Error(`Task with ID ${taskId} not found`);
    }
    ...
    const result: TaskExecutionResult = {
      ...
      endTime: now,
      duration: task.metrics.startTime ? now.getTime() - task.metrics.startTime.getTime() : 0,
  ...
  // Task Dependencies
  async addDependency(taskId: string, dependencyTaskId: string, type: 'blocks' | 'requires' | 'optional'): Promise<boolean> {
    const task = await this.repository.findById(taskId);
  ...
  async removeDependency(taskId: string, dependencyTaskId: string): Promise<boolean> {
    const task = await this.repository.findById(taskId);
    if (!task) {
      return false;
    }
  ...
  async getBlockedBy(taskId: string): Promise<Task[]> {
    ...
    for (const dep of task.dependencies.filter(d => d.type === 'blocks' || d.type === 'requires')) {
  ...
  async getProgress(taskId: string): Promise<TaskProgress> {
    ...
    return {
      ...
      stepsCompleted: task.implementationTasks.filter(t => t.completed).length,
      totalSteps: task.implementationTasks.length,
      message: this.getStatusMessage(task.status)
    };
  }

  async updateProgress(taskId: string, percentage: number, message?: string): Promise<boolean> {
    const task = await this.repository.findById(taskId);
    if (!task) {
      return false;
    }
  ...
  async executeBatch(batchId: string, maxConcurrency?: number): Promise<TaskExecutionResult[]> {
    const batch = await this.repository.getBatch(batchId);
    if (!batch) {
      throw new Error(`Batch with ID ${batchId} not found`);
    }

    const strategy = batch.parallelizable ? ExecutionStrategy.PARALLEL : ExecutionStrategy.SEQUENTIAL;
    const config = {
      strategy,
      maxConcurrency: maxConcurrency || batch.maxConcurrency
    };

    return this.executor.executeTasks(batch.tasks, config);
  }

  async getBatchProgress(batchId: string): Promise<Record<string, TaskProgress>> {
    const batch = await this.repository.getBatch(batchId);
    if (!batch) {
      throw new Error(`Batch with ID ${batchId} not found`);
    }
  ...
  async estimateCompletionTime(): Promise<number> {
    ...
    return incompleteTasks.reduce((total, task) => {
      const remainingTime = task.metrics.estimatedDuration * (100 - task.metrics.completionPercentage) / 100;
  ...
  // Event System
  onTaskStatusChange(callback: (taskId: string, oldStatus: TaskStatus, newStatus: TaskStatus) => void): void {
    this.statusChangeCallbacks.push(callback);
  }

  onTaskProgress(callback: (taskId: string, progress: TaskProgress) => void): void {
    this.progressCallbacks.push(callback);
  }

  onTaskCompleted(callback: (taskId: string, result: TaskExecutionResult) => void): void {
  ...
  private async wouldCreateCircularDependency(taskId: string, dependencyTaskId: string): Promise<boolean> {
  ...
  private exportToCsv(tasks: Task[]): string {
    const headers = ['ID', 'Title', 'Status', 'Priority', 'Complexity', 'Risk', 'Phase', 'Estimated Duration', 'Completion %'];
