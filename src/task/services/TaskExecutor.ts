...
export class TaskExecutor implements ITaskExecutor {
  ...
  async executeTask(task: Task, context?: TaskExecutionContext): Promise<TaskExecutionResult> {
    ...
    try {
      ...
      if (resourceUsage.activeExecutions >= this.resourceLimits.maxConcurrentTasks) {
        throw new Error(`Maximum concurrent tasks limit reached (${this.resourceLimits.maxConcurrentTasks})`);
      }

      // Create execution promise
      const executionPromise = this.performTaskExecution(task, context || {});
  ...
  async executeTasks(tasks: Task[], config?: TaskExecutionConfig): Promise<TaskExecutionResult[]> {
  ...
  async executeBatch(tasks: Task[], batchSize = 5, config?: TaskExecutionConfig): Promise<TaskExecutionResult[]> {
  ...
  async getExecutionProgress(taskId: string): Promise<TaskProgress> {
    ...
    return {
      taskId,
      status: this.activeExecutions.has(taskId) ? TaskStatus.IN_PROGRESS : TaskStatus.PENDING,
      completionPercentage: this.activeExecutions.has(taskId) ? 50 : 0,
      stepsCompleted: 0,
      totalSteps: 1,
      message: this.activeExecutions.has(taskId) ? 'Executing task...' : 'Task not started'
    };
  }
  ...
  async validateTaskExecution(task: Task): Promise<ValidationResult[]> {
    ...
    for (const criterion of task.validationCriteria) {
      ...
      if (criterion.description.includes('file') && (!task.files || task.files.length === 0)) {
        result.passed = false;
  ...
  async validateExecutionEnvironment(context: TaskExecutionContext): Promise<boolean> {
    // Basic validation
  ...
  private async executeSequential(tasks: Task[], config?: TaskExecutionConfig): Promise<TaskExecutionResult[]> {
    const results: TaskExecutionResult[] = [];
  ...
  private async executeParallel(tasks: Task[], config?: TaskExecutionConfig): Promise<TaskExecutionResult[]> {
    ...
    for (let i = 0; i < tasks.length; i++) {
      ...
      if (executing.length >= maxConcurrency) {
        ...
        const completedIndex = executing.findIndex(promise => promise === Promise.resolve(completedResult));
        if (completedIndex !== -1) {
  ...
  private async executeAdaptive(tasks: Task[], config?: TaskExecutionConfig): Promise<TaskExecutionResult[]> {
    ...
    if (parallelizableTasks.length > 0) {
      const parallelResults = await this.executeParallel(parallelizableTasks, config);
      results.push(...parallelResults);

      if (config?.failFast && parallelResults.some(result => !result.success)) {
        return results;
      }
    }
    ...
    if (sequentialTasks.length > 0) {
      const sequentialResults = await this.executeSequential(sequentialTasks, config);
  ...
  private async performTaskExecution(task: Task, context: TaskExecutionContext): Promise<TaskExecutionResult> {
    const startTime = new Date();
    ...
    try {
      ...
      for (let progress = 0; progress <= 100; progress += 20) {
        await this.updateProgress(task.id, progress, `Executing task... ${progress}%`);
  ...
  private async updateProgress(taskId: string, percentage: number, message?: string): Promise<void> {
