...
export interface ITaskManager {
   ...
   */
  startTask(taskId: string, context?: TaskExecutionContext): Promise<TaskExecutionResult>;
  pauseTask(taskId: string): Promise<boolean>;
  resumeTask(taskId: string): Promise<boolean>;
  completeTask(taskId: string, validationResults?: ValidationResult[]): Promise<TaskExecutionResult>;
   ...
   * Task Dependencies
   */
  addDependency(taskId: string, dependencyTaskId: string, type: 'blocks' | 'requires' | 'optional'): Promise<boolean>;
  ...
  updateProgress(taskId: string, percentage: number, message?: string): Promise<boolean>;
  
  /**
   * Batch Operations
   */
  createBatch(batch: Omit<TaskBatch, 'id'>): Promise<TaskBatch>;
  executeBatch(batchId: string, maxConcurrency?: number): Promise<TaskExecutionResult[]>;
