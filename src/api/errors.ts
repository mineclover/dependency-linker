...
export class InvalidFileTypeError extends AnalysisError {
  constructor(filePath: string, supportedTypes: string[] = []) {
    super(
      `Invalid file type for analysis: ${filePath}`,
      'INVALID_FILE_TYPE',
  ...
  override getUserMessage(): string {
    ...
    const supportedTypesText = supportedTypes.length > 0 
...
export class ConfigurationError extends AnalysisError {
  constructor(parameter: string, value: any, expectedType?: string, cause?: Error) {
    super(
...
export class BatchError extends AnalysisError {
  ...
  constructor(
    ...
    super(message, 'BATCH_ERROR', { ...details, failedFileCount: failedFiles.length }, cause);
...
export class ResourceError extends AnalysisError {
  constructor(resource: string, details: Record<string, any> = {}, cause?: Error) {
    super(
...
export class InternalError extends AnalysisError {
  constructor(context: string, details: Record<string, any> = {}, cause?: Error) {
    super(
...
export class DependencyError extends AnalysisError {
  constructor(dependency: string, details: Record<string, any> = {}, cause?: Error) {
    super(
...
export class UnsupportedFormatError extends AnalysisError {
  ...
  override getUserMessage(): string {
    ...
    const supportedText = supportedFormats.length > 0 
...
export class ErrorUtils {
  ...
  static createBatchError(
    ...
    const errorDetails = errors.map(e => ({
      filePath: e.filePath,
      error: this.getErrorCode(e.error),
      message: this.getUserMessage(e.error)
    }));
