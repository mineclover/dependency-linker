...
export class DebugHelper {
  ...
  public static gatherFileContext(filePath: string): DebugContext {
    ...
    try {
      ...
      throw new Error(`Failed to gather file context: ${(error as Error).message} (Error ID: ${errorId})`);
    }
  }
   ...
   */
  public static analyzeParseFailure(filePath: string, error: Error): {
    ...
    if (context.hasUnicodeChars) {
      ...
      suggestions.push('Check for invisible Unicode characters that might break parsing');
    ...
    diagnosticInfo.performanceMetrics = {
      ...
      fileSizeToMemoryRatio: context.fileSize / (process.memoryUsage().heapUsed || 1)
    };
   ...
   */
  public static endPerformanceTracking(operationId: string): PerformanceMetrics | null {
   ...
   */
  public static validateAnalysisResult(result: AnalysisResult, filePath: string): {
    isValid: boolean;
    warnings: string[];
    errors: string[];
    suggestions: string[];
    ...
    if (!result.filePath || result.filePath !== filePath) {
      errors.push(`File path mismatch: expected ${filePath}, got ${result.filePath}`);
    }
    ...
    if (result.success) {
      ...
      if (result.dependencies && result.imports) {
        ...
        if (Math.abs(dependencyCount - importCount) > importCount * 0.5) {
          warnings.push(`Dependency count (${dependencyCount}) and import count (${importCount}) differ significantly`);
  ...
  public static analyzeDependencyPatterns(dependencies: DependencyInfo[]): {
    ...
    dependencyMap.forEach((deps, source) => {
      if (deps.length > 1) {
        ...
        if (types.size > 1) {
          circularRisks.push(`${source} imported with multiple types: ${Array.from(types).join(', ')}`);
        }
      }
    });
    ...
    if (externalDeps.length > 20) {
      unusualPatterns.push(`High number of external dependencies (${externalDeps.length})`);
      recommendations.push('Consider consolidating external dependencies');
    }

    if (relativeDeps.length > 15) {
      unusualPatterns.push(`High number of relative imports (${relativeDeps.length})`);
    ...
    const nodeBuiltins = ['fs', 'path', 'os', 'crypto', 'util', 'http', 'https', 'url', 'querystring', 'stream'];
    const builtinDeps = dependencies.filter(d => nodeBuiltins.includes(d.source) || d.source.startsWith('node:'));
    if (builtinDeps.length > 10) {
      unusualPatterns.push(`Many Node.js builtin modules used (${builtinDeps.length})`);
      recommendations.push('Verify all builtin imports are necessary');
    }

    if (heavyDependencies.length > 0) {
      recommendations.push('Review frequently imported modules for potential optimization');
    }
  ...
  public static exportDebugInfo(): {
    ...
    return {
      ...
      performanceMarks: Array.from(DebugHelper.performanceMarks.entries()).map(([operation, startTime]) => ({
        operation,
        startTime
      })),
      systemInfo: {
   ...
   */
  public static createDebugReport(filePath?: string, analysisResult?: AnalysisResult): string {
    ...
    if (analysisResult && filePath) {
      const validation = DebugHelper.validateAnalysisResult(analysisResult, filePath);
      ...
      if (analysisResult.success && analysisResult.dependencies) {
        const depAnalysis = DebugHelper.analyzeDependencyPatterns(analysisResult.dependencies);
