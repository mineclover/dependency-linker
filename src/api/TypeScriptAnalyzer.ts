...
import { IFileAnalyzer } from '../core/interfaces/IFileAnalyzer';
import { ITypeScriptParser } from '../core/interfaces/ITypeScriptParser';
import { IOutputFormatter } from '../core/interfaces/IOutputFormatter';

import { AnalysisResult } from '../models/AnalysisResult';
import { FileAnalysisRequest } from '../models/FileAnalysisRequest';
import { ValidationResult } from '../core/types/ParseTypes';
...
import {
  ...
  AnalyzerOptions,
  FileAnalysisRequest as APIFileAnalysisRequest,
...
export class TypeScriptAnalyzer {
  ...
  constructor(
    ...
    this.fileAnalyzer = dependencies?.fileAnalyzer || new FileAnalyzer(this.parser);
    this.formatter = dependencies?.formatter || new OutputFormatter();
    this.logger = dependencies?.logger || this.config.logger || createLogger('TypeScriptAnalyzer');
  ...
  async analyzeFile(filePath: string, options?: AnalysisOptions): Promise<AnalysisResult> {
    ...
    try {
      ...
      const request: FileAnalysisRequest = this.createFileAnalysisRequest(filePath, options);
      ...
      this.logger.debug(`File analysis completed: ${filePath} (${analysisTime}ms)`);
      
      return result;
    } catch (error) {
      const analysisError = ErrorUtils.fromUnknown(error, 'analyzeFile');
      this.emitEvent(AnalyzerEvent.ANALYSIS_ERROR, { filePath, error: analysisError });
   ...
   */
  async analyzeFiles(filePaths: string[], options?: BatchAnalysisOptions): Promise<BatchResult> {
    this.throwIfNotInitialized();
   ...
   * @returns Promise resolving to analysis result
   */
  async analyzeSource(source: string, options?: SourceAnalysisOptions): Promise<AnalysisResult> {
    this.throwIfNotInitialized();
    ...
    try {
      ...
      let result: AnalysisResult = {
        ...
        success: !parseResult.hasParseErrors || parseResult.dependencies.length > 0,
      ...
      if (parseResult.hasParseErrors) {
        result = {
          ...
          error: {
            code: 'PARSE_ERROR',
            message: 'Source contains syntax errors but partial analysis was possible',
      ...
      this.updateMetrics(analysisTime);
      this.emitEvent(AnalyzerEvent.ANALYSIS_COMPLETE, { filePath: contextPath, result });
      
      return result;
    } catch (error) {
      const analysisError = ErrorUtils.fromUnknown(error, 'analyzeSource');
      this.emitEvent(AnalyzerEvent.ANALYSIS_ERROR, { filePath: contextPath, error: analysisError });
      throw analysisError;
    }
  }
  ...
  async extractDependencies(filePath: string): Promise<string[]> {
    const result = await this.analyzeFile(filePath, { includeTypeImports: false });
   ...
   * @returns Promise resolving to batch results
   */
  private async analyzeBatch(request: BatchAnalysisRequest): Promise<BatchResult> {
    const { filePaths, options = {} } = request;
    const startTime = Date.now();
    
    this.emitEvent(AnalyzerEvent.BATCH_START, { filePaths, totalFiles: filePaths.length });
   ...
   */
  private createFileAnalysisRequest(filePath: string, options?: AnalysisOptions): FileAnalysisRequest {
    return {
      filePath,
      options: {
        format: options?.format ?? 'json',
        includeSources: options?.includeSources ?? false,
        parseTimeout: options?.parseTimeout ?? this.config.defaultTimeout ?? 30000
      }
    };
  }
  ...
  private updateMetrics(analysisTime: number): void {
    ...
    this.metrics.averageAnalysisTime = this.metrics.totalAnalysisTime / this.metrics.totalFilesAnalyzed;
  }
  ...
  /**
   * Enhanced analyzeFile with diagnostic capabilities
   */
  private async analyzeFileWithDiagnostics(filePath: string, options?: AnalysisOptions): Promise<AnalysisResult> {
    const operationId = `analyze_${filePath}_${Date.now()}`;
    
    try {
      // Start performance tracking
      DebugHelper.startPerformanceTracking(operationId);
      
      // Perform the actual analysis
      this.throwIfNotInitialized();
      const request: FileAnalysisRequest = { filePath, options: options || {} };
      const result = await this.fileAnalyzer.analyzeFile(request);
      
      // End performance tracking
      DebugHelper.endPerformanceTracking(operationId);
      ...
      // Add debug trace for error
      DebugHelper.addTrace({
        timestamp: Date.now(),
        operation: 'analyzeFile',
        phase: 'error',
        error: error as Error,
        data: { filePath, errorId }
      });
      ...
      throw new ParseError((error as Error).message, filePath);
    }
  }
  ...
  private throwIfNotInitialized(): void {
    if (!this.isInitialized) {
      throw new ConfigurationError('analyzer', 'not initialized', 'initialized analyzer');
    }
  }
}
