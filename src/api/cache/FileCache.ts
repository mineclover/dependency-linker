...
import { CacheProvider, CacheEntry, CacheStats, CacheOptions } from './index';
...
export class FileCache<T> implements CacheProvider<T> {
  ...
  constructor(options: CacheOptions & { cacheDir?: string } = {}) {
    this.cacheDir = options.cacheDir || path.join(os.tmpdir(), 'ts-analyzer-cache');
  ...
  private async cleanupExpired(): Promise<void> {
    try {
      ...
      this.metadata.totalEntries = Math.max(0, this.metadata.totalEntries - cleanedCount);
    } catch {
