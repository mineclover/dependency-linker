...
import type { CacheOptions, CacheProvider, CacheStats } from "./index";
...
export class CacheManager<T> implements CacheProvider<T> {
	...
	getStats(): CacheStats {
		...
		const totalRequests =
			memoryStats.totalHits +
			memoryStats.totalMisses +
			(fileStats.totalHits + fileStats.totalMisses);

		const combinedHitRate =
			totalRequests > 0
				? (memoryStats.totalHits + fileStats.totalHits) / totalRequests
				: 0;
	...
	getCacheHealth(): {
		...
		if (stats.memory) {
			if (
				stats.memory.hitRate < 0.5 &&
				stats.memory.totalHits + stats.memory.totalMisses > 100
			) {
				issues.push("Low memory cache hit rate");
				recommendations.push("Consider increasing memory cache size or TTL");
			}

			if (stats.memory.memoryUsage > 100 * 1024 * 1024) {
				// 100MB
		...
		if (stats.file) {
			if (
				stats.file.hitRate < 0.3 &&
				stats.file.totalHits + stats.file.totalMisses > 100
			) {
