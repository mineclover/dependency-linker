...
export type { PythonQueryResultMap };

// ===== TYPE UTILITIES =====
export type QueryResult<K extends QueryKey> = UnifiedQueryResultMap[K];

export type LanguageSpecificQueryKey<L extends LanguageGroup> =
	LanguageQueryKey<L>;

export type LanguageSpecificQueryResult<
	L extends LanguageGroup,
	K extends LanguageSpecificQueryKey<L>,
> = L extends "typescript"
	? K extends keyof TypeScriptQueryResultMap
		? TypeScriptQueryResultMap[K]
		: never
	: L extends "javascript"
		? K extends keyof JavaScriptQueryResultMap
			? JavaScriptQueryResultMap[K]
			: never
		: L extends "go"
			? K extends keyof GoQueryResultMap
				? GoQueryResultMap[K]
				: never
			: K extends keyof JavaQueryResultMap
				? JavaQueryResultMap[K]
				: never;
...
export type ValidatedQueryResultMap = {
	[K in keyof UnifiedQueryResultMap]: UnifiedQueryResultMap[K] extends {
		queryName: K;
	}
		? UnifiedQueryResultMap[K]
		: never;
};
...
export type ClassQueryKeys = Extract<
	QueryKey,
	| `${string}class${string}`
...
>;
export type FunctionQueryKeys = Extract<
	QueryKey,
...
export type LanguageQueryKeys<L extends LanguageGroup> = Extract<
	QueryKey,
	`${L}-${string}`
>;
 ...
 * 쿼리 키에서 언어를 추출
 */
export function extractLanguageFromQueryKey(
	queryKey: QueryKey,
): LanguageGroup | "common" {
	const parts = queryKey.split("-");
	const language = parts[0];

	if (["typescript", "javascript", "go", "java", "python"].includes(language)) {
 ...
 */
export function groupQueryKeysByLanguage(
	queryKeys: QueryKey[],
): Record<LanguageGroup | "common", QueryKey[]> {
...
// ===== CONSTANTS =====
export const ALL_LANGUAGE_GROUPS: LanguageGroup[] = [
	"typescript",
	"javascript",
	"go",
	"java",
...
export const IMPORT_QUERY_PATTERN = /^(ts|js|go|java|python)-?.*import/;
export const EXPORT_QUERY_PATTERN = /^(ts|js|go|java|python)-?.*export/;
export const CLASS_QUERY_PATTERN =
	/^(ts|js|go|java|python)-?.*(class|interface|enum|struct)/;
export const FUNCTION_QUERY_PATTERN =
	/^(ts|js|go|java|python)-?.*(function|method|arrow|lambda|closure)/;
