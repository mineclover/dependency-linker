...
import type { QueryResultMap, QueryKey, QueryResult } from "./QueryResultMap";
...
export class QueryRegistry {
  ...
  register<K extends QueryKey>(
    ...
    if (queryFunction.resultType !== queryKey) {
      throw new Error(`Query function result type "${queryFunction.resultType}" does not match key "${queryKey}"`);
    }

    this.queries.set(queryKey, queryFunction as QueryFunction<BaseQueryResult, string>);
   ...
   * 쿼리 조회
   */
  get<K extends QueryKey>(queryKey: K): QueryFunction<QueryResult<K>> | undefined {
    return this.queries.get(queryKey) as QueryFunction<QueryResult<K>> | undefined;
  }
   ...
   * 쿼리가 언어를 지원하는지 확인
   */
  supportsLanguage<K extends QueryKey>(queryKey: K, language: SupportedLanguage): boolean {
  ...
  validate(): ValidationResult {
    ...
    for (const [key, query] of this.queries) {
      ...
      if (query.resultType !== key) {
        errors.push(`Query "${key}" has mismatched resultType: ${query.resultType}`);
      }
...
export class QueryEngine {
  ...
  async execute<K extends QueryKey>(
    ...
    try {
      ...
      if (!this.registry.supportsLanguage(queryKey, context.language)) {
        throw new Error(`Query "${queryKey}" does not support language "${context.language}"`);
      }
   ...
   * 쿼리 결과 검증
   */
  private validateResults<K extends QueryKey>(queryKey: K, results: QueryResult<K>[]): void {
    for (const result of results) {
      ...
      if (result.queryName !== queryKey) {
        throw new Error(`Result queryName "${result.queryName}" does not match query key "${queryKey}"`);
      }
   ...
   * 성능 메트릭 기록
   */
  private recordPerformanceMetrics(queryKey: QueryKey, metrics: QueryPerformanceMetrics): void {
   ...
   * 성능 메트릭 조회
   */
  getPerformanceMetrics(queryKey?: QueryKey): Map<QueryKey, QueryPerformanceMetrics[]> | QueryPerformanceMetrics[] {
    if (queryKey) {
