...
import type { ITypeScriptParser } from "../interfaces/ITypeScriptParser";
...
export class TypeScriptParser implements ITypeScriptParser {
	...
	async parseSource(
		...
		try {
			...
			const _parseTime = Date.now() - startTime;
	 ...
	 */
	private async performSourceParsing(
		source: string,
		options?: ParseOptions,
	): Promise<ParseResult> {
		try {
			...
			if (options?.includeTypeImports === false) {
				...
				finalResult.imports = result.imports.filter(
					(imp: any) => !imp.isTypeOnly,
				);
			}

			logger.debug(
				`Source parsing complete: ${result.dependencies.length} dependencies, ${result.imports.length} imports, ${result.exports.length} exports`,
			);
