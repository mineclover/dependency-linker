...
export class TypeScriptParser {
	...
	async parseFile(filePath: string, content: string): Promise<AnalysisResult> {
		...
		try {
			...
			logger.debug(
				`Parsing complete: ${result.dependencies.length} dependencies, ${result.imports.length} imports, ${result.exports.length} exports`,
			);
			...
			if (
				...
				const analysisResult = createSuccessResult(
					filePath,
					parseTime,
					result.dependencies,
					result.imports,
				...
				analysisResult.error = {
					code: "PARSE_ERROR",
					message:
						"File contains syntax errors but partial analysis was possible",
				};
				return analysisResult;
			} else if (result.hasParseErrors) {
				// No results and parse errors - this is still a failure
				const err = new Error(
					"Parse error: Unable to parse TypeScript content. The file contains syntax errors or invalid TypeScript code.",
				);
