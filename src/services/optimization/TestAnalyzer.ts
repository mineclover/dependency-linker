...
export class TestAnalyzer {
  ...
  async analyzeTestSuite(testDirectory: string): Promise<TestSuiteAnalysis> {
    ...
    try {
      ...
      const executionTime = testSuites.reduce((sum, suite) => sum + suite.executionTime, 0);

      // Calculate failure rate
      const failedTests = testSuites.reduce(
        (sum, suite) => sum + suite.testCases.filter(tc => tc.isFlaky || tc.lastFailure).length,
        0
      );
      ...
      const analysis: TestSuiteAnalysis = {
        testSuites,
        categorizedTests: { critical: [], optimize: testSuites, remove: [], duplicates: [] },
        opportunities: [],
        totalTests,
        totalSuites,
      ...
      throw new Error(`Test suite analysis failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }

  async categorizeTests(analysis: TestSuiteAnalysis): Promise<CategorizedTests> {
    if (!analysis) {
    ...
    for (const suite of analysis.testSuites) {
      const suiteDuplicates = await this.findDuplicatesInSuite(suite, analysis.testSuites);
  ...
  async identifyOptimizations(categorized: CategorizedTests): Promise<OptimizationOpportunity[]> {
    if (!categorized) {
      throw new Error('Categorized tests are required for optimization identification');
    }
    ...
    if (categorized.duplicates.length > 0) {
      const duplicateOpportunities = this.createDuplicateRemovalOpportunities(categorized.duplicates);
  ...
  private async analyzeTestFiles(testFiles: TestFileInfo[]): Promise<TestSuite[]> {
  ...
  private determineTestType(testName: string, content: string): TestType {
    ...
    if (name.includes('contract') || name.includes('interface') || contentLower.includes('contract')) {
      return TestType.Contract;
    }

    if (name.includes('integration') || name.includes('e2e') || contentLower.includes('integration')) {
      return TestType.Integration;
    }

    if (name.includes('unit') || (!name.includes('integration') && !name.includes('e2e'))) {
  ...
  private determinePriority(testName: string, content: string): Priority {
    ...
    if (name.includes('critical') || name.includes('core') ||
        contentLower.includes('critical') || contentLower.includes('essential')) {
    ...
    if (name.includes('edge') || name.includes('error') || name.includes('fail')) {
  ...
  private estimateExecutionTime(testName: string, content: string): number {
    ...
    if (name.includes('async') || contentLower.includes('await') || contentLower.includes('promise')) {
  ...
  private detectFlakyTest(testName: string, content: string): boolean {
    ...
    const flakyIndicators = [
      'flaky', 'intermittent', 'sometimes', 'random',
      'settimeout', 'retry', 'attempt', 'timing'
    ];
  ...
  private isCriticalSuite(suite: TestSuite): boolean {
    ...
    const criticalIndicators = [
      'api', 'contract', 'interface', 'core', 'engine', 'parser'
    ];

    // Check filename
    if (criticalIndicators.some(indicator => fileName.toLowerCase().includes(indicator))) {
      return true;
    }

    // Check if majority of tests are critical priority
    const criticalTests = suite.testCases.filter(tc => tc.priority === Priority.Critical);
  ...
  private shouldRemoveSuite(suite: TestSuite): boolean {
    ...
    if (duplicates.length === suite.testCases.length && suite.testCases.length > 0) {
      return true;
    }

    // Remove if all tests are flaky and low priority
    const flakyLowPriority = suite.testCases.filter(tc =>
      tc.isFlaky && tc.priority === Priority.Low
    );
    if (flakyLowPriority.length === suite.testCases.length && suite.testCases.length > 0) {
      return true;
    }
  ...
  private async findDuplicatesInSuite(suite: TestSuite, allSuites: TestSuite[]): Promise<DuplicateTest[]> {
    const duplicates: DuplicateTest[] = [];
    ...
    for (const testCase of suite.testCases) {
      for (const otherSuite of allSuites) {
        ...
        for (const otherTestCase of otherSuite.testCases) {
          const similarity = this.calculateTestSimilarity(testCase, otherTestCase);
  ...
  private calculateTestSimilarity(test1: TestCase, test2: TestCase): number {
    ...
    const coverageSimilarity = this.calculateArraySimilarity(test1.coverageAreas, test2.coverageAreas);
    ...
    // Weighted average
    return (nameSimilarity * 0.4) + (coverageSimilarity * 0.3) + (typeSimilarity * 0.2) + (prioritySimilarity * 0.1);
  }
  ...
  private createDuplicateRemovalOpportunities(duplicates: DuplicateTest[]): OptimizationOpportunity[] {
    const opportunities: OptimizationOpportunity[] = [];

    // Group duplicates by original test
    const grouped = duplicates.reduce((groups, duplicate) => {
    ...
    Object.entries(grouped).forEach(([originalId, dups]) => {
      opportunities.push({
        ...
        estimatedTimeSaving: dups.length * 50, // Assume 50ms per duplicate
  ...
  private async identifySuiteOptimizations(suite: TestSuite): Promise<OptimizationOpportunity[]> {
    const opportunities: OptimizationOpportunity[] = [];
    ...
    if (flakyTests.length > 0) {
      opportunities.push({
        ...
        estimatedTimeSaving: flakyTests.length * 200, // Flaky tests often take longer
    ...
    if (suite.setupComplexity === ComplexityLevel.High) {
      opportunities.push({
        ...
        estimatedTimeSaving: 100, // Reduced setup time
        riskLevel: RiskLevel.Medium,
  ...
  private identifyCrossSuiteOptimizations(suites: TestSuite[]): OptimizationOpportunity[] {
    const opportunities: OptimizationOpportunity[] = [];
    ...
    if (duplicatedSetups.length > 2) {
      opportunities.push({
        ...
        estimatedTimeSaving: duplicatedSetups.length * 50,
        riskLevel: RiskLevel.Low,
        implementationEffort: EffortLevel.Low,
        prerequisites: [],
        status: 'identified' as any,
  ...
  private findDuplicatedSetups(suites: TestSuite[]): string[] {
    ...
    const dependencyGroups = suites.reduce((groups, suite) => {
    ...
    return Object.values(dependencyGroups).filter(group => group.length > 1).flat();
  }
  ...
  private determineSuiteCategory(content: string, testCases: TestCase[]): TestCategory {
    // Analyze content and test cases to determine category
    const contentLower = content.toLowerCase();

    if (contentLower.includes('contract') || contentLower.includes('interface') ||
  ...
  private estimateSetupComplexity(content: string): ComplexityLevel {
    ...
    if (contentLower.includes('beforeall') || contentLower.includes('beforeeach')) complexityScore += 2;
    if (contentLower.includes('afterall') || contentLower.includes('aftereach')) complexityScore += 2;
    ...
    if (contentLower.includes('database') || contentLower.includes('db')) complexityScore += 3;
    if (contentLower.includes('server') || contentLower.includes('api')) complexityScore += 3;
