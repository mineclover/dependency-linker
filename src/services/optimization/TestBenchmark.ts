...
import { TestCase } from '../../models/optimization/TestCase';
...
export class TestBenchmark {
  ...
  async benchmarkTestSuite(
    ...
    console.log(`🏃 Starting benchmark: ${testSuite.name} (${configuration.iterations} iterations)`);
    ...
    if (configuration.warmupIterations > 0) {
      console.log(`🔥 Warmup phase: ${configuration.warmupIterations} iterations`);
    ...
    for (let i = 0; i < configuration.iterations; i++) {
      try {
        const measurement = await this.runSingleIteration(testRunner, i, configuration, true);
        measurements.push(measurement);

        if (i > 0 && i % Math.max(1, Math.floor(configuration.iterations / 10)) === 0) {
          const progress = ((i / configuration.iterations) * 100).toFixed(0);
          console.log(`📈 Progress: ${progress}% (${i}/${configuration.iterations})`);
        }
      } catch (error) {
  ...
  async compareBenchmarks(
    ...
    const baseline = this.results.get(baselineId) || this.baselines.get(baselineId);

    if (!current || !baseline) {
      throw new Error(`Benchmark not found: current=${!!current}, baseline=${!!baseline}`);
    }
  ...
  private createConfiguration(config: Partial<BenchmarkConfiguration>): BenchmarkConfiguration {
    return {
  ...
  private async runSingleIteration(
    ...
    const cpuUsage = endCPU ? this.calculateCPUMeasurement(endCPU, duration) : undefined;
  ...
  private calculateCPUMeasurement(cpuUsage: NodeJS.CpuUsage, duration: number): CPUMeasurement {
  ...
  private calculateStatistics(measurements: BenchmarkMeasurement[]): BenchmarkStatistics {
    const successfulMeasurements = measurements.filter(m => m.success);
    const durations = successfulMeasurements.map(m => m.duration);
    const memoryUsages = successfulMeasurements.map(m => m.memoryUsage.heapUsed);
    ...
    const throughput = measurements.length > 0 ?
      1000 / durationStats.mean : 0; // operations per second
  ...
  private calculateComparison(baseline: BenchmarkResult, current: BenchmarkResult): ComparisonResult {
    const durationChange = (current.statistics.duration.mean - baseline.statistics.duration.mean) / baseline.statistics.duration.mean;
    const memoryChange = (current.statistics.memoryUsage.mean - baseline.statistics.memoryUsage.mean) / baseline.statistics.memoryUsage.mean;
    const throughputChange = (current.statistics.throughput - baseline.statistics.throughput) / baseline.statistics.throughput;

    // Simple statistical significance test (simplified)
    const combinedStdErr = Math.sqrt(
      Math.pow(baseline.statistics.duration.stdDev, 2) + Math.pow(current.statistics.duration.stdDev, 2)
    );
    const tStatistic = Math.abs(current.statistics.duration.mean - baseline.statistics.duration.mean) / combinedStdErr;
  ...
  private analyzeComparison(
    ...
    if (verdict === 'regression') {
      ...
      warnings.push(`Performance degraded by ${(comparison.durationChange * 100).toFixed(1)}%`);
    } else if (verdict === 'improvement') {
      recommendations.push('Document optimization techniques for future reference');
      recommendations.push('Consider applying similar optimizations to other areas');
    } else {
      recommendations.push('Continue monitoring performance trends');
    }

    if (comparison.memoryChange > 0.1) {
      warnings.push(`Memory usage increased by ${(comparison.memoryChange * 100).toFixed(1)}%`);
    }
  ...
  private printBenchmarkSummary(result: BenchmarkResult): void {
    ...
    console.log(`   Duration: ${result.statistics.duration.mean.toFixed(2)}ms ±${result.statistics.duration.stdDev.toFixed(2)}ms`);
    console.log(`   Memory: ${(result.statistics.memoryUsage.mean / 1024 / 1024).toFixed(2)}MB`);
    console.log(`   Throughput: ${result.statistics.throughput.toFixed(2)} ops/sec`);
