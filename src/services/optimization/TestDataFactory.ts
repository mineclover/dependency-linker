...
export class TestDataFactory {
  ...
  createTestSuite(options: TestDataOptions = {}): TestSuite {
    ...
    return {
      ...
      lastModified: options.realistic ? new Date(Date.now() - this.randomInt(0, 7 * 24 * 60 * 60 * 1000)) : new Date(),
  ...
  createTestCase(options: TestDataOptions = {}): TestCase {
    ...
    return {
      ...
      lastFailure: options.realistic && this.randomInt(1, 5) === 1 ? new Date(Date.now() - this.randomInt(0, 30 * 24 * 60 * 60 * 1000)) : undefined,
   ...
   */
  createOptimizationOpportunity(options: TestDataOptions = {}): OptimizationOpportunity {
    this.setSeed(options.seed);
    ...
    const type = opportunityTypes[this.randomInt(0, opportunityTypes.length - 1)];
    const impact = ['low', 'medium', 'high', 'critical'][this.randomInt(0, 3)] as 'low' | 'medium' | 'high' | 'critical';
    const effort = ['low', 'medium', 'high'][this.randomInt(0, 2)] as 'low' | 'medium' | 'high';
    ...
    return {
      ...
      estimatedTimeSaving: this.calculateTimeReduction(impact),
   ...
   */
  createPerformanceBaseline(options: TestDataOptions = {}): PerformanceBaseline {
    this.setSeed(options.seed);
    const size = options.size || 'medium';
   ...
   */
  createTestSuiteCollection(count: number, options: TestDataOptions = {}): TestSuite[] {
   ...
   */
  createPerformanceScenario(name: string, options: TestDataOptions = {}): {
    suites: TestSuite[];
  ...
  private setSeed(seed?: string | number): void {
    if (seed !== undefined) {
      this.seedValue = typeof seed === 'string' ?
  ...
  private getTestCaseCount(size: string): number {
    switch (size) {
      case 'small': return this.randomInt(3, 8);
      case 'medium': return this.randomInt(8, 20);
      case 'large': return this.randomInt(20, 50);
      case 'xlarge': return this.randomInt(50, 100);
      default: return this.randomInt(8, 20);
    }
  }

  private generateTestSuiteName(options: TestDataOptions): string {
    const prefixes = ['Unit Tests', 'Integration Tests', 'API Tests', 'Component Tests', 'Service Tests'];
    const suffixes = ['Suite', 'Spec', 'Test Cases', 'Validation'];
    const domains = ['User', 'Authentication', 'Database', 'Payment', 'Notification', 'Analytics'];
  ...
  private generateTestCaseName(options: TestDataOptions): string {
    const actions = ['should create', 'should update', 'should delete', 'should validate', 'should process', 'should handle'];
    const subjects = ['user data', 'authentication', 'payment', 'notification', 'file upload', 'API request'];
    const conditions = ['successfully', 'with validation', 'with error handling', 'asynchronously', 'with retries'];
  ...
  private generateFilePath(type: string, options: TestDataOptions): string {
    const directories = ['tests/unit', 'tests/integration', 'tests/e2e', 'tests/api', 'tests/component'];
    const fileNames = ['user', 'auth', 'payment', 'notification', 'analytics', 'core', 'utils'];
  ...
  private generateDuration(size: string, includeEdgeCases?: boolean): number {
    ...
    const range = baseDurations[size as keyof typeof baseDurations] || baseDurations.medium;
  ...
  private generateCategory(size: string): string {
    ...
    const options = categories[size as keyof typeof categories] || categories.medium;
    return this.randomChoice(options);
  }

  private generateCategoryEnum(size: string): TestCategory {
    switch (size) {
      case 'small': return TestCategory.Critical;
      case 'medium': return TestCategory.Optimize;
      case 'large': return TestCategory.Optimize;
      case 'xlarge': return TestCategory.Remove;
      default: return TestCategory.Optimize;
    }
  }

  private getComplexityLevel(size: string): ComplexityLevel {
    switch (size) {
      case 'small': return ComplexityLevel.Low;
      case 'medium': return ComplexityLevel.Medium;
      case 'large': return ComplexityLevel.High;
      case 'xlarge': return ComplexityLevel.High;
  ...
  private generateTestType(category: string): TestType {
    switch (category) {
      case 'fast': return TestType.Unit;
      case 'medium': return TestType.Integration;
      case 'slow': return TestType.E2E;
      default: return TestType.Unit;
    }
  }

  private generatePriority(category: string): Priority {
    switch (category) {
      case 'fast': return Priority.High;
      case 'medium': return Priority.Medium;
      case 'slow': return Priority.Low;
      default: return Priority.Medium;
    }
  }

  private generateCoverageAreas(options: TestDataOptions): string[] {
    const areas = ['auth', 'api', 'ui', 'database', 'validation', 'business-logic'];
  ...
  private mapToEffortLevel(effort: string): EffortLevel {
    switch (effort) {
      case 'low': return EffortLevel.Low;
      case 'medium': return EffortLevel.Medium;
      case 'high': return EffortLevel.High;
      default: return EffortLevel.Medium;
    }
  }
  ...
  private generateTestEnvironment(options: TestDataOptions): TestEnvironment {
    ...
    return {
      ...
      totalMemory: Math.round((os.totalmem?.() || 16 * 1024 * 1024 * 1024) / 1024 / 1024),
      availableMemory: Math.round((os.freemem?.() || 8 * 1024 * 1024 * 1024) / 1024 / 1024)
    };
  }
  ...
  private generateRollbackPlan(type: string): string {
    ...
    return rollbackPlans[type] || 'Revert changes and restore original implementation';
  }

  private generateSetupCode(category: string, options: TestDataOptions): string {
    const setupPatterns = {
      fast: [
    ...
    const patterns = setupPatterns[category as keyof typeof setupPatterns] || setupPatterns.medium;
    return this.randomChoice(patterns);
  }

  private generateTeardownCode(category: string, options: TestDataOptions): string {
    ...
    const patterns = teardownPatterns[category as keyof typeof teardownPatterns] || teardownPatterns.medium;
    return this.randomChoice(patterns);
  }
  ...
  private generateCharacteristics(category: string, size: string, options: TestDataOptions): any {
  ...
  private generateConstraints(options: TestDataOptions): string[] {
    ...
    const possibleConstraints = ['no-parallel', 'sequential', 'isolated', 'requires-admin', 'network-dependent'];
  ...
  private generateOpportunityDescription(type: string, impact: string): string {
    ...
    return descriptions[type as keyof typeof descriptions] || `${type} optimization (${impact} impact)`;
  }
  ...
  private calculateResourceReduction(impact: string): number {
    const reductions = {
      ...
      high: this.randomFloat(0.35, 0.60),
      critical: this.randomFloat(0.60, 0.85)
    };
  ...
  private generateImplementationStrategy(type: string): string {
    const strategies = {
      ...
      timeout_optimization: 'Set optimal timeouts based on test characteristics'
    };

    return strategies[type as keyof typeof strategies] || `Implement ${type} optimization`;
  }
  ...
  private generateImplementationRisks(type: string, effort: string): string[] {
    const baseRisks = ['Performance regression', 'Test instability', 'False positives'];

    if (effort === 'high') {
      baseRisks.push('High implementation complexity', 'Extended development time');
    }
  ...
  private generatePrerequisites(type: string): string[] {
    return ['Stable test suite', 'Performance baseline established', 'Test isolation verified'];
  }
  ...
  private generateValidationMetrics(type: string): string[] {
    return ['Execution time reduction', 'Resource usage improvement', 'Test stability metrics'];
  }
  ...
  private generateMeasurements(baseTime: number, baseMemory: number, size: string): any[] {
    const measurementCount = this.randomInt(5, 15);
    const measurements = [];

    for (let i = 0; i < measurementCount; i++) {
      measurements.push({
        timestamp: new Date(Date.now() - (measurementCount - i) * 24 * 60 * 60 * 1000),
  ...
  private generateBaselineName(options: TestDataOptions): string {
    const adjectives = ['Initial', 'Updated', 'Optimized', 'Baseline', 'Reference'];
