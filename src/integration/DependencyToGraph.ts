...
import type { StorageResult } from '../database';
...
export class DependencyToGraph {
  ...
  async analyzeAndStore(): Promise<IntegrationResult> {
    ...
    try {
      ...
      // 3. 그래프 데이터베이스에 저장
      const storageResult = await this.graphSystem.store(analysisResults);
   ...
   * 파일들 분석
   */
  private async analyzeFiles(files: string[]): Promise<Array<{
    filePath: string;
    language: SupportedLanguage;
    result: ParseResult;
  }>> {
