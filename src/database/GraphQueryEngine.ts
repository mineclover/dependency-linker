...
export class GraphQueryEngine {
  ...
  private async queryNodes(filter: QueryFilter): Promise<GraphNode[]> {
    return new Promise((resolve, reject) => {
      ...
      if (filter.nodeTypes?.length) {
        conditions.push(`type IN (${filter.nodeTypes.map(() => '?').join(', ')})`);
        params.push(...filter.nodeTypes);
      }

      if (filter.sourceFiles?.length) {
        conditions.push(`source_file IN (${filter.sourceFiles.map(() => '?').join(', ')})`);
        params.push(...filter.sourceFiles);
      }

      if (filter.languages?.length) {
        conditions.push(`language IN (${filter.languages.map(() => '?').join(', ')})`);
  ...
  private async queryEdges(filter: QueryFilter): Promise<GraphEdge[]> {
    return new Promise((resolve, reject) => {
      ...
      if (filter.edgeTypes?.length) {
        conditions.push(`e.type IN (${filter.edgeTypes.map(() => '?').join(', ')})`);
      ...
      if (filter.sourceFiles?.length) {
        conditions.push(`(n1.source_file IN (${filter.sourceFiles.map(() => '?').join(', ')}) OR n2.source_file IN (${filter.sourceFiles.map(() => '?').join(', ')}))`);
   ...
   * 추론된 관계 쿼리
   */
  private async queryInferred(filter: QueryFilter): Promise<InferredRelationship[]> {
    return new Promise((resolve, reject) => {
      const conditions: string[] = [];
      const params: any[] = [];

      if (filter.edgeTypes?.length) {
        conditions.push(`inferred_type IN (${filter.edgeTypes.map(() => '?').join(', ')})`);
        params.push(...filter.edgeTypes);
      }
      ...
      this.db.db?.all(sql, params, (err, rows: any[]) => {
        if (err) {
          reject(new Error(`Failed to query inferred relationships: ${err.message}`));
        } else {
  ...
  private async computeTransitiveInferences(edgeType: string): Promise<number> {
    return new Promise((resolve, reject) => {
      ...
      this.db.db?.run(sql, [edgeType, edgeType, edgeType], function(err) {
        if (err) {
          reject(new Error(`Failed to compute transitive inferences: ${err.message}`));
        } else {
   ...
   * 상속 가능한 관계 계산
   */
  private async computeInheritableInferences(edgeType: EdgeType): Promise<number> {
    ...
    return new Promise((resolve, reject) => {
      ...
      // @ts-ignore
      this.db.db?.run(sql, [edgeType.type, edgeType.parentType, edgeType.type], function(err) {
        if (err) {
          reject(new Error(`Failed to compute inheritable inferences: ${err.message}`));
        } else {
          resolve(this.changes || 0);
        }
      });
   ...
   */
  async getNodeRelationships(nodeId: number, includeInferred = true): Promise<{
    ...
    // 추론된 관계
    if (includeInferred) {
      const inferredOutgoing = await this.queryInferred({ startNodeId: nodeId });
   ...
   * 두 노드 간의 모든 경로 찾기
   */
  async findAllPaths(startNodeId: number, endNodeId: number, maxDepth = 5): Promise<{
    ...
    const queue: { nodeId: number; path: GraphEdge[] }[] = [{ nodeId: startNodeId, path: [] }];

    while (queue.length > 0 && directPaths.length < 10) { // 최대 10개 경로
