...
export class EdgeTypeManager {
        ...
        rows.forEach(row => {
          try {
            ...
            const edgeType: EdgeTypeDefinition = {
              ...
              conflictResolution: metadata.conflictResolution || 'priority_based'
            };
   ...
   * 새로운 엣지 타입 생성
   */
  async createEdgeType(definition: EdgeTypeDefinition): Promise<EdgeTypeOperationResult> {
    try {
      // 1. 유효성 검증
      ...
      if (definition.inferenceRules) {
        affectedInferences = await this.inferenceEngine.activateRules(definition.inferenceRules);
      }

      // 6. 기존 데이터에 대한 소급 추론 실행
      if (definition.isTransitive || definition.isInheritable) {
        await this.inferenceEngine.computeRetroactiveInferences(definition.type);
      }
      ...
      return {
        success: false,
        errors: [`Failed to create edge type: ${error instanceof Error ? error.message : String(error)}`]
      };
    }
  }
  ...
  async updateEdgeType(
    ...
    try {
      ...
      let affectedInferences = 0;
      if (impactAnalysis.requiresRecomputation) {
        affectedInferences = await this.inferenceEngine.recomputeInferences(type);
      }
      ...
      return {
        success: false,
        errors: [`Failed to update edge type: ${error instanceof Error ? error.message : String(error)}`]
      };
    }
  }
   ...
   * 엣지 타입 삭제
   */
  async deleteEdgeType(type: string, force = false): Promise<EdgeTypeOperationResult> {
    try {
      // 의존성 체크
      ...
      return {
        success: false,
        errors: [`Failed to delete edge type: ${error instanceof Error ? error.message : String(error)}`]
      };
    }
  }
  ...
  private async validateEdgeType(definition: EdgeTypeDefinition): Promise<ValidationResult> {
    const errors: string[] = [];
    ...
    if (!/^[a-z][a-z0-9_]*$/.test(definition.type)) {
      errors.push('Edge type name must start with lowercase letter and contain only lowercase letters, numbers, and underscores');
    }
  ...
  private async validateHierarchy(definition: EdgeTypeDefinition): Promise<ValidationResult> {
    const errors: string[] = [];

    if (definition.parentType) {
      const parent = this.edgeTypes.get(definition.parentType);
      if (!parent) {
        errors.push(`Parent edge type '${definition.parentType}' does not exist`);
      } else {
        // 순환 참조 검사
        if (await this.wouldCreateCycle(definition.type, definition.parentType)) {
  ...
  private async wouldCreateCycle(childType: string, parentType: string): Promise<boolean> {
      ...
      if (edgeType.inferenceRules) {
        for (const rule of edgeType.inferenceRules) {
          if (rule.action.targetEdgeType === type) {
            dependencies.push(`Inference rule: ${rule.name} in ${edgeType.type}`);
          }
        }
      }
    }
...
export class InferenceEngine {
  ...
  private async evaluateCondition(condition: InferenceCondition, edge: any): Promise<boolean> {
  ...
  private evaluatePathCondition(condition: InferenceCondition, edge: any): boolean {
    ...
    if (condition.nodeTypeConstraints) {
      ...
      if (!condition.nodeTypeConstraints.includes(fromNodeType) ||
  ...
  private evaluatePatternCondition(condition: InferenceCondition, edge: any): boolean {
    if (!condition.pattern) return false;
  ...
  private evaluateCustomCondition(condition: InferenceCondition, edge: any): boolean {
  ...
  private async executeAction(action: InferenceAction, edge: any): Promise<void> {
    switch (action.type) {
      case 'create_edge':
  ...
  private async createInferredEdge(action: InferenceAction, sourceEdge: any): Promise<void> {
  ...
  private async updateEdgeMetadata(action: InferenceAction, edge: any): Promise<void> {
  ...
  private async aggregateEdgeData(action: InferenceAction, edge: any): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      this.database.get(sql, [edge.type, edge.startNodeId, edge.endNodeId],
        async (err: Error | null, row: any) => {
          ...
          switch (action.aggregationMethod) {
            case 'count': aggregatedValue = row.count; break;
            case 'sum': aggregatedValue = row.total_weight; break;
            case 'avg': aggregatedValue = row.avg_weight; break;
            case 'max': aggregatedValue = row.max_weight; break;
            case 'min': aggregatedValue = row.min_weight; break;
            default: aggregatedValue = row.count;
          }
  ...
  private async executeCustomAction(action: InferenceAction, edge: any): Promise<void> {
    if (!action.customFunction) return;

    try {
      // 안전한 커스텀 액션 실행 (실제 구현에서는 sandbox 필요)
      const func = new Function('edge', 'database', 'action', action.customFunction);
