...
export class GraphStorage {
	...
	constructor(options: StorageOptions) {
		this.options = {
			...
			sessionName:
				options.sessionName || `Analysis-${new Date().toISOString()}`,
			dbPath:
				options.dbPath ||
				join(options.projectRoot, ".dependency-linker", "graph.db"),
		};
				...
				if (result.imports?.length) {
					for (const importSource of result.imports) {
						...
						const importNode: GraphNode = {
							...
							sourceFile: this.isExternalPackage(importSource)
								? importSource
								: this.normalizeImportPath(importSource, filePath),
							language: this.isExternalPackage(importSource)
								? "external"
								: language,
							metadata: {
	...
	private async updateSessionStats(
      ...
      `;
			...
			// @ts-expect-error - db는 private이지만 접근 필요
	...
	async getFileDependencies(filePath: string): Promise<{
		...
		const dependents = await this.db.findNodeDependents(fileNode.id, [
			"imports",
		]);
	...
	async findDependencyPath(
		...
		if (!fromNodes[0].id || !toNodes[0].id) return null;
