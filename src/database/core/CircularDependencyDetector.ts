...
export class CircularDependencyDetector {
	...
	async detectFromNode(
		...
		try {
			...
			this.stats.processingTime = Date.now() - this.startTime;
			throw new Error(
				`Circular dependency detection from node failed: ${error}`,
			);
		}
	}
	...
	private async dfs(
		...
		try {
			...
			for (const edge of relevantEdges) {
				...
				if (
					targetNodeType &&
					this.options.excludeNodeTypes.includes(targetNodeType)
				) {
	...
	private normalizeCycle(nodes: string[]): string[] {
		...
		const cycle =
			nodes[nodes.length - 1] === nodes[0] ? nodes.slice(0, -1) : nodes;

		if (cycle.length === 0) return cycle;

		// 사전순으로 가장 작은 노드를 시작점으로
		const minNode = cycle.reduce((min, current) =>
