...
export class NodeIdentifier {
  ...
  createIdentifier(
    ...
    switch (type) {
      ...
      case 'type':
      case 'enum':
        return this.createTypeIdentifier(type, name, normalizedContext, location);

      case 'method':
      case 'function':
        return this.createFunctionIdentifier(type, name, normalizedContext, location, metadata);

      case 'variable':
      case 'constant':
      case 'property':
      case 'parameter':
        return this.createVariableIdentifier(type, name, normalizedContext, location, metadata);

      default:
        return this.createGenericIdentifier(type, name, normalizedContext, location);
    }
  }
  ...
  parseIdentifier(identifier: string): Partial<UniqueNodeIdentity> | null {
    try {
      // 기본 유효성 검사
      if (!identifier || identifier.trim() === '' || !identifier.includes('#')) {
        return null;
      }
  ...
  validateIdentifier(identifier: string): boolean {
    try {
      ...
      // 유효한 노드 타입인지 확인
      const validTypes = [
        'file', 'class', 'interface', 'function', 'method', 'variable',
        'constant', 'property', 'import', 'export', 'library', 'package',
        'lib', 'pkg'
      ];
  ...
  private createDirectoryIdentifier(name: string, context: NodeContext): string {
  ...
  private createExternalIdentifier(type: 'library' | 'package', name: string): string {
  ...
  private createImportIdentifier(
    ...
    const hash = this.createHash(`${context.sourceFile}::import::${importPath}`);
...
export function createStandardNode(
  ...
  return {
    identifier: identifier.createIdentifier(type, name, context, location, metadata),
    type,
    name,
    context,
    location,
