...
export class NodeCentricAnalyzer {
  ...
  async analyzeNodeNeighborhood(
    ...
    // 클러스터 분석
    const allNeighbors = [...immediate.incoming, ...immediate.outgoing, ...level2, ...level3];
  ...
  async analyzeNodeEvolution(
    ...
    // 리팩토링 위험도 평가
    const refactoringRisk = this.assessRefactoringRisk(changeFrequency, impactRadius, stabilityScore);

    // 추천사항 생성
    const recommendations = this.generateRecommendations(node, changeFrequency, impactRadius, stabilityScore);
  ...
  async findShortestPath(
    ...
    const path = await this.db.findDependencyPath(fromNode.id!, toNode.id!, opts.maxDepth);
    ...
    return path.nodes.map((node, index) => ({
      ...
      type: node.type,
      name: node.name,
      relationshipType: index === 0 ? 'start' : index === path.nodes.length - 1 ? 'end' : 'intermediate',
      distance: index,
      weight: 1,
    }));
  }
  ...
  private async analyzeDependencies(
    ...
    const direct = await this.db.findNodeDependencies(nodeId, options.edgeTypes);
  ...
  private async calculateNodeMetrics(
    ...
    const criticalityScore = this.calculateCriticalityScore(fanIn, fanOut, instability);
  ...
  private async analyzeNodeRisks(
    ...
    const circularResult = await detector.detectFromNode(
      nodeId.toString(),
      async (id) => {
        const edges = await this.db.findNodeDependencies(parseInt(id), options.edgeTypes);
        return edges.map(node => ({ to: node.id!.toString(), type: 'dependency' }));
      }
    );

    const circularDependencies: CircularRisk[] = circularResult.cycles.map(cycle => ({
      cycle: cycle.nodes,
      depth: cycle.depth,
  ...
  private async getTransitiveDependencies(
    ...
    const dfs = async (currentId: number, distance: number): Promise<void> => {
      ...
      const dependencies = await this.db.findNodeDependencies(currentId, options.edgeTypes);
  ...
  private async getTransitiveDependents(
    ...
    const dfs = async (currentId: number, distance: number): Promise<void> => {
      ...
      const dependents = await this.db.findNodeDependents(currentId, options.edgeTypes);
  ...
  private async getImmediateNeighbors(
    ...
    return {
      incoming: dependents.map(node => this.nodeToReference(node, 'dependent', 1)),
      outgoing: dependencies.map(node => this.nodeToReference(node, 'dependency', 1)),
    };
  }
  ...
  private async getNeighborsAtDistance(
    ...
    // BFS로 특정 거리의 노드들 찾기
    const visited = new Set<number>();
    const queue: Array<{ id: number; dist: number }> = [{ id: nodeId, dist: 0 }];
    const result: NodeReference[] = [];
    ...
    while (queue.length > 0) {
      ...
      if (dist < distance) {
        const neighbors = await this.db.findNodeDependencies(id, options.edgeTypes);
  ...
  private calculateCriticalityScore(fanIn: number, fanOut: number, instability: number): number {
    return (fanIn + fanOut) * (1 - instability);
  }

  private assessCycleSeverity(depth: number, weight: number): 'low' | 'medium' | 'high' {
  ...
  private async calculateImpactRadius(nodeId: number, options: Required<NodeAnalysisOptions>): Promise<number> {
    const transitiveDependents = await this.getTransitiveDependents(nodeId, options);
    return Math.max(...transitiveDependents.map(ref => ref.distance), 0);
  }

  private async calculateStabilityScore(nodeId: number, options: Required<NodeAnalysisOptions>): Promise<number> {
    const [dependencies, dependents] = await Promise.all([
      this.db.findNodeDependencies(nodeId, options.edgeTypes),
      this.db.findNodeDependents(nodeId, options.edgeTypes),
    ]);
  ...
  private generateRecommendations(
    ...
    if (changeFrequency > 0.7) {
      recommendations.push('Consider stabilizing this frequently changing component');
    }

    if (impactRadius > 5) {
      recommendations.push('High impact radius - changes affect many components');
    }

    if (stabilityScore < 0.3) {
      recommendations.push('Low stability - consider reducing outgoing dependencies');
    }
