 ...
 */
export function getUnknownSymbolManager(): UnknownSymbolManager {
	if (!unknownSymbolManager) {
		unknownSymbolManager = new UnknownSymbolManager();
	}
	return unknownSymbolManager;
}

/**
 * File Dependency Analyzer 싱글톤 인스턴스 반환
 */
export function getFileDependencyAnalyzer(): FileDependencyAnalyzer {
...
export async function initializeAll(): Promise<void> {
	const unknownManager = getUnknownSymbolManager();
