...
export class EdgeTypeRegistry {
  ...
  static initialize(): void {
    ...
    // Core types 등록
    for (const def of this.CORE_TYPES) {
      this.definitions.set(def.type, def);
    }

    // Extended types 등록
    for (const def of this.EXTENDED_TYPES) {
      this.definitions.set(def.type, def);
    }
  }
  ...
  static getHierarchyPath(type: string): string[] {
    ...
    let current = this.definitions.get(type);

    while (current?.parentType) {
      path.push(current.parentType);
      current = this.definitions.get(current.parentType);
    }
  ...
  static validateHierarchy(): { valid: boolean; errors: string[] } {
    ...
    for (const def of this.definitions.values()) {
      if (def.parentType) {
        // Parent가 존재하는지 확인
        if (!this.definitions.has(def.parentType)) {
          errors.push(`${def.type}: parent type '${def.parentType}' does not exist`);
        }

        // 순환 참조 확인
        const path = this.getHierarchyPath(def.type);
  ...
  static printHierarchy(): string {
    ...
    const printNode = (type: string, indent: number = 0) => {
      ...
      // Children
      const children = this.getChildTypes(type);
    ...
    const roots = Array.from(this.definitions.values()).filter(
      def => !def.parentType
    );
