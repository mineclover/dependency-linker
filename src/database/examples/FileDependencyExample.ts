  ...
  return {
    ...
    '/src/App.tsx': [
      ...
      },
      {
        ...
        imports: [
          { name: 'UserProfile', isDefault: true, isNamespace: false }
        ],
    ...
    '/src/components/Header.tsx': [
      ...
      },
      {
        ...
        imports: [
          { name: 'useAuth', isDefault: true, isNamespace: false }
        ],
        location: { line: 2, column: 1 }
      },
      {
    ...
    '/src/components/UserProfile.tsx': [
      ...
      },
      {
        ...
        source: './Avatar',
        imports: [
          { name: 'Avatar', isDefault: true, isNamespace: false }
        ],
        location: { line: 4, column: 1 }
      },
      {
    ...
    '/src/hooks/useAuth.ts': [
      ...
      },
      {
        ...
        imports: [
          { name: 'localStorage', alias: 'storage', isDefault: false, isNamespace: false }
        ],
        location: { line: 3, column: 1 }
      },
      {
    ...
    '/src/components/Avatar.tsx': [
      ...
      },
      {
        ...
        source: 'classnames',
        imports: [
          { name: 'classNames', isDefault: true, isNamespace: false }
        ],
        location: { line: 2, column: 1 }
      }
    ]
...
export async function runFileDependencyAnalysisExample(
  ...
  const totalStats = results.reduce(
    (acc, result) => ({
      ...
      totalLibraryImports: acc.totalLibraryImports + result.stats.libraryImports,
  ...
  if (allMissingLinks.length > 0) {
    ...
    const missingLinksByType = allMissingLinks.reduce((acc, link) => {
...
async function demonstrateQueries(database: GraphDatabase): Promise<void> {
  ...
  if (appNode?.id) {
    console.log('\n4. 📁 App.tsx의 직접 의존성:');
    const dependencies = await database.findNodeDependencies(
      appNode.id,
  ...
  // 5. 미싱 링크 조회
  console.log('\n5. ❌ 존재하지 않는 파일들:');
  const missingFiles = fileNodes.filter(node => node.metadata?.exists === false);
...
export async function simulateFileUpdate(
  ...
  await analyzer.analyzeFile('/src/UpdatedFile.tsx', 'typescript', originalImports);
  ...
  const result = await analyzer.analyzeFile('/src/UpdatedFile.tsx', 'typescript', updatedImports);
...
export async function simulateLargeProjectAnalysis(
  ...
  for (let i = 1; i <= 100; i++) {
    ...
    if (i > 1 && i % 3 === 0) {
      imports.push({
        ...
        imports: [{ name: `Component${i - 1}`, isDefault: true, isNamespace: false }],
        location: { line: 2, column: 1 }
      });
    }
    ...
    filePromises.push(
      analyzer.analyzeFile(filePath, 'typescript', imports)
    );
  }
  ...
  const totalStats = results.reduce(
    (acc, result) => ({
      totalFiles: acc.totalFiles + 1,
      totalImports: acc.totalImports + result.stats.totalImports,
      totalRelationships: acc.totalRelationships + result.createdRelationships.length
    }),
  ...
  console.log(`   처리율: ${Math.round(totalStats.totalFiles / (duration / 1000))} 파일/초`);
}