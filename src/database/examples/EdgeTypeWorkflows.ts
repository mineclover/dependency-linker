...
export async function createProjectAnalysisWorkflow(
	...
	availableTypes.forEach((type) => {
		console.log(
			`  - ${type.type}: ${type.description} (우선순위: ${type.priority})`,
		);
	});
}
...
export async function demonstrateDynamicEdgeTypeManagement(
	...
	const deleteResult = await edgeTypeManager.deleteEdgeType(
		"depends_on",
		false,
	);
	...
	if (!deleteResult.success) {
		...
		// 강제 삭제
		const forceDeleteResult = await edgeTypeManager.deleteEdgeType(
			"depends_on",
			true,
		);
 ...
 * 통합 예제 실행 함수
 */
export async function runAllEdgeTypeWorkflows(
	database: GraphDatabase,
): Promise<void> {
