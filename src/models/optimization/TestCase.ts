...
import type { TestCase } from "./TestSuite";
import { Priority, TestType } from "./types";
...
// TestCaseAnalyzer functionality converted to individual functions

/**
 * Analyze a test case for optimization opportunities
 */
export function analyzeTestCase(
	testCase: TestCase,
	allTestCases: TestCase[],
): TestCaseAnalysis {
	const metrics = calculateTestCaseMetrics(testCase);
	const dependencies = analyzeTestCaseDependencies(testCase);
	const duplicateScore = calculateTestCaseDuplicateScore(
		testCase,
		allTestCases,
	);
	const optimizationPotential = assessTestCaseOptimizationPotential(
		testCase,
		metrics,
		duplicateScore,
	);
 ...
 */
export function findSimilarTests(
	target: TestCase,
	candidates: TestCase[],
): TestCase[] {
	return candidates.filter((candidate) => {
		...
		const namesSimilarity = calculateStringSimilarity(
			target.name,
			candidate.name,
		);
		const coverageOverlap = calculateArrayOverlap(
...
export function identifyConsolidationCandidates(
	...
	testCases.forEach((testCase) => {
		...
		const similar = findSimilarTests(testCase, testCases);
		if (similar.length > 0) {
...
function calculateTestCaseMetrics(testCase: TestCase): TestCaseMetrics {
...
function analyzeTestCaseDependencies(testCase: TestCase): TestCaseDependency[] {
	...
	if (
		nameWords.some((word) =>
			["api", "http", "network", "request"].includes(word),
		)
	) {
		dependencies.push({
			testCaseId: testCase.id,
...
function calculateTestCaseDuplicateScore(
	testCase: TestCase,
	allTestCases: TestCase[],
): number {
	...
	allTestCases.forEach((other) => {
		...
		const nameSimilarity = calculateStringSimilarity(testCase.name, other.name);
		const coverageSimilarity = calculateArrayOverlap(
...
function assessTestCaseOptimizationPotential(
	testCase: TestCase,
	metrics: TestCaseMetrics,
	...
	if (testCase.executionTime > 1000) {
		// >1 second
 ...
 */
export function findSlowestTestCases(
	testCases: TestCase[],
	count: number = 10,
): TestCase[] {
	return [...testCases]
 ...
 */
export function calculateTestCasesTotalTime(testCases: TestCase[]): number {
 ...
 */
export function calculateTestCasesFailureRate(testCases: TestCase[]): number {
 ...
 */
export function validateTestCases(testCases: TestCase[]): string[] {
	const errors: string[] = [];
	...
	testCases.forEach((testCase, index) => {
		...
		if (testCase.executionTime <= 0) {
			errors.push(
				`Test case ${testCase.id} has invalid execution time: ${testCase.executionTime}`,
			);
		}

		// Validate duplicate reference
		if (
			testCase.duplicateOf &&
			!testCases.some((tc) => tc.id === testCase.duplicateOf)
		) {
			errors.push(
				`Test case ${testCase.id} references non-existent duplicate: ${testCase.duplicateOf}`,
			);
		}
	});
