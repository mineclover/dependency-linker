...
export class TestCaseAnalyzer {
	...
	static analyze(
		...
		const metrics = TestCaseAnalyzer.calculateMetrics(testCase);
		const dependencies = TestCaseAnalyzer.analyzeDependencies(testCase);
		const duplicateScore = TestCaseAnalyzer.calculateDuplicateScore(
			testCase,
			allTestCases,
		);
		const optimizationPotential = TestCaseAnalyzer.assessOptimizationPotential(
			testCase,
			metrics,
			duplicateScore,
		);
	...
	static findSimilarTests(
		...
		return candidates.filter((candidate) => {
			...
			const namesSimilarity = TestCaseAnalyzer.calculateStringSimilarity(
				target.name,
				candidate.name,
			);
			const coverageOverlap = TestCaseAnalyzer.calculateArrayOverlap(
	...
	static identifyConsolidationCandidates(testCases: TestCase[]): TestCase[][] {
		...
		testCases.forEach((testCase) => {
			...
			const similar = TestCaseAnalyzer.findSimilarTests(testCase, testCases);
			if (similar.length > 0) {
	...
	private static calculateDuplicateScore(
		testCase: TestCase,
		allTestCases: TestCase[],
	): number {
		...
		allTestCases.forEach((other) => {
			...
			const nameSimilarity = TestCaseAnalyzer.calculateStringSimilarity(
				testCase.name,
				other.name,
			);
			const coverageSimilarity = TestCaseAnalyzer.calculateArrayOverlap(
