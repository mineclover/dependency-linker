...
export class PerformanceBaselineBuilder {
  ...
  withTestCounts(total: number, failed: number, failedSuites: number): PerformanceBaselineBuilder {
  ...
  withIssues(workerIssues: boolean, parserWarnings: number): PerformanceBaselineBuilder {
    this.baseline.workerIssues = workerIssues;
    this.baseline.parserWarnings = parserWarnings;
    return this;
  }

  withEnvironment(environment: Partial<TestEnvironment>): PerformanceBaselineBuilder {
    this.baseline.environment = {
      ...
      totalMemory: environment.totalMemory || require('os').totalmem() / 1024 / 1024,
      availableMemory: environment.availableMemory || require('os').freemem() / 1024 / 1024
    };
    return this;
  }

  withMetadata(metadata: Partial<BaselineMetadata>): PerformanceBaselineBuilder {
    this.baseline.metadata = {
      ...this.baseline.metadata!,
      ...metadata
    };
  ...
  private validateBaseline(): void {
    if (this.baseline.totalExecutionTime === undefined || this.baseline.totalExecutionTime < 0) {
      throw new Error('Total execution time must be non-negative');
    }
    if (this.baseline.totalTests === undefined || this.baseline.totalTests < 0) {
      throw new Error('Total tests must be non-negative');
    }
    if (this.baseline.failedTests === undefined || this.baseline.failedTests < 0) {
...
export class PerformanceAnalyzer {
  ...
  static comparePerformance(
    ...
    const timeImprovement = this.calculatePercentageChange(
    ...
    const coverageChange = current.coveragePercentage - baseline.coveragePercentage;

    const memoryImprovement = this.calculatePercentageChange(
    ...
    const details = this.evaluateTargets(current, targets);
    ...
    return {
      ...
      coverageChange,
      memoryImprovement,
      meetsTargets: this.evaluateOverallSuccess(current, targets),
      details
    };
  }
  ...
  static evaluateOptimizationSuccess(
    ...
    const actualTargets = targets || this.getDefaultTargets();
  ...
  static generateRecommendations(baseline: PerformanceBaseline): string[] {
    ...
    if (baseline.parserWarnings > 0) {
      recommendations.push(`Eliminate ${baseline.parserWarnings} parser registration warnings`);
    }

    if (baseline.memoryUsage > 100) {
      recommendations.push(`Reduce memory usage from ${baseline.memoryUsage.toFixed(1)}MB to under 100MB`);
    }
  ...
  static trackTrends(baselines: PerformanceBaseline[]): PerformanceTrend {
    ...
    const sorted = [...baselines].sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
    const oldest = sorted[0];
    const newest = sorted[sorted.length - 1];

    return {
      timespan: newest.timestamp.getTime() - oldest.timestamp.getTime(),
      executionTimeChange: this.calculatePercentageChange(
      ...
      direction: this.determineTrendDirection(oldest, newest),
  ...
  private static evaluateTargets(
    ...
    const reliabilityMet = (1 - current.failureRate) * 100 >= targets.targetSuiteReliability;

    const improvements: string[] = [];
    const regressions: string[] = [];

    if (executionTimeMet) improvements.push('Execution time target achieved');
    else regressions.push(`Execution time ${current.executionTime}ms exceeds target ${targets.targetExecutionTime}ms`);

    if (testCountMet) improvements.push('Test count target achieved');
    else regressions.push(`Test count ${current.testCount} exceeds target ${targets.targetTests}`);

    if (passRateMet) improvements.push('Pass rate target achieved');
    else regressions.push(`Pass rate ${current.passRate.toFixed(1)}% below target ${targets.targetPassRate}%`);

    if (coverageMet) improvements.push('Coverage target maintained');
    else regressions.push(`Coverage ${current.coveragePercentage.toFixed(1)}% below minimum 80%`);

    if (memoryMet) improvements.push('Memory usage target achieved');
    else regressions.push(`Memory usage ${current.memoryUsage.toFixed(1)}MB exceeds target ${targets.targetMemoryUsage}MB`);
  ...
  private static determineTrendDirection(
    ...
    else if (newest.totalExecutionTime > oldest.totalExecutionTime) degradations++;
    ...
    else if (newest.coveragePercentage < oldest.coveragePercentage) degradations++;
