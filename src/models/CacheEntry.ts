...
export type CacheSource = 'ast' | 'analysis' | 'extractor' | 'interpreter' | 'external';
...
export class CacheEntryUtils {
  ...
  static createKey(filePath: string, config?: any): string {
    const configHash = config ? this.hashObject(config) : 'default';
   ...
   */
  static isValid(entry: CacheEntry, currentFileMetadata?: FileMetadata): boolean {
    ...
    return (
      entry.fileMetadata.lastModified.getTime() === currentFileMetadata.lastModified.getTime() &&
  ...
  static createAnalysisEntry(
    ...
    const size = this.calculateSize(analysisData);
   ...
   */
  static compress(entry: CacheEntry, algorithm: CompressionInfo['algorithm'] = 'gzip'): CacheEntry {
    ...
    const compressionRatio = this.estimateCompressionRatio(originalData, algorithm);
   ...
   */
  private static estimateCompressionRatio(data: string, algorithm: CompressionInfo['algorithm']): number {
    const repetitionScore = this.calculateRepetitionScore(data);
