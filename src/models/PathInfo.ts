...
export function createPathInfo(
	...
	const normalizedProjectRootPath = normalizedProjectRoot.replace(
		/[/\\]/g,
		sep,
	);
	...
	if (isWithinProject && relativePath !== "." && relativePath !== "") {
		...
		const pathSegments = normalizedForDepth
			.split("/")
 ...
 */
export function groupPathInfoByDirectory(
	pathInfos: PathInfo[],
): Map<string, PathInfo[]> {
	...
	for (const pathInfo of pathInfos) {
		...
		const separator =
			pathInfo.separator || (process.platform === "win32" ? "\\" : "/");
		const dir =
			pathInfo?.relativeDirectory ||
...
export function filterPathInfo(
	...
	return pathInfos.filter((pathInfo) => {
		...
		if (
			...
			pathInfo.isWithinProject !== withinProject
		) {
			return false;
		}
		...
		if (directories) {
			const normalizedRelativeDir = pathInfo.relativeDirectory.replace(
				/[/\\]/g,
				"/",
			);
