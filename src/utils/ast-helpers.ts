 ...
 * 소스 코드에서 실제 오프셋 계산
 */
export function calculateOffsets(sourceCode: string, location: ExtendedSourceLocation): ExtendedSourceLocation {
 ...
 * 첫 번째 매칭 노드 찾기
 */
export function findFirstNodeByType(root: ASTNode, nodeType: string): ASTNode | undefined {
  for (const node of traverseNodes(root)) {
    if (node.type === nodeType) {
      return node;
    }
 ...
 * 노드 깊이 계산
 */
export function getNodeDepth(node: ASTNode, root: ASTNode): number {
  let depth = 0;
  let current = node;
...
export function isNodeInside(inner: ASTNode, outer: ASTNode): boolean {
  ...
  return (
    (outerStart.row < innerStart.row ||
     (outerStart.row === innerStart.row && outerStart.column <= innerStart.column)) &&
...
export function nodesOverlap(node1: ASTNode, node2: ASTNode): boolean {
  ...
  return !(
    (end1.row < start2.row || (end1.row === start2.row && end1.column <= start2.column)) ||
    (end2.row < start1.row || (end2.row === start1.row && end2.column <= start1.column))
  );
}
