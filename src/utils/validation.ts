...
import type { BaseQueryResult, ValidationResult } from "../core/types";
 ...
 * 기본 쿼리 결과 검증
 */
export function validateBaseQueryResult(
	result: unknown,
): result is BaseQueryResult {
	return (
...
export function validateQueryResults(
	...
	for (let i = 0; i < results.length; i++) {
		...
		if (!validateBaseQueryResult(result)) {
			errors.push(
				`Result ${i} for query "${queryKey}" is not a valid BaseQueryResult`,
			);
			continue;
		}
		...
		if (result.queryName !== queryKey) {
			errors.push(
				`Result ${i} has queryName "${result.queryName}" but expected "${queryKey}"`,
			);
		}
		...
		if (!result.location) {
			...
			if (locationErrors.length > 0) {
				errors.push(
					`Result ${i} location invalid: ${locationErrors.join(", ")}`,
				);
			}
		}
		...
		if (!result.nodeText || typeof result.nodeText !== "string") {
			warnings.push(
				`Result ${i} for query "${queryKey}" has empty or invalid nodeText`,
			);
		}
	}
...
export function validateLocation(location: unknown): string[] {
	...
	if (
		...
		(typeof loc.line === "number" && loc.endLine < loc.line)
	) {
		errors.push("Invalid endLine number");
	}

	if (
		typeof loc.endColumn !== "number" ||
...
export function validateQueryKeyFormat(queryKey: string): ValidationResult {
	...
	if (!keyPattern.test(queryKey)) {
		errors.push(
			`Query key "${queryKey}" does not follow naming convention (language-domain-target)`,
		);
	}
	...
	if (queryKey.length > 50) {
		warnings.push(
			`Query key "${queryKey}" is very long (${queryKey.length} characters)`,
		);
	}
	...
	if (parts.length >= 2 && validLanguagePrefixes.includes(parts[0])) {
		...
		if (parts.length < 2) {
			warnings.push(
				`Language-specific query "${queryKey}" should have at least domain part`,
			);
		}
	}
 ...
 * 쿼리 키 충돌 검증
 */
export function validateQueryKeyConflicts(
	queryKeys: string[],
): ValidationResult {
	const errors: string[] = [];
	const warnings: string[] = [];
	...
	if (duplicates.length > 0) {
		errors.push(
			`Duplicate query keys found: ${[...new Set(duplicates)].join(", ")}`,
		);
	}
	...
	for (let i = 0; i < queryKeys.length; i++) {
		for (let j = i + 1; j < queryKeys.length; j++) {
			...
			if (similarity > 0.8 && key1 !== key2) {
				warnings.push(
					`Query keys "${key1}" and "${key2}" are very similar (${Math.round(similarity * 100)}% similar)`,
				);
			}
		}
	}
...
export function validateLanguageSupport(
	...
	if (invalidLanguages.length > 0) {
		errors.push(
			`Query "${queryKey}" has invalid languages: ${invalidLanguages.join(", ")}`,
		);
	}
	...
	if (keyParts.length >= 2) {
		...
		if (languageMap[keyLanguage]) {
			...
			const hasCorrectLanguages = expectedLanguages.every((lang) =>
				supportedLanguages.includes(lang),
			);

			if (!hasCorrectLanguages) {
				warnings.push(
					`Query "${queryKey}" language prefix "${keyLanguage}" doesn't match supported languages`,
				);
			}
		}
	}
