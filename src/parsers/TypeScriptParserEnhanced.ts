...
import { ImportInfo, ImportSpecifier } from '../models/ImportInfo';
...
export class TypeScriptParserEnhanced {
  ...
  parseFile(content: string): {
    ...
    if (content.length === 0) {
      ...
      return { dependencies: [], imports: [], exports: [], hasParseErrors: false };
    }
    ...
    try {
      ...
      throw new Error(`Failed to set parser language: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    try {
      ...
      throw new Error(`Parser.parse() failed: ${error instanceof Error ? error.message : String(error)}`);
    }
    
    if (!tree) {
      throw new Error('Parser failed to create syntax tree - parse() returned null');
    }
    
    const rootNode = tree.rootNode;
    if (!rootNode) {
      throw new Error('Parser failed to create root node - tree.rootNode is null');
    }
   ...
   */
  private extractImportInfo(node: Parser.SyntaxNode, content: string): ImportInfo | null {
    const sourceNode = node.childForFieldName('source');
    ...
    const importClause = node.childForFieldName('import') || node.children.find(child => child.type === 'import_clause');
    if (importClause) {
      // Extract named imports
      const namedImports = this.findNodesByType(importClause, 'import_specifier');
      for (const spec of namedImports) {
        const nameNode = spec.childForFieldName('name') || spec.children.find(child => child.type === 'identifier');
        const aliasNode = spec.childForFieldName('alias');
        
        // For aliased imports: identifier "as" identifier
        const identifiers = spec.children.filter(child => child.type === 'identifier');
        
        if (identifiers.length >= 2 && spec.children.some(child => child.text === 'as')) {
      ...
      const directIdentifier = importClause.children.find(child => child.type === 'identifier');
      if (directIdentifier) {
        specifiers.push({
          type: 'default',
          imported: 'default',
      ...
      if (namespaceImport) {
        const aliasNode = namespaceImport.childForFieldName('name') || namespaceImport.children.find(child => child.type === 'identifier');
        if (aliasNode) {
          specifiers.push({
            type: 'namespace',
            imported: '*',
   ...
   */
  private extractRequireCall(node: Parser.SyntaxNode, content: string): DependencyInfo | null {
    ...
    const firstArg = argumentsNode.children.find(child => child.type === 'string');
   ...
   */
  private extractDynamicImport(node: Parser.SyntaxNode, content: string): DependencyInfo | null {
    const argumentsNode = node.childForFieldName('arguments');
    if (!argumentsNode) return null;
   ...
   */
  private extractExportInfo(node: Parser.SyntaxNode, content: string): ExportInfo[] {
    ...
    const lexicalDeclaration = node.children.find(child => child.type === 'lexical_declaration');
    const interfaceDeclaration = node.children.find(child => child.type === 'interface_declaration');
    const functionDeclaration = node.children.find(child => child.type === 'function_declaration');
    const classDeclaration = node.children.find(child => child.type === 'class_declaration');
    const typeAliasDeclaration = node.children.find(child => child.type === 'type_alias_declaration');
    ...
    if (declarationNode || lexicalDeclaration || interfaceDeclaration || functionDeclaration || classDeclaration || typeAliasDeclaration) {
      // Export declaration (export const, export function, etc.)
      const actualDeclaration = declarationNode || lexicalDeclaration || interfaceDeclaration || functionDeclaration || classDeclaration || typeAliasDeclaration;
      const exportInfo = this.extractExportDeclaration(actualDeclaration!, node);
   ...
   */
  private extractExportDeclaration(declarationNode: Parser.SyntaxNode, exportNode: Parser.SyntaxNode): ExportInfo | null {
    let name = 'default';
    ...
    if (declarationNode.type === 'function_declaration' || declarationNode.type === 'class_declaration') {
      ...
      const nameNode = declarationNode.children.find(child => child.type === 'type_identifier');
      if (nameNode) {
        name = nameNode.text;
      }
      ...
      const declarators = this.findNodesByType(declarationNode, 'variable_declarator');
      if (declarators.length > 0) {
        const nameNode = declarators[0].children.find(child => child.type === 'identifier');
        if (nameNode) {
          name = nameNode.text;
        }
      }
   ...
   */
  private extractReExports(exportNode: Parser.SyntaxNode, sourceNode: Parser.SyntaxNode): ExportInfo[] {
    const source = sourceNode.text.replace(/['"`]/g, '');
    const exports: ExportInfo[] = [];
    ...
    if (hasWildcard) {
      ...
      const namespaceExport = this.findNodesByType(exportNode, 'namespace_export')[0];
      if (namespaceExport) {
        const nameNode = namespaceExport.children.find(child => child.type === 'identifier');
        if (nameNode) {
          exports.push({
            name: nameNode.text,
            type: 're-export',
    ...
    const exportSpecifiers = this.findNodesByType(exportNode, 'export_specifier');
   ...
   */
  private extractImportMetadata(node: Parser.SyntaxNode, content: string): Record<string, any> {
    ...
    if (importClause) {
      ...
      if (namedImports.length > 0) {
        ...
        metadata.members = namedImports.map(spec => {
          const name = spec.childForFieldName('name');
          const alias = spec.childForFieldName('alias');
          return alias ? `${name?.text} as ${alias.text}` : name?.text || '';
        });
      } else if (namespaceImport) {
        metadata.importType = 'namespace';
        const alias = namespaceImport.childForFieldName('name');
        if (alias) {
          metadata.alias = alias.text;
        }
      } else if (defaultImport) {
        metadata.importType = 'default';
        metadata.isDefault = true;
      } else {
        metadata.importType = 'side-effect';
      }
    }

    if (this.isTypeOnlyImport(node)) {
   ...
   */
  private classifyDependencyType(source: string): 'external' | 'internal' | 'relative' {
  ...
  private isTypeOnlyExport(node: Parser.SyntaxNode): boolean {
    ...
    const hasInterfaceDeclaration = node.children.some(child => child.type === 'interface_declaration');
    const hasTypeAliasDeclaration = node.children.some(child => child.type === 'type_alias_declaration');
