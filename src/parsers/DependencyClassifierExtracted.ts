...
export class DependencyClassifierExtracted {
  ...
  private categorizeLibrary(packageName: string): LibraryInfo['category'] {
    // Frameworks
    const frameworks = ['react', 'vue', 'angular', 'svelte', 'next', 'nuxt', 'gatsby', 'express'];
    ...
    // Testing tools
    const testingTools = ['jest', 'mocha', 'chai', 'cypress', 'playwright', 'vitest'];
    if (testingTools.some(tool => packageName.includes(tool))) {
      return 'Testing';
    }

    // Build tools
    const buildTools = ['webpack', 'vite', 'rollup', 'parcel', 'esbuild', 'babel', 'tsc'];
