...
async function basicSingleFileAnalysis() {
	...
	try {
		...
		console.log(
			`Declarations: ${result.parseResult.declarations?.length || 0}`,
		);
	} catch (error) {
...
async function customDbPath() {
	...
	try {
		...
		console.log(
			`Nodes: ${result.stats.nodesCreated}, Edges: ${result.stats.edgesCreated}`,
		);
	} catch (error) {
...
async function multipleFilesAnalysis() {
	...
	try {
		...
		const totalNodes = results.reduce(
			(sum, r) => sum + r.stats.nodesCreated,
			0,
		);
		const totalEdges = results.reduce(
			(sum, r) => sum + r.stats.edgesCreated,
			0,
		);
		const totalTime = results.reduce(
			(sum, r) => sum + r.stats.processingTime,
			0,
		);
...
async function reuseAnalyzer() {
	...
	try {
		...
		for (const file of files) {
			...
			console.log(
				`  Nodes: ${result.stats.nodesCreated}, Time: ${result.stats.processingTime}ms`,
			);
		}
