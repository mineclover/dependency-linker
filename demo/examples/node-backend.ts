...
class ApiServer {
  ...
  private setupMiddleware(): void {
    ...
    if (config.environment === 'development') {
      this.app.use('/api/external', createProxyMiddleware({
  ...
  private setupRoutes(): void {
    ...
    // 사용자 목록 (관리자만)
    this.app.get('/api/users', validateUserRole(['admin']), async (req: Request, res: Response, next: NextFunction) => {
      try {
    ...
    // 사용자 프로필
    this.app.get('/api/profile', validateApiKey, async (req: Request, res: Response, next: NextFunction) => {
      try {
