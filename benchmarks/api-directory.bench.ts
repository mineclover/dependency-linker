...
class DirectoryAnalysisBenchmark {
  ...
  async setup(): Promise<DirectoryTestContext> {
    ...
    context.fileCount.small = await this.createProjectStructure(context.testDirectories.small, 'small');
    context.fileCount.medium = await this.createProjectStructure(context.testDirectories.medium, 'medium');
    context.fileCount.large = await this.createProjectStructure(context.testDirectories.large, 'large');
    context.fileCount.nested = await this.createNestedStructure(context.testDirectories.nested);
    context.fileCount.mixed = await this.createMixedStructure(context.testDirectories.mixed);
  ...
  private async createProjectStructure(projectDir: string, size: 'small' | 'medium' | 'large'): Promise<number> {
    const config = {
      small: {
      ...
      medium: {
        directories: ['src', 'src/components', 'src/utils', 'src/services', 'src/types', 'tests'],
        filesPerDir: 5,
        complexity: 'medium'
      },
      large: {
        directories: [
          'src', 'src/components', 'src/utils', 'src/services', 'src/types',
          'src/api', 'src/hooks', 'src/contexts', 'tests', 'tests/unit', 'tests/integration'
        ],
        filesPerDir: 8,
        complexity: 'high'
      }
  ...
  private async createNestedStructure(projectDir: string): Promise<number> {
    ...
    for (let depth = 0; depth < maxDepth; depth++) {
      const nestedPath = path.join(projectDir, ...Array(depth + 1).fill('level').map((l, i) => `${l}${i}`));
  ...
  private async createStructureRecursive(basePath: string, structure: any): Promise<number> {
    let fileCount = 0;
    ...
    for (const [name, config] of Object.entries(structure)) {
      ...
      if ((config as any).dirs) {
        fileCount += await this.createStructureRecursive(fullPath, (config as any).dirs);
      }
    }
  ...
  private generateFileContent(complexity: string, dirName: string, index: number): string {
    ...
    const config = complexityLevels[complexity as keyof typeof complexityLevels] || complexityLevels.medium;
  ...
  private generateFileContentByExtension(fileName: string, dirName: string): string {
  ...
  async runBenchmark(
    name: string,
    testFn: (context: DirectoryTestContext) => Promise<{ result: any; metrics?: any }>,
    iterations: number = 3
    ...
    return {
      ...
      directoriesPerSecond: (lastMetrics.totalDirectories || 0) / (averageTime / 1000),
      memoryUsage: {
  ...
  async runAllBenchmarks(): Promise<DirectoryBenchmarkResult[]> {
    ...
    try {
      ...
      const smallProjectResult = await this.runBenchmark('DirectoryAnalyzer.smallProject', async (ctx) => {
        const analyzer = new TypeScriptAnalyzer({ enableCache: true });
        const directoryAnalyzer = new DirectoryAnalyzer(analyzer);

        const result = await directoryAnalyzer.analyzeDirectory(ctx.testDirectories.small, {
          recursive: true,
      ...
      const mediumProjectResult = await this.runBenchmark('DirectoryAnalyzer.mediumProject', async (ctx) => {
        const analyzer = new TypeScriptAnalyzer({ enableCache: true });
        const directoryAnalyzer = new DirectoryAnalyzer(analyzer);

        const result = await directoryAnalyzer.analyzeDirectory(ctx.testDirectories.medium, {
          recursive: true,
          includeNodeModules: false,
          includeTests: true
        });
      ...
      const largeProjectResult = await this.runBenchmark('DirectoryAnalyzer.largeProject', async (ctx) => {
        const analyzer = new TypeScriptAnalyzer({ enableCache: true });
        const directoryAnalyzer = new DirectoryAnalyzer(analyzer);

        const result = await directoryAnalyzer.analyzeDirectory(ctx.testDirectories.large, {
          recursive: true,
          includeNodeModules: false,
          includeTests: true
        });
      ...
      const nestedResult = await this.runBenchmark('DirectoryAnalyzer.nestedStructure', async (ctx) => {
        const analyzer = new TypeScriptAnalyzer({ enableCache: true });
        const directoryAnalyzer = new DirectoryAnalyzer(analyzer);

        const result = await directoryAnalyzer.analyzeDirectory(ctx.testDirectories.nested, {
          recursive: true,
          maxDepth: 10
        });
      ...
      const mixedResult = await this.runBenchmark('DirectoryAnalyzer.mixedTypes', async (ctx) => {
        const analyzer = new TypeScriptAnalyzer({ enableCache: true });
        const directoryAnalyzer = new DirectoryAnalyzer(analyzer);

        const result = await directoryAnalyzer.analyzeDirectory(ctx.testDirectories.mixed, {
          recursive: true,
      ...
      const pathUtilsResult = await this.runBenchmark('PathUtils.operations', async (ctx) => {
      ...
      const batchResult = await this.runBenchmark('BatchAnalyzer.directories', async (ctx) => {
        const analyzer = new TypeScriptAnalyzer({ enableCache: true });
...
function formatDirectoryResults(results: DirectoryBenchmarkResult[]): void {
  ...
  results.forEach((result, index) => {
    ...
    console.log(`   Directories/sec: ${result.directoriesPerSecond.toFixed(2)}`);
    console.log(`   Memory Usage: ${result.memoryUsage.heapUsed}MB (peak: ${result.memoryUsage.peakUsage}MB)`);
    ...
    console.log(`   Dependencies Found: ${result.analysisMetrics.totalDependencies}`);
  ...
  const mostEfficient = results.reduce((efficient, r) =>
    ...
    (efficient.analysisMetrics.totalFiles / efficient.memoryUsage.heapUsed) ? r : efficient
  );

  console.log('\n🏆 Performance Insights');
  console.log(`   Fastest Analysis: ${fastestAnalysis.name} (${fastestAnalysis.filesPerSecond.toFixed(2)} files/sec)`);
  console.log(`   Most Memory Efficient: ${mostEfficient.name}`);
  console.log(`   Average Memory per File: ${(results.reduce((sum, r) => sum + (r.memoryUsage.heapUsed / r.analysisMetrics.totalFiles), 0) / results.length).toFixed(2)}MB`);
}

// Export for use in other scripts
export { DirectoryAnalysisBenchmark, DirectoryBenchmarkResult, formatDirectoryResults };
