...
class BatchProcessingBenchmark {
	...
	private generateComplexityContent(
		complexity: "small" | "medium" | "large",
		seed: number,
	): string {
		const config = {
	...
	async runBenchmark(
		...
		const avgMemoryUsage =
			memorySnapshots.reduce((sum, snapshot) => {
	...
	async runAllBenchmarks(): Promise<BatchBenchmarkResult[]> {
		...
		try {
			...
			const smallFilesResult = await this.runBenchmark(
				"BatchAnalyzer.smallFiles",
				async (ctx) => {
					...
					return {
						...
						metrics: {
							...
							cacheEfficiency:
								analyzer.getCacheStats().hitRate /
								(analyzer.getCacheStats().hitRate +
									analyzer.getCacheStats().missRate),
						},
					};
				},
			);
			...
			const mediumFilesResult = await this.runBenchmark(
				"BatchAnalyzer.mediumFiles",
				async (ctx) => {
					const analyzer = new TypeScriptAnalyzer({ enableCache: true });
					const batchAnalyzer = new BatchAnalyzer(analyzer, {
						maxConcurrency: 5,
			...
			const largeFilesResult = await this.runBenchmark(
				"BatchAnalyzer.largeFiles",
				async (ctx) => {
					const analyzer = new TypeScriptAnalyzer({ enableCache: true });
					const batchAnalyzer = new BatchAnalyzer(analyzer, {
						maxConcurrency: 2,
			...
			const mixedFilesResult = await this.runBenchmark(
				"BatchAnalyzer.mixedFiles",
				async (ctx) => {
					const analyzer = new TypeScriptAnalyzer({ enableCache: true });
					const batchAnalyzer = new BatchAnalyzer(analyzer, {
						maxConcurrency: 4,
			...
			const factoryComparisonResult = await this.runBenchmark(
				"FactoryFunctions.batch",
				async (ctx) => {
			...
			const cachePerformanceResult = await this.runBenchmark(
				"CachePerformance.duplicates",
				async (ctx) => {
					const analyzer = new TypeScriptAnalyzer({ enableCache: true });
					const batchAnalyzer = new BatchAnalyzer(analyzer, {
						maxConcurrency: 3,
					...
					return {
						...
						metrics: {
							...
							cacheEfficiency:
								cacheStats.hitRate /
								(cacheStats.hitRate + cacheStats.missRate || 1),
						},
					};
				},
			);
			...
			const adaptiveConcurrencyResult = await this.runBenchmark(
				"AdaptiveConcurrency.highLoad",
				async (ctx) => {
					const analyzer = new TypeScriptAnalyzer({ enableCache: true });
					const batchAnalyzer = new BatchAnalyzer(analyzer, {
						maxConcurrency: 8,
					...
					return {
						...
						metrics: {
							...
							cacheEfficiency:
								analyzer.getCacheStats().hitRate /
								(analyzer.getCacheStats().hitRate +
									analyzer.getCacheStats().missRate || 1),
						},
					};
				},
			);
...
function formatBatchResults(results: BatchBenchmarkResult[]): void {
	...
	results.forEach((result, index) => {
		...
		console.log(
			`   Memory Efficiency: ${result.memoryEfficiency.toFixed(2)} MB/file`,
		);
		console.log(
			`   Concurrency: ${result.concurrencyMetrics.actualConcurrency} (optimal: ${result.concurrencyMetrics.optimalConcurrency})`,
		);
		console.log(
			`   Resource Utilization: ${(result.concurrencyMetrics.resourceUtilization * 100).toFixed(1)}%`,
		);
		console.log(
			`   Cache Hit Rate: ${(result.cacheMetrics.hitRate * 100).toFixed(1)}%`,
		);
		console.log(
			`   Cache Efficiency: ${(result.cacheMetrics.efficiency * 100).toFixed(1)}%`,
		);
	});
	...
	const mostEfficient = results.reduce((best, r) =>
		r.memoryEfficiency < best.memoryEfficiency ? r : best,
	);
	const bestCache = results.reduce((best, r) =>
		r.cacheMetrics.efficiency > best.cacheMetrics.efficiency ? r : best,
	);

	console.log("\n🏆 Performance Insights");
	console.log(
		`   Highest Throughput: ${bestThroughput.name} (${bestThroughput.throughput.toFixed(2)} files/sec)`,
	);
	console.log(
		`   Most Memory Efficient: ${mostEfficient.name} (${mostEfficient.memoryEfficiency.toFixed(2)} MB/file)`,
	);
	console.log(
		`   Best Cache Performance: ${bestCache.name} (${(bestCache.cacheMetrics.efficiency * 100).toFixed(1)}% efficiency)`,
	);
}
