...
async function legacyUsage() {
  ...
  try {
    ...
    const dependencyNames = await analyzer.extractDependencies('./src/index.ts');
...
async function newApiUsage() {
  ...
  try {
    ...
    console.log('- Extraction time:', result.performanceMetrics.extraction, 'ms');
    console.log('- Interpretation time:', result.performanceMetrics.interpretation, 'ms');
    console.log('- Total time:', result.performanceMetrics.total, 'ms');
    console.log('- Memory usage:', result.performanceMetrics.memory.peak, 'bytes');
...
async function migrationExample() {
  ...
  try {
    ...
    console.log('Dependencies match:',
      convertedToLegacy.dependencies.length === legacyResult.dependencies.length);
    ...
    const directNewResult = await newEngineFromLegacy.analyzeFile('./src/index.ts');
...
async function versionManagementExample() {
  ...
  try {
    ...
    console.log('Version 2:', `${v2.major}.${v2.minor}.${v2.patch}-${v2.prerelease}`);
    ...
    console.log('V1 adapter version:', `${v1Adapter.version.major}.${v1Adapter.version.minor}`);
    console.log('V2 adapter version:', `${v2Adapter.version.major}.${v2Adapter.version.minor}`);
    ...
    const migrationPlan = migrationUtility.getMigrationPlan(fromVersion, toVersion);
    ...
    migrationPlan.steps.forEach((step, index) => {
      console.log(`  ${index + 1}. ${step.description} (${step.automated ? 'automated' : 'manual'})`);
    });

    // List breaking changes
    console.log('Breaking changes:');
    migrationPlan.breakingChanges.forEach((change, index) => {
      console.log(`  ${index + 1}. ${change.component}: ${change.description} (${change.impact} impact)`);
    });

    // Execute migration (dry run)
    const migrationResult = await migrationUtility.executeMigration(migrationPlan, {
      dryRun: true
    });

    console.log('Migration dry run result:');
    console.log('- Success:', migrationResult.success);
    console.log('- Steps completed:', migrationResult.stepsCompleted, '/', migrationResult.stepsTotal);
...
async function performanceComparison() {
  ...
  try {
    ...
    console.log('- Average time:', legacyState.metrics.averageAnalysisTime, 'ms');
...
async function errorHandlingExample() {
  ...
  try {
    ...
    try {
      const legacyResult = await legacyAnalyzer.analyzeFile('./non-existent.ts');
    ...
    try {
      ...
      console.log('New result for missing file:', newResult.errors.length, 'errors');
    } catch (newError) {
    ...
    try {
      ...
      console.log('New syntax error handling:', newResult.errors.length, 'errors');
      if (newResult.errors.length > 0) {
        console.log('First error:', newResult.errors[0].category, '-', newResult.errors[0].message);
      }
    } catch (error) {
