...
class TestPerformanceMeasurer {
	...
	async measureBaseline() {
		...
		try {
			...
			const integrationTests = await this.measureTestCommand(
			...
			const contractTests = await this.measureTestCommand(
	...
	generateReport() {
		...
		// Overall summary
		const fullSuite = this.results.measurements.find(
			(m) => m.name === "Full Test Suite",
		);
		if (fullSuite) {
			console.log("\n📊 Optimization Targets:");
			console.log(
				`  Current: ${fullSuite.duration.toFixed(0)}ms, ${fullSuite.passRate.toFixed(3)} pass rate`,
			);
			...
			console.log(
				`    - Reduce duration by ${durationImprovement.toFixed(0)}ms`,
			);
			console.log(
				`    - Improve pass rate by ${(passRateImprovement * 100).toFixed(1)}%`,
			);
		}
