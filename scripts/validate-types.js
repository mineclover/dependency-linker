	...
	const validations = [
		{
			...
			test: () => {
				if (typeof pkg.TypeScriptAnalyzer !== "function") {
					throw new Error(
						"TypeScriptAnalyzer is not exported as a constructor",
					);
				}
...

		{
			...
			test: () => {
				const requiredRuntimeExports = ["OutputFormatter", "createLogger"];
...

		{
			...
			test: () => {
				...
				if (packageJson.types !== "dist/index.d.ts") {
					throw new Error(
						`Expected types field to be 'dist/index.d.ts', got '${packageJson.types}'`,
					);
				}
				...
				if (!fs.existsSync("./dist/index.d.ts")) {
					throw new Error(
						"TypeScript declaration file dist/index.d.ts not found",
					);
				}
