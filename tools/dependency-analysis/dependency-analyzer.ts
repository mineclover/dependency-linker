...
class DependencyAnalyzer {
	...
	async analyzeDependencies(startFile: string): Promise<AnalysisResult> {
		...
		return {
			...
			maxDepth: Math.max(
				...Array.from(this.dependencies.values()).map((d) => d.depth),
			),
	...
	private buildDependencyTree(startFile: string): string[] {
		...
		const buildTree = (filePath: string, depth: number): void => {
			...
			tree.push(
				`${indent}📄 ${relativePath} (의존성: ${info?.dependencies.length || 0}개)`,
			);
	...
	printResults(result: AnalysisResult): void {
		...
		if (result.circularDependencies.length > 0) {
			...
			result.circularDependencies.forEach((cycle, i) => {
				console.log(
					`${i + 1}. ${cycle.map((f) => path.relative(this.projectRoot, f)).join(" → ")}`,
				);
			});
		}
		...
		// 의존성 수 기준으로 정렬
		const sortedFiles = Array.from(result.dependencies.values()).sort(
		...
		sortedFiles.forEach((info) => {
			...
			if (info.dependencies.length > 0) {
				console.log(
					`  └─ 의존성: ${info.dependencies.map((d) => path.relative(this.projectRoot, d)).join(", ")}`,
				);
			}

			if (info.dependents.length > 0) {
				console.log(
					`  └─ 의존받음: ${info.dependents.map((d) => path.relative(this.projectRoot, d)).join(", ")}`,
				);
			}
			console.log("");
		});
			...
			dependencies: Object.fromEntries(
				Array.from(result.dependencies.entries()).map(([key, value]) => [
					path.relative(this.projectRoot, key),
					{
						...
						dependencies: value.dependencies.map((d) =>
							path.relative(this.projectRoot, d),
						),
						dependents: value.dependents.map((d) =>
							path.relative(this.projectRoot, d),
						),
					},
				]),
