...
async function analyzeCoreFiles() {
	...
	for (const file of coreFiles) {
		try {
			...
			// 간단한 요약 표시
			console.log("\n📋 직접 의존성:");
			const rootInfo = result.dependencies.get(
				result.rootFile.includes("/")
					? `/Users/junwoobang/project/dependency-linker/${result.rootFile}`
					: result.rootFile,
			);

			if (rootInfo && rootInfo.dependencies.length > 0) {
				rootInfo.dependencies.forEach((dep) => {
					const relativeDep = dep.replace(
						"/Users/junwoobang/project/dependency-linker/",
						"",
					);
			...
			await analyzer.saveResultsToFile(
				result,
				`core-analysis-${filename}-${timestamp}.json`,
			);
