...
class DependencyVisualizer {
   ...
   * Mermaid 그래프 생성
   */
  generateMermaidGraph(result: AnalysisResult, options: VisualizationOptions = {}): string {
    ...
    // 스타일 정의
    lines.push('');
    lines.push('    classDef critical fill:#ff6b6b,stroke:#333,stroke-width:2px');
    lines.push('    classDef important fill:#4ecdc4,stroke:#333,stroke-width:2px');
   ...
   * ASCII 트리 생성 (개선된 버전)
   */
  generateASCIITree(result: AnalysisResult, options: VisualizationOptions = {}): string {
    const lines: string[] = [];
    const visited = new Set<string>();
    const rootFile = path.resolve(this.projectRoot, result.rootFile);

    const buildTree = (filePath: string, depth: number, isLast: boolean, prefix: string): void => {
      if (visited.has(filePath) || (options.maxDepth && depth > options.maxDepth)) {
        return;
      }
      ...
      const depText = options.showDepth ? ` [d:${depth}, deps:${depCount}]` : ` (${depCount})`;
  ...
  generateDirectorySummary(result: AnalysisResult): string {
    const dirSummary = new Map<string, {
      files: string[];
      totalDeps: number;
    ...
    // 디렉토리를 파일 수 기준으로 정렬
    const sortedDirs = Array.from(dirSummary.entries())
    ...
    for (const [dir, summary] of sortedDirs) {
      ...
      if (summary.files.length <= 5) {
        ...
        lines.push(`  📄 주요 파일들: ${summary.files.slice(0, 3).join(', ')} 외 ${summary.files.length - 3}개`);
      }
    }
  ...
  generateCriticalPathAnalysis(result: AnalysisResult): string {
    ...
    if (highDependencyFiles.length > 0) {
      ...
      highDependencyFiles.forEach((info, index) => {
        lines.push(`  ${index + 1}. ${info.relativePath} (${info.dependencies.length}개 의존성)`);
      });
    }
    ...
    if (highDependentFiles.length > 0) {
      ...
      highDependentFiles.forEach((info, index) => {
        lines.push(`  ${index + 1}. ${info.relativePath} (${info.dependents.length}개가 의존)`);
      });
    }

    // 잠재적 병목 지점
    const bottleneckFiles = Array.from(result.dependencies.values())
      .filter(info => info.dependencies.length >= 2 && info.dependents.length >= 2)
    ...
    if (bottleneckFiles.length > 0) {
      ...
      bottleneckFiles.forEach((info, index) => {
        ...
        lines.push(`  ${index + 1}. ${info.relativePath} (의존성: ${info.dependencies.length}, 피의존성: ${info.dependents.length}, 총점: ${score})`);
      });
    }
   ...
   */
  generateComprehensiveReport(result: AnalysisResult, options: VisualizationOptions = {}): string {
    const lines: string[] = [];
    ...
    if (result.circularDependencies.length > 0) {
      ...
      result.circularDependencies.forEach((cycle, i) => {
        const relativeCycle = cycle.map(f => path.relative(this.projectRoot, f));
   ...
   * 시각화 결과를 파일로 저장
   */
  async saveVisualization(result: AnalysisResult, outputPrefix: string = 'dependency-visual'): Promise<void> {
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');

    // 종합 리포트
    const report = this.generateComprehensiveReport(result, { showDepth: true, maxDepth: 10 });
    await fs.promises.writeFile(`${outputPrefix}-report-${timestamp}.md`, report);

    // Mermaid 그래프만 별도 저장
    const mermaidGraph = this.generateMermaidGraph(result);
    await fs.promises.writeFile(`${outputPrefix}-mermaid-${timestamp}.md`, `\`\`\`mermaid\n${mermaidGraph}\n\`\`\``);
