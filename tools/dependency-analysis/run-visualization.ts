...
async function main() {
  ...
  try {
    // 이전 분석 결과 로드
    const resultData = await fs.promises.readFile('dependency-analysis-result.json', 'utf-8');
    ...
    const absoluteResult: AnalysisResult = {
      ...
      dependencies: new Map(
        Object.entries(result.dependencies).map(([key, value]) => [
          key.startsWith('src/') ? `/Users/junwoobang/project/dependency-linker/${key}` : key,
          {
            ...value,
            filePath: value.filePath.startsWith('src/') ? `/Users/junwoobang/project/dependency-linker/${value.filePath}` : value.filePath,
            dependencies: value.dependencies.map(dep =>
              dep.startsWith('src/') ? `/Users/junwoobang/project/dependency-linker/${dep}` : dep
            ),
