...
import { DependencyAnalyzer, ClassifiedDependency, DependencyStats } from '../../../src/services/DependencyAnalyzer';
...
describe('DependencyAnalyzer', () => {
  ...
  describe('classifyDependencies', () => {
    test('should classify multiple dependencies', async () => {
      const dependencies: DependencyInfo[] = [
        ...
        { source: './utils.ts', type: 'relative', location: { line: 2, column: 0, offset: 0 } },
        { source: 'fs', type: 'external', location: { line: 3, column: 0, offset: 0 } }
      ];
  ...
  describe('getTopPackages', () => {
    ...
    test('should limit results correctly', () => {
      const dependencies: ClassifiedDependency[] = [
        { source: 'a', type: 'external', location: { line: 1, column: 0, offset: 0 }, isNodeBuiltin: false, isScopedPackage: false, packageName: 'a' },
        { source: 'b', type: 'external', location: { line: 2, column: 0, offset: 0 }, isNodeBuiltin: false, isScopedPackage: false, packageName: 'b' },
        { source: 'c', type: 'external', location: { line: 3, column: 0, offset: 0 }, isNodeBuiltin: false, isScopedPackage: false, packageName: 'c' }
      ];
    ...
    test('should ignore non-external dependencies', () => {
      const dependencies: ClassifiedDependency[] = [
        { source: './utils', type: 'relative', location: { line: 1, column: 0, offset: 0 }, isNodeBuiltin: false, isScopedPackage: false, packageName: './utils' },
        { source: 'internal/module', type: 'internal', location: { line: 2, column: 0, offset: 0 }, isNodeBuiltin: false, isScopedPackage: false, packageName: 'internal/module' }
      ];
  ...
  describe('detectPotentialCircularDependencies', () => {
    test('should detect relative dependencies as potential circular', () => {
      ...
      const circular = analyzer.detectPotentialCircularDependencies(dependencies);
  ...
  describe('suggestOptimizations', () => {
    test('should suggest combining multiple imports from same package', () => {
      const dependencies: ClassifiedDependency[] = Array.from({ length: 4 }, (_, i) => ({
        source: `lodash/${i}`,
    ...
    test('should suggest barrel exports for many internal imports', () => {
      const dependencies: ClassifiedDependency[] = Array.from({ length: 6 }, (_, i) => ({
    ...
    test('should warn about deep relative imports', () => {
      ...
      expect(suggestions.some(s => s.includes('deep relative paths'))).toBe(true);
    });
  });
  ...
  describe('validateDependencies', () => {
    test('should warn about Node.js built-ins', () => {
      ...
      expect(warnings.some(w => w.includes('Node.js built-in modules'))).toBe(true);
    });
    ...
    test('should warn about relative imports without extensions', () => {
      ...
      expect(warnings.some(w => w.includes('missing file extensions'))).toBe(true);
    });
  });
  ...
  describe('private methods', () => {
    test('extractExtension should work correctly', () => {
      ...
      const extractExtension = (analyzer as any).extractExtension.bind(analyzer);
