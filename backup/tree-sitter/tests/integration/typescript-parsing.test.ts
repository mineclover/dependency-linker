...
describe('TypeScript Parsing Integration', () => {
  ...
  afterAll(async () => {
    ...
    const tempFiles = ['types.ts', 'imports.ts', 'exports.ts', 'react.tsx', 'syntax-error.ts', 'large-file.ts'];
...
`);
      ...
      const reactImport = result.imports.find(i => i.source === 'react' && !i.isTypeOnly);
      ...
      const namedReactImport = result.imports.find(
        i => i.source === 'react' && i.specifiers.some(s => s.imported === 'useState')
      );
      ...
      const typeOnlyImport = result.imports.find(i => i.source === 'react' && i.isTypeOnly);
      expect(typeOnlyImport).toBeDefined();
      expect(typeOnlyImport?.isTypeOnly).toBe(true);

      // Test side-effect imports
      const sideEffectImport = result.imports.find(i => i.source === './styles.css');
      expect(sideEffectImport?.specifiers).toHaveLength(0);

      // Test aliased imports
      const aliasedImport = result.imports.find(i => 
        i.specifiers.some(s => s.imported === 'LongComponentName' && s.local === 'Component')
      );
...
`);
  ...
  describe('Performance and memory', () => {
    test('should parse large files within time limit', async () => {
      ...
      for (let i = 0; i < 50; i++) { // Reduced due to tree-sitter memory limits
        content += `
