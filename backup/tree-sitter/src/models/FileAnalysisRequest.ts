 ...
 */
export function normalizeAnalysisOptions(options?: Partial<AnalysisOptions>): AnalysisOptions {
  return {
    format: options?.format ?? DEFAULT_ANALYSIS_OPTIONS.format,
    includeSources: options?.includeSources ?? DEFAULT_ANALYSIS_OPTIONS.includeSources,
    parseTimeout: options?.parseTimeout ?? DEFAULT_ANALYSIS_OPTIONS.parseTimeout
  };
}
