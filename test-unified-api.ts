...
async function testUnifiedAPI() {
	...
	try {
		...
		console.log(
			`   Dependencies: ${tsResult.extractedData.dependencies?.length || "N/A"}`,
		);
		...
		console.log(
			`   Links extracted: ${Array.isArray(linkData) ? linkData.length : 0}`,
		);
		console.log(
			`   Link health: ${linkAnalysis ? linkAnalysis.summary.totalLinks - linkAnalysis.summary.brokenLinks : 0}/${linkAnalysis ? linkAnalysis.summary.totalLinks : 0} working`,
		);
		...
		console.log("\n⚡ Performance Comparison:");
		console.log(
			`   TypeScript: ${tsResult.performanceMetrics.totalTime}ms total`,
		);
		console.log(
			`   Markdown: ${mdResult.performanceMetrics.totalTime}ms total`,
		);

		console.log("\n🎉 Unified API test completed successfully!");
		console.log(
			"✅ Both TypeScript and Markdown analysis return consistent AnalysisResult interface",
		);
