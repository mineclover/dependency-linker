...
function typescriptAnalyzer(options = {}) {
	...
	return {
			...
			try {
				...
				analyzedModules.set(id, {
					...
					bundleInfo: {
						isEntry:
							bundleInfo.input === id ||
							(Array.isArray(bundleInfo.input) &&
								bundleInfo.input.includes(id)),
						transformTime: Date.now(),
					},
				});

				console.log(
					`[rollup:typescript-analyzer] Analyzed: ${path.relative(process.cwd(), id)}`,
				);
			} catch (error) {
				console.warn(
					`[rollup:typescript-analyzer] Failed to analyze ${id}:`,
					error.message,
				);
			}
		...
		generateBundle(outputOptions, bundle) {
			...
			console.log(
				`[rollup:typescript-analyzer] Generating analysis report for ${analyzedModules.size} modules`,
			);
			...
			const report = {
				...
				optimization: enableBundleOptimization
					? this.generateOptimizationSuggestions()
					: null,
				treeShaking: enableTreeShakingAnalysis
					? this.analyzeTreeShaking(bundle)
					: null,
			};
		...
		generateSummary() {
			...
			return {
				...
				averageDependenciesPerModule:
					modules.length > 0
						? (totalDependencies / modules.length).toFixed(2)
						: 0,
			};
		},
		...
		generateOptimizationSuggestions() {
			...
			const heavyModules = modules.filter(
				(m) => m.dependencies && m.dependencies.length > 15,
			);
		...
		analyzeTreeShaking(bundle) {
			...
			analysis.treeShakingEfficiency =
				analysis.totalExports > 0
					? ((analysis.usedExports / analysis.totalExports) * 100).toFixed(2) +
						"%"
					: "100%";
		...
		logBundleAnalysis(analysis) {
			...
			console.log(
				`   Bundle size: ${(analysis.totalBundleSize / 1024).toFixed(2)} KB`,
			);
			console.log(
				`   Largest chunk: ${Math.max(...analysis.chunkSizes.map((c) => c.size / 1024)).toFixed(2)} KB`,
			);
			console.log("");
		},
	};
