...
function typescriptAnalyzerPlugin(options = {}) {
	...
	return {
		...
		configResolved(config) {
			...
			log(
				"info",
				`Plugin initialized for ${isProduction ? "production" : "development"} build`,
			);
		},
		...
		buildStart(opts) {
			if (isProduction) {
				log(
					"info",
					"Starting TypeScript dependency analysis for production build",
				);
				analyzedFiles.clear();
			}
		},
		...
		async generateBundle(options, bundle) {
			...
			log(
				"info",
				`Generating dependency analysis for ${analyzedFiles.size} files`,
			);
...
async function detectCircularDependencies(currentFile, result, analyzedFiles) {
	...
	if (hasCycle(currentFile, analyzedFiles)) {
		console.warn(
			`[vite:typescript-analyzer] Circular dependency detected involving: ${currentFile}`,
		);
	}
}
...
function generateAnalysisSummary(results) {
	...
	const totalDependencies = results.reduce(
		(sum, r) => sum + (r.dependencies?.length || 0),
		0,
	);
	const totalImports = results.reduce(
		(sum, r) => sum + (r.imports?.length || 0),
		0,
	);
	const totalExports = results.reduce(
		(sum, r) => sum + (r.exports?.length || 0),
		0,
	);
	...
	return {
		...
		averageDependenciesPerFile:
			totalFiles > 0 ? (totalDependencies / totalFiles).toFixed(2) : 0,
	};
}
...
async function generateWarnings(results) {
	...
	const noExportFiles = results.filter(
		(r) => r.success && (!r.exports || r.exports.length === 0),
	);
	...
	const heavyFiles = results.filter(
		(r) => r.success && r.dependencies && r.dependencies.length > 20,
	);
