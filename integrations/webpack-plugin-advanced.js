...
class TypeScriptAnalyzerWebpackPlugin {
	...
	apply(compiler) {
		...
		compiler.hooks.compile.tap(pluginName, () => {
			...
			console.log(
				"[webpack:typescript-analyzer] Starting comprehensive analysis...",
			);
		});
	...
	async analyzeModule(module, compilation) {
		...
		try {
			...
			console.warn(
				`[webpack:typescript-analyzer] Analysis failed for ${resource}:`,
				error.message,
			);
		}
	}
	...
	analyzeChunkOptimization(chunks, compilation) {
		chunks.forEach((chunk) => {
			...
			const typeScriptModules = chunkModules.filter((m) =>
				this.isTypeScriptModule(m),
			);
	...
	detectCircularDependencies(currentFile, analysis) {
		...
		if (hasCycle(currentFile)) {
			console.warn(
				`[webpack:typescript-analyzer] Circular dependency detected: ${currentFile}`,
			);
		}
	}
	...
	generateSummary() {
		...
		return {
			...
			totalDependencies: modules.reduce(
			...
			averageModuleSize:
				modules.length > 0
					? modules.reduce((sum, m) => sum + (m.webpackInfo?.size || 0), 0) /
						modules.length
					: 0,
			totalBuildTime:
				this.analysisData.performance.endTime -
				this.analysisData.performance.startTime,
	...
	generateOptimizationReport() {
		...
		const largeChunks = Array.from(this.analysisData.chunks.values()).filter(
			(chunk) => chunk.moduleCount > 10,
		);
		...
		if (commonDeps.length > 0) {
			suggestions.push({
				...
				priority: "low",
				description:
					"Consider extracting common dependencies into separate chunks",
		...
		return {
			...
			efficiency: {
				averageTimePerModule:
					this.analysisData.modules.size > 0
						? this.analysisData.performance.moduleAnalysisTime /
							this.analysisData.modules.size
						: 0,
			},
		};
	}
	...
	generateWarnings() {
		...
		const noExportModules = modules.filter(
			(m) => m.success && (!m.exports || m.exports.length === 0),
		);
		...
		// Large modules
		const largeModules = modules.filter(
			(m) => m.webpackInfo && m.webpackInfo.size > 50000, // 50KB
		);
	...
	logPerformanceSummary() {
		...
		console.log(
			`   Average per module: ${performance.efficiency.averageTimePerModule.toFixed(2)}ms`,
		);
