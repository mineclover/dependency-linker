...
async function runComparison() {
	...
	const testFiles = ["./README.md", "./docs/quickstart.md", "./CLAUDE.md"];

	const batchResults = await getBatchMarkdownAnalysis(testFiles, {
	...
	const newSorted = batchResults.sort((a, b) =>
	...
	newSorted.forEach((result, index) => {
		console.log(
			`  ${index + 1}. ${result.pathInfo.relative} (깊이: ${result.pathInfo.depth})`,
		);
	});
	...
	const improvement = (((beforeTime - afterTime) / beforeTime) * 100).toFixed(
		1,
	);
	...
	console.log(colorize("\n🔴 BEFORE 코드 라인 수:", "red"));
	console.log(
		`  extractPathInfoLegacy 함수: ${getExtractPathInfoLegacyLines()}줄`,
	);
	...
	console.log(colorize("\n✨ 코드 간소화:", "cyan"));
	const reduction = (
		((getExtractPathInfoLegacyLines() - 1) / getExtractPathInfoLegacyLines()) *
		100
	).toFixed(1);
