...
async function runMacOSDemo() {
	...
	console.log(
		`   시스템: ${colorize(systemInfo.productName, "green")} ${colorize(systemInfo.productVersion, "green")}`,
	);
	console.log(`   빌드: ${colorize(systemInfo.buildVersion, "green")}`);
	console.log(
		`   아키텍처: ${colorize(process.arch, "green")} ${systemInfo.isAppleSilicon ? colorize("(Apple Silicon)", "magenta") : colorize("(Intel)", "cyan")}`,
	);
	console.log(`   Node.js: ${colorize(process.version, "green")}`);
	console.log(
		`   CPU 코어: ${colorize(systemInfo.cpuInfo.length.toString(), "green")}개`,
	);
	...
	try {
		...
		macOSPathInfos.forEach((pathInfo, i) => {
			...
			// macOS 특별 경로 식별
			let pathType = "";
			if (pathInfo.input.includes(".app/"))
				pathType = colorize("📱 앱번들", "magenta");
			else if (pathInfo.input.startsWith("~/"))
				pathType = colorize("🏠 홈", "cyan");
			else if (pathInfo.input.startsWith("/System/"))
				pathType = colorize("🔧 시스템", "blue");
			else if (pathInfo.input.startsWith("/Applications/"))
				pathType = colorize("📱 앱", "magenta");
			else if (pathInfo.input.startsWith("/Volumes/"))
				pathType = colorize("💾 볼륨", "yellow");

			console.log(`   ${i + 1}. ${pathInfo.input}`);
			console.log(
				`      → ${pathInfo.relative} (깊이: ${pathInfo.depth}, 구분자: ${separator}, ${status}${pathType ? ", " + pathType : ""}))`,
			);
		});
		...
		if (systemInfo.isAppleSilicon) {
			console.log(
				`   ${colorize("🚀 Apple Silicon 최적화 활성화", "magenta")}`,
			);
		}
		...
		sortedDirs.forEach((dir, dirIndex) => {
			...
			files.forEach((file, fileIndex) => {
				...
				console.log(
					`   ${fileSymbol}${filePrefix} ${fileIcon} ${file.fileName}`,
				);
			});
		});
		...
		const benchmarkPaths = Array.from(
			{ length: 1000 },
			(_, i) => `./src/component${i % 10}/module${i % 5}/file${i}.ts`,
		);

		console.log(
			colorize(
				`\n⚡ ${benchmarkPaths.length}개 경로 성능 테스트 (${systemInfo.isAppleSilicon ? "Apple Silicon" : "Intel"} 최적화):`,
				"yellow",
			),
		);
		...
		if (systemInfo.isAppleSilicon) {
			console.log(
				`   ${colorize("🚀 Apple Silicon 가속: 네이티브 ARM64 최적화", "magenta")}`,
			);
		...
		if (existsSync(packageJsonPath)) {
			...
			try {
				...
				console.log(
					`   이름: ${colorize(packageJson.name || "Unknown", "cyan")}`,
				);
				console.log(
					`   버전: ${colorize(packageJson.version || "0.0.0", "cyan")}`,
				);
				...
				console.log(
					`   개발 의존성: ${colorize(devDeps.length.toString(), "cyan")}개`,
				);
			} catch (error) {
		...
		macOSSpecialPaths.forEach((path, i) => {
			...
			if (path.includes("Library"))
				pathType = colorize("📚 라이브러리", "cyan");
			else if (path.includes("Applications"))
				pathType = colorize("📱 앱", "magenta");
			else if (path.includes("Developer"))
				pathType = colorize("🛠️ 개발", "blue");
			else if (path.includes("homebrew") || path.includes("local"))
				pathType = colorize("🍺 Homebrew", "yellow");
		...
		console.log(
			colorize(
				`   ✅ ${systemInfo.isAppleSilicon ? "Apple Silicon" : "Intel"} 아키텍처 최적화`,
				"green",
			),
		);
		...
		console.log(
			colorize(
				"\n🍎 macOS PathInfo 시스템 - 개발자 친화적 환경에서 준비 완료!",
				"bright",
			),
		);
	} catch (error) {
