...
import { createPathInfo, createBatchPathInfo, filterPathInfo, groupPathInfoByDirectory, comparePathInfo } from './src/lib/index';
...
async function testCrossPlatformPaths() {
	...
	try {
		...
		linuxResults.forEach((pathInfo, i) => {
			...
			console.log(`      Separator: "${pathInfo.separator}", Depth: ${pathInfo.depth}, Within: ${pathInfo.isWithinProject ? '✅' : '❌'}`);
		});
		console.log('');
		...
		linuxSorted.forEach((p, i) => console.log(`     ${i + 1}. ${p.relative} (depth: ${p.depth})`));

		console.log('   Windows order:');
		windowsSorted.forEach((p, i) => console.log(`     ${i + 1}. ${p.relative} (depth: ${p.depth})`));
		...
		const consistentSorting = linuxOrder.every((path, i) => path === windowsOrder[i]);
		...
		edgeCases.forEach(testCase => {
			try {
				...
				console.log(`   "${testCase}" → Windows: ${windowsResult.isWithinProject ? '✅' : '❌'}, POSIX: ${posixResult.isWithinProject ? '✅' : '❌'}`);
			} catch (error) {
