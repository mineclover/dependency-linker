...
`;
  ...
  try {
    ...
    console.log('📊 Results:');
    console.log('Statistics:', JSON.stringify(exportResult.statistics, null, 2));
    console.log('\n📋 Export Methods:');
    ...
    exportResult.classes.forEach(cls => {
      console.log(`- ${cls.className} (${cls.methods.length} methods, ${cls.properties.length} properties)`);
    });
