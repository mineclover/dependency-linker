...
async function testInferenceWithSourceFile() {
	...
	libraryImports.forEach((edge) => {
		console.log(
			`  - ${edge.fromIdentifier?.split("::").pop()} → ${edge.toIdentifier?.split("::").pop()}`,
		);
	});
	...
	if (nodes.length > 0) {
		...
		// 직접 의존성
		const directDeps = await db.findNodeDependencies(appNode.id!, [
			"imports_file",
			"imports_library",
		]);
...
function countIndirectDependencies(tree: any): {
	total: number;
	direct: number;
	indirect: number;
} {
