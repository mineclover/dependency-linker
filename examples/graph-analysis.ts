	...
	try {
		...
		if (cycles.length > 0) {
			...
			cycles.forEach((cycle, index) => {
				console.log(
					`  Cycle ${index + 1}: ${cycle.map((n) => n.name).join(" → ")}`,
				);
			});
		} else {
		...
		console.log(
			`🧠 Inferred relationships: ${withInferences.inferred?.length || 0}`,
		);
...
async function hierarchyAnalysisExample() {
	...
	try {
		...
		for (const file of fileNodes.slice(0, 5)) {
			// 상위 5개 파일만 표시
			const fileEdges = hierarchy.edges.filter(
				(e) => e.startNodeId === file.id,
			);
			...
			const declarations = fileEdges.map((edge) => {
				const declaration = hierarchy.nodes.find(
					(n) => n.id === edge.endNodeId,
				);
				return declaration
			...
			console.log(
				`   └─ ${declarations.slice(0, 3).join(", ")}${declarations.length > 3 ? "..." : ""}`,
			);
		}
