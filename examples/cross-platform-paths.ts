...
import { createPathInfo, createBatchPathInfo, PathInfo } from '../src/lib/index';
...
export class WindowsPathUtils {
	...
	static expandVariables(inputPath: string): string {
		...
		for (const [variable, value] of Object.entries(variables)) {
			expandedPath = expandedPath.replace(new RegExp(variable.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi'), value);
		}
...
export class MacOSPathUtils {
	...
	static analyzeAppBundle(pathInfo: PathInfo): {
		...
		appName: string;
		bundlePath: string;
		contentType: 'executable' | 'resources' | 'frameworks' | 'plugins' | 'other';
	} {
		const absolutePath = pathInfo.absolute;
		const appBundleMatch = absolutePath.match(/^(.+\.app)\/(.*)$/);

		if (!appBundleMatch) {
			return { isAppBundle: false, appName: '', bundlePath: '', contentType: 'other' };
		}
		...
		let contentType: 'executable' | 'resources' | 'frameworks' | 'plugins' | 'other' = 'other';
		if (internalPath.startsWith('Contents/MacOS/')) contentType = 'executable';
		else if (internalPath.startsWith('Contents/Resources/')) contentType = 'resources';
		else if (internalPath.startsWith('Contents/Frameworks/')) contentType = 'frameworks';
		else if (internalPath.startsWith('Contents/PlugIns/')) contentType = 'plugins';
	...
	static classifySystemPath(pathInfo: PathInfo): {
		pathType: 'system' | 'library' | 'applications' | 'user' | 'homebrew' | 'developer' | 'volumes' | 'other';
		isProtected: boolean;
		description: string;
	} {
		const absolutePath = pathInfo.absolute;
		...
		if (absolutePath.startsWith('/System/')) {
			return { pathType: 'system', isProtected: true, description: 'macOS 시스템 파일' };
		}
		if (absolutePath.startsWith('/Library/')) {
			return { pathType: 'library', isProtected: true, description: '시스템 라이브러리' };
		}
		if (absolutePath.startsWith('/Applications/')) {
			return { pathType: 'applications', isProtected: false, description: '설치된 애플리케이션' };
		}
		if (absolutePath.startsWith(homedir())) {
			return { pathType: 'user', isProtected: false, description: '사용자 디렉토리' };
		}
		if (absolutePath.startsWith('/opt/homebrew/') || absolutePath.startsWith('/usr/local/')) {
			return { pathType: 'homebrew', isProtected: false, description: 'Homebrew 패키지' };
		}
		if (absolutePath.startsWith('/Library/Developer/') || absolutePath.includes('Xcode')) {
			return { pathType: 'developer', isProtected: false, description: '개발 도구' };
		}
		if (absolutePath.startsWith('/Volumes/')) {
			return { pathType: 'volumes', isProtected: false, description: '마운트된 볼륨' };
		}
...
export class LinuxPathUtils {
	...
	static classifyFHSPath(pathInfo: PathInfo): {
		category: 'root' | 'bin' | 'etc' | 'home' | 'lib' | 'opt' | 'tmp' | 'usr' | 'var' | 'dev' | 'proc' | 'sys' | 'other';
		...
		const fhsMap = [
			{ pattern: /^\/bin\//, category: 'bin' as const, critical: true, desc: '필수 바이너리' },
			{ pattern: /^\/etc\//, category: 'etc' as const, critical: true, desc: '시스템 설정' },
			{ pattern: /^\/home\//, category: 'home' as const, critical: false, desc: '사용자 홈' },
			{ pattern: /^\/lib\//, category: 'lib' as const, critical: true, desc: '필수 라이브러리' },
			{ pattern: /^\/opt\//, category: 'opt' as const, critical: false, desc: '옵션 소프트웨어' },
			{ pattern: /^\/tmp\//, category: 'tmp' as const, critical: false, desc: '임시 파일' },
			{ pattern: /^\/usr\//, category: 'usr' as const, critical: false, desc: '사용자 프로그램' },
			{ pattern: /^\/var\//, category: 'var' as const, critical: false, desc: '가변 데이터' },
			{ pattern: /^\/dev\//, category: 'dev' as const, critical: true, desc: '디바이스 파일' },
			{ pattern: /^\/proc\//, category: 'proc' as const, critical: true, desc: '프로세스 정보' },
			{ pattern: /^\/sys\//, category: 'sys' as const, critical: true, desc: '시스템 정보' },
		];
		...
		if (absolutePath === '/') {
			return { category: 'root', isSystemCritical: true, description: '루트 디렉토리' };
		}

		return { category: 'other', isSystemCritical: false, description: '기타 경로' };
	}
	...
	static analyzePermissions(pathInfo: PathInfo): {
		...
		// 시스템 디렉토리
		if (absolutePath.startsWith('/etc/') || absolutePath.startsWith('/bin/') || absolutePath.startsWith('/lib/')) {
			return { likelyOwner: 'root', likelyPermissions: '644', securityLevel: 'high' };
		}

		// 사용자 홈
		if (absolutePath.startsWith('/home/') || absolutePath.startsWith(homedir())) {
			return { likelyOwner: 'user', likelyPermissions: '755', securityLevel: 'low' };
		}

		// 임시 디렉토리
		if (absolutePath.startsWith('/tmp/')) {
			return { likelyOwner: 'user', likelyPermissions: '777', securityLevel: 'low' };
		}

		// 로그 디렉토리
		if (absolutePath.startsWith('/var/log/')) {
			return { likelyOwner: 'root', likelyPermissions: '644', securityLevel: 'medium' };
		}

		return { likelyOwner: 'unknown', likelyPermissions: '644', securityLevel: 'medium' };
	}
}
				...
				return {
					...
					windows: {
						...
						unc: pathInfo.absolute.startsWith('\\\\') ?
							WindowsPathUtils.handleUNCPath(pathInfo.absolute) : null
					}
				};
...
export async function demonstrateCrossPlatformPaths() {
	...
	pathInfos.forEach((pathInfo, i) => {
		...
		// 플랫폼별 추가 정보 표시
		if (extended.windows) {
			console.log(`   Windows: Drive ${extended.windows.drive.drive}, Type: ${extended.windows.drive.driveType}`);
		}
		if (extended.macos) {
		...
		if (extended.linux) {
			console.log(`   Linux: ${extended.linux.fhs.description} (${extended.linux.fhs.category})`);
		}
		console.log('');
	});
	...
	const testPaths = ['~/documents/file.txt', '%USERPROFILE%\\file.txt', './local/file.txt'];
