...
async function analyzeReactComponentDependencies() {
	...
	try {
		...
		console.log(
			`📊 분석 완료: ${result.stats.totalFiles}개 파일, ${result.stats.totalNodes}개 노드`,
		);
		...
		const rootComponents = components.nodes.filter((comp) => {
			const imports = hierarchy.edges.filter(
				(edge) => edge.startNodeId.toString() === comp.id?.toString(),
			);
			const imported = hierarchy.edges.filter(
				(edge) => edge.endNodeId.toString() === comp.id?.toString(),
			);
			return imports.length > 0 && imported.length === 0;
		});

		console.log(
			`🌳 루트 컴포넌트: ${rootComponents.map((c) => c.name).join(", ")}`,
		);
	...
	try {
		...
		if (highCouplingServices.length > 0) {
			...
			highCouplingServices.forEach((item) => {
				...
				console.log(
					`  - ${service}: Fan-in ${analysis.metrics.fanIn}, Fan-out ${analysis.metrics.fanOut}`,
				);
		...
		if (criticalServices.length > 0) {
			...
			criticalServices.forEach((item) => {
				const { service, analysis } = item!;
				console.log(
					`  - ${service}: ${analysis.dependents.direct.length}개 서비스가 의존`,
				);
			});
		}
		...
		validAnalyses.forEach((item) => {
			...
			if (analysis.metrics.criticalityScore > 20) {
				console.log(
					`  - ${service}: 중요도가 높음, 안정성 및 테스트 강화 필요`,
				);
			}
		});
		...
		if (result.cycles.length > 0) {
			...
			if (frequentNodes.length > 0) {
				...
				frequentNodes.forEach(([node, count]) => {
					...
					console.log(
						`    💡 이 노드를 우선적으로 리팩토링하면 여러 순환 해결 가능`,
					);
				});
			}
		} else {
...
async function analyzeNodeEcosystem() {
	...
	try {
		...
		console.log(
			`  - 중요도 점수: ${impact.metrics.criticalityScore.toFixed(1)}`,
		);

		// 3. 이웃 노드 분석
		const neighborhood =
			await analyzer.analyzeNodeNeighborhood(targetIdentifier);
		...
		if (neighborhood.clusters.length > 0) {
			...
			neighborhood.clusters.forEach((cluster, index) => {
				console.log(
					`  ${index + 1}. ${cluster.purpose} (${cluster.nodes.length}개 노드, 응집도: ${cluster.cohesion.toFixed(2)})`,
				);
			});
		}
		...
		if (impact.risks.circularDependencies.length > 0) {
			console.log(
				`  - 순환 의존성: ${impact.risks.circularDependencies.length}개`,
			);
			impact.risks.circularDependencies.forEach((risk, index) => {
				console.log(
					`    ${index + 1}. ${risk.severity} 위험: ${risk.description}`,
				);
			});
		}
		...
		if (neighborhood.immediate.outgoing.length > 0) {
			...
			const path = await analyzer.findShortestPath(
				targetIdentifier,
				targetDep.identifier,
			);
...
async function largescaleProjectAnalysis() {
	...
	try {
		...
		console.log(
			`  - 평균 파일당 처리 시간: ${(totalTime / totalFiles).toFixed(1)}ms`,
		);

		// 4. 메모리 사용량 추정
		const memoryEstimate =
			(stats.totalNodes * 0.5 + stats.totalRelationships * 0.3) / 1024; // KB 추정
		...
		if (totalTime > 60000) {
			// 1분 이상
			console.log(
				"  - 처리 시간이 깁니다. 배치 크기를 줄이거나 병렬 처리를 고려하세요.",
			);
		}

		if (inferenceTime > totalTime * 0.5) {
			console.log(
				"  - 추론 계산이 오래 걸립니다. 추론 깊이를 제한하거나 선택적 추론을 사용하세요.",
			);
		}

		if (stats.totalRelationships / stats.totalNodes > 3) {
			console.log(
				"  - 노드당 관계 수가 많습니다. 그래프 복잡도가 높으니 인덱스 최적화를 고려하세요.",
			);
		}
