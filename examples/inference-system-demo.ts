...
async function runInferenceDemo() {
  ...
  const groupedByType = allImports.reduce((acc, rel) => {
  ...
  const allDependencies = await db.queryHierarchicalRelationships('depends_on', {
    includeChildren: true,
    includeParents: false
  });

  console.log(`✅ 결과: ${allDependencies.length}개 관계 발견`);
  console.log('   계층 구조 덕분에 imports_library, imports_file도 자동 포함됨');
  ...
  try {
    ...
    console.log('   → imports_file은 transitive 타입이 아니므로 직접 추론 불가');
  ...
  const helperNodes = await db.findNodes({ sourceFiles: ['/src/utils/helpers.ts'] });
  ...
  try {
    const hierarchicalResult = await inferenceEngine.queryHierarchical('imports', {
      includeChildren: true,
      includeParents: false
    });
    ...
    const typeCount = hierarchicalResult.reduce((acc, rel) => {
      acc[rel.type] = (acc[rel.type] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);
  ...
  console.log('  2️⃣  Transitive: A→B→C 체인에서 A→C 추론 가능 (SQL Recursive CTE)');
