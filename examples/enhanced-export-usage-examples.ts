...
`;
  ...
  try {
    const parseResult = await parser.parse('/repositories.ts', complexClassCode);
    ...
    const result = extractor.extractExports(parseResult.ast, '/repositories.ts');
    ...
    result.classes.forEach(cls => {
      ...
      if (cls.methods.length > 0) {
        ...
        cls.methods.forEach(method => {
          ...
          const signature = method.parameters.map(p =>
            `${p.name}${p.optional ? '?' : ''}: ${p.type || 'any'}`
          ).join(', ');

          console.log(`     - ${modifiers.join(' ')} ${method.visibility} ${method.name}(${signature})`);
        });
      }
      ...
      if (cls.properties.length > 0) {
        ...
        cls.properties.forEach(prop => {
          ...
          console.log(`     - ${modifiers} ${prop.visibility} ${prop.name}${prop.type ? `: ${prop.type}` : ''}`);
        });
      }
    });
    ...
    // Export 유형별 분류
    console.log('\n📊 Export 유형별 분석:');
    const byType = result.exportMethods.reduce((acc, exp) => {
...
`
  ...
  results.forEach(({ file, result }) => {
    ...
    console.log(`  ✅ ${file}:`);
    console.log(`     exports: ${result.statistics.totalExports}, functions: ${result.statistics.functionExports}, classes: ${result.statistics.classExports}`);
  });
...
`;
  ...
  for (const testCase of testCases) {
    ...
    try {
      const parseResult = await parser.parse(`/${testCase.name}.ts`, testCase.code);
      ...
      // Export 추출 시도
      const result = extractor.extractExports(parseResult.ast, `/${testCase.name}.ts`);
      ...
      if (!validation.isValid) {
        ...
        console.log(`  ✅ 성공: ${result.statistics.totalExports}개 export 발견`);
      }
...
async function performanceConfigExample() {
  ...
  console.log(`  메모리 제한: ${(currentConfig.memoryLimit! / 1024 / 1024).toFixed(1)}MB`);
  ...
  console.log(`  평균 처리 속도: ${metadata.performance?.averageTimePerNode}ms/노드`);
  console.log(`  메모리 사용량: ${metadata.performance?.memoryUsage}`);
  console.log(`  시간 복잡도: ${metadata.performance?.timeComplexity}`);

  // 대용량 파일 시뮬레이션
  const largeFileCode = Array(100).fill(0).map((_, i) => `
...
`).join('\n');
  ...
  try {
    ...
    if (parseResult.ast) {
      const result = extractor.extractExports(parseResult.ast, '/large-file.ts');
      ...
      console.log(`  💾 메모리 사용: ${((memoryAfter.heapUsed - memoryBefore.heapUsed) / 1024 / 1024).toFixed(2)}MB 증가`);
    }

  } catch (error) {
    console.log(`  ❌ 처리 실패: ${error instanceof Error ? error.message : String(error)}`);
  }
...
function formatResultAsJson(result: EnhancedExportExtractionResult): string {
  return JSON.stringify({
    ...
    exports: result.exportMethods.map(exp => ({
      ...
      ...(exp.parameters && {
        parameters: exp.parameters.map(p => `${p.name}${p.optional ? '?' : ''}`)
      })
    })),
