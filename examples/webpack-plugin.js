...
class TypeScriptDependencyPlugin {
  ...
  apply(compiler) {
    ...
    compiler.hooks.compilation.tap(pluginName, (compilation) => {
      ...
      compilation.hooks.afterOptimizeModules.tapAsync(pluginName, (modules, callback) => {
        this.generateReport(compilation, modules).then(() => {
          callback();
        }).catch(callback);
      });
    });
  ...
  async analyzeModule(module, compilation) {
    ...
    try {
      ...
      console.log(`[TypeScriptDependencyPlugin] Analyzing: ${path.relative(process.cwd(), filePath)}`);
      ...
      if (result.success) {
        ...
        compilation.warnings.push(
          new Error(`[TypeScriptDependencyPlugin] Failed to analyze ${filePath}: ${result.error?.message}`)
        );
      }

    } catch (error) {
      compilation.warnings.push(
        new Error(`[TypeScriptDependencyPlugin] Error analyzing module: ${error.message}`)
      );
    }
  }
  ...
  async generateReport(compilation, modules) {
    try {
      ...
      // Check for circular dependencies
      if (this.options.failOnCircularDeps && this.circularDependencies.length > 0) {
        compilation.errors.push(
          new Error(`[TypeScriptDependencyPlugin] Circular dependencies detected: ${this.circularDependencies.length} cycles`)
        );
      }

      console.log(`[TypeScriptDependencyPlugin] Report generated: ${this.options.outputFile}`);
      console.log(`[TypeScriptDependencyPlugin] Analyzed ${this.dependencies.size} TypeScript modules`);

    } catch (error) {
      compilation.errors.push(
        new Error(`[TypeScriptDependencyPlugin] Failed to generate report: ${error.message}`)
      );
    }
  }
  ...
  generateSummary() {
    ...
    const totalDependencies = modules.reduce((sum, mod) => sum + mod.dependencies.length, 0);
    const totalImports = modules.reduce((sum, mod) => sum + mod.imports.length, 0);
    ...
    return {
      ...
      averageParseTime: modules.length > 0 ? 
        modules.reduce((sum, mod) => sum + (mod.parseTime || 0), 0) / modules.length : 0
    };
  }
  ...
  suggestOptimizations(modules) {
    ...
    const heavyModules = modules
      ...
      .map(mod => ({ module: mod.module, dependencies: mod.dependencies.length }));
  ...
  detectCircularDependencies(filePath, dependencies) {
    ...
    dependencies.forEach(dep => {
      if (dep.source.includes(baseName) && !this.isExternalDependency(dep.source)) {
...
async function standaloneExample() {
  ...
  console.log('Report content:', JSON.stringify(mockCompilation.assets[plugin.options.outputFile].source(), null, 2));
}
