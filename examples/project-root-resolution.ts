  ...
  for (const testFile of testFiles) {
    ...
    resolvedPaths.forEach((resolved, original) => {
      if (resolved) {
        ...
        console.log(`  ${original.padEnd(25)} → ${relativePath} ${isInternal ? '🏠' : '📦'}`);
      } else {
...
async function integrateWithTypeScriptConfig() {
  ...
  projectRelativeResults.forEach((dep: any) => {
    if (dep.resolutionType === 'relative' || dep.resolutionType === 'alias') {
      console.log(`  ${dep.originalSource} → ${dep.projectRelativePath || dep.resolvedPath}`);
    }
  });
...
async function monorepoPathResolution() {
  ...
  for (const packagePath of packages) {
    ...
    const resolvedPaths = await batchResolvePaths(crossPackageDependencies, context);
    ...
    resolvedPaths.forEach((resolved, original) => {
      if (resolved) {
        ...
        if (isWithinPackage) indicator = '🏠'; // 같은 패키지
        else if (isWithinMonorepo) indicator = '🔗'; // 다른 패키지

        console.log(`  ${original.padEnd(30)} → ${indicator} ${path.relative(monorepoRoot, resolved)}`);
      } else {
...
async function dynamicPathResolution() {
  ...
  const scenarios = [
    {
      ...
      after: ['./Button.tsx', '../shared/utils/index.ts', '@/components/shared/NewButton.tsx']
    },
    {
      name: '디렉토리 구조 변경',
  ...
  for (const scenario of scenarios) {
    ...
    console.log(`  변경 전: 해결 ${beforeResult.summary.resolvedCount}/${beforeResult.summary.totalDependencies}`);
    console.log(`  변경 후: 해결 ${afterResult.summary.resolvedCount}/${afterResult.summary.totalDependencies}`);

    // 공통 기준 경로 찾기
    const allPaths = [
      ...beforeResult.resolvedDependencies.map(d => d.resolvedPath).filter(Boolean),
      ...afterResult.resolvedDependencies.map(d => d.resolvedPath).filter(Boolean)
    ] as string[];
...
async function performanceOptimizedResolution() {
  ...
  largeDependencySet.push(...[
    ...
    'node:fs', 'node:path', 'node:util', 'node:crypto'
  ]);
  ...
  console.log(`  처리 속도: ${Math.round(largeDependencySet.length / duration * 1000)} 의존성/초`);
  console.log(`  해결된 경로: ${resolved}개`);
  console.log(`  미해결 경로: ${unresolved}개`);
  console.log(`  성공률: ${Math.round(resolved / largeDependencySet.length * 100)}%`);
