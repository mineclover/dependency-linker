...
async function basicExample() {
	...
	try {
		...
		const result1 = await analyzeTypeScriptFile(
		...
		const result3 = await analyzer.analyzeFile(
		...
		if (result3.success) {
			...
			console.log(
				`  📦 Dependencies: ${result3.dependencies.map((d) => d.source).join(", ")}`,
			);
		...
		const result5 = await analyzer.analyzeFile(
			"./demo/examples/simple-component.tsx",
		);
...
async function runWithPerformanceMonitoring() {
	...
	console.log(
		`Peak memory usage: ${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB`,
	);
}
