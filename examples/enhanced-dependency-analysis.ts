...
`;
  ...
  try {
    ...
    const parseResult = await parser.parse('/example/UserProfile.tsx', sampleCode);
    ...
    const result = enhancedExtractor.extractEnhanced(parseResult.ast, '/example/UserProfile.tsx');
    ...
    result.enhancedDependencies.forEach(dep => {
      console.log(`\n📦 ${dep.source}:`);
      console.log(`  임포트된 항목: ${dep.importedNames?.join(', ') || 'None'}`);
      ...
      if (dep.usedMethods && dep.usedMethods.length > 0) {
        ...
        dep.usedMethods.forEach(method => {
          console.log(`    - ${method.methodName}: ${method.callCount}회 (${method.usageType})`);
          console.log(`      위치: ${method.locations.map(loc => `L${loc.line}:${loc.column}`).join(', ')}`);
        });
      }
    ...
    result.usageAnalysis.mostUsedMethods.slice(0, 5).forEach((item, index) => {
      console.log(`  ${index + 1}. ${item.method} (${item.source}): ${item.count}회`);
    });
...
`;
  ...
  try {
    const parser = new TypeScriptParser();
    const parseResult = await parser.parse('/example/optimization.ts', lodashHeavyCode);

    if (!parseResult.ast) return;

    const enhancedExtractor = new EnhancedDependencyExtractor();
    const result = enhancedExtractor.extractEnhanced(parseResult.ast, '/example/optimization.ts');
    ...
    result.enhancedDependencies.forEach(dep => {
      if (dep.source === 'lodash') {
        ...
        if (dep.usedMethods) {
          ...
          const namedImportMethods = dep.usedMethods.filter(m => !m.methodName.startsWith('_'));
        ...
        if (dep.unusedImports && dep.unusedImports.length > 0) {
          console.log(`  🗑️  제거 가능한 import: ${dep.unusedImports.join(', ')}`);
        }
      }
    });
...
`;
  ...
  try {
    const parser = new TypeScriptParser();
    const parseResult = await parser.parse('/example/Dashboard.tsx', complexCode);

    if (!parseResult.ast) return;

    const enhancedExtractor = new EnhancedDependencyExtractor();
    const result = enhancedExtractor.extractEnhanced(parseResult.ast, '/example/Dashboard.tsx');
    ...
    packageUtilization.forEach(pkg => {
      ...
      console.log(`    활용률: ${pkg.utilizationRate.toFixed(1)}% (${pkg.usedCount}/${pkg.totalCount})`);
...
function calculatePackageUtilization(result: any) {
  ...
  result.enhancedDependencies.forEach((dep: any) => {
    ...
    const coreMethods = dep.usedMethods
