...
async function basicPathResolverExample() {
	...
	result.resolvedDependencies.forEach((dep) => {
		console.log(
			`  ${dep.originalSource} → ${dep.resolvedPath || "UNRESOLVED"}`,
		);
		console.log(
			`    타입: ${dep.resolutionType}, 존재: ${dep.exists ? "✅" : "❌"}`,
		);
	});
...
`;
	...
	try {
		...
		// 타입별 분류
		const dependencySources = result.resolvedDependencies.map(
		...
		// 프로젝트 내부 파일만 필터링
		const internalFiles = result.resolvedDependencies.filter(
			(dep) =>
		...
		internalFiles.forEach((dep) => {
			...
			console.log(
				`    존재: ${dep.exists ? "✅" : "❌"}, 타입: ${dep.resolutionType}`,
			);
		});
