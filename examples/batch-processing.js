...
async function batchProcessingExample() {
	...
	try {
		...
		const results1 = await getBatchAnalysis(filePaths, {
			...
			onProgress: (completed, total) => {
				...
				process.stdout.write(
					`\r  Progress: ${completed}/${total} (${percentage}%)`,
				);
			},
		});
		...
		results1.forEach((result) => {
			if (result.success) {
				console.log(
					`  ✅ ${path.basename(result.filePath)}: ${result.dependencies.length} deps`,
				);
			} else {
				console.log(
					`  ❌ ${path.basename(result.filePath)}: ${result.error?.message}`,
				);
			}
		});
		console.log();
		...
		const totalDeps = dirResults.reduce(
		...
		const batchResult = await batchAnalyzer.processBatch(filePaths, {
			...
			onProgress: (completed, total) => {
				const percentage = Math.round((completed / total) * 100);
				process.stdout.write(
					`\r  Advanced progress: ${completed}/${total} (${percentage}%)`,
				);
			},
		...
		console.log("\n");
		console.log(
			`  ✅ Batch completed: ${batchResult.summary.successfulFiles}/${batchResult.summary.totalFiles} files`,
		);
		console.log(`  📊 Summary:`, {
		...
		console.log(
			`  ✅ Completed operations: ${resourceMetrics.completedOperations}`,
		);
		...
		console.log(
			`    ✅ Best effort: ${bestEffortResult.summary.successfulFiles} successful, ${bestEffortResult.errors.length} errors\n`,
		);
