...
async function testTypeImports() {
  ...
  const parseResult = await parser.parse('/TypeImportTest.tsx', typeImportTestCode);
  ...
  try {
    ...
    const typeQuery = new Parser.Query(parser.getGrammar(), TYPED_TYPE_IMPORTS_QUERY.query);
    ...
    if (typeResult.success && typeResult.results.length > 0) {
      ...
      typeResult.results.forEach((result: TypeImportResult, index: number) => {
        console.log(`    ${index + 1}. ${result.typeName}${result.alias ? ` as ${result.alias}` : ''} from "${result.source}"`);
        console.log(`       타입: ${result.importType}`);
        console.log(`       위치: ${result.location.line}:${result.location.column}`);
      });

      // 타입별 분류
      const byImportType = typeResult.results.reduce((acc: Record<string, number>, result: TypeImportResult) => {
  ...
  try {
    ...
    const sourcesQuery = new Parser.Query(parser.getGrammar(), TYPED_IMPORT_SOURCES_QUERY.query);
    ...
    // Named Imports 분석
    const namedQuery = new Parser.Query(parser.getGrammar(), TYPED_NAMED_IMPORTS_QUERY.query);
    ...
    Array.from(allSources).forEach(source => {
      const namedCount = namedResult.results.filter((r: NamedImportResult) => r.source === source).length;
      const typeCount = typeResult.results.filter((r: TypeImportResult) => r.source === source).length;

      console.log(`    "${source}": Named ${namedCount}개, Type ${typeCount}개`);
    });
  ...
  if (typeResult.success) {
    ...
    if (patterns.aliasedTypes.length > 0) {
      ...
      patterns.aliasedTypes.forEach(type => {
        console.log(`    - ${type.typeName} as ${type.alias} from "${type.source}"`);
      });
    }
  }
